...
class StackedResidualAtlasRegressor(BaseEstimator, RegressorMixin):
    """OOF-weighted tree stack with residual correction and prompt-aligned query atlas."""
    ...
    def fit(self, X, y):
        ...
        blend_imp = w[0] * hgb_imp + w[1] * rf_imp + w[2] * et_imp + 0.4 * w[3] * ridge_imp
    ...
    def predict(self, X):
        ...
        return base_pred + self.residual_ridge_.predict(Xf)
    ...
    def __str__(self):
        ...
        lines = [
            "Stacked Residual Atlas Regressor (Prompt-Aligned)",
            ...
            f"blend_weights[hgb, rf, et, ridge]: [{self.blend_weights_[0]:.3f}, {self.blend_weights_[1]:.3f}, {self.blend_weights_[2]:.3f}, {self.blend_weights_[3]:.3f}]",
...
StackedResidualAtlasRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "StackedResidualAtlas_v4"
model_description = "Reliability-aware OOF blend of HistGBR/RF/ExtraTrees/Ridge with residual ridge and expanded prompt-aligned atlas probes for hard mixed-sign and triple-interaction simulation"
model_defs = [(model_shorthand_name, StackedResidualAtlasRegressor())]
