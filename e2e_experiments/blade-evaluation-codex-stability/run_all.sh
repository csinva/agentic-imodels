...
run_dataset() {
    ...
    # Hard 30-minute timeout per dataset to avoid infinite hangs.
    timeout 1800 npx @openai/codex exec \
        --config model_reasoning_effort="high" \
        --sandbox danger-full-access \
        "Read the file AGENTS.md for instructions. Then write a python script called analysis.py and run it with python3 analysis.py. The script must create conclusion.txt. Do NOT just show code - you must actually write the file and execute it." || echo "Codex exited non-zero (timeout or error) for $dataset"
...
for dataset in "${DATASETS[@]}"; do
    ...
    # Don't suppress errors — a silent refresh failure leaves a stale token
    # and codex gets 401s for the rest of the run.
    refresh_token || true
    if [[ -z "$AZURE_OPENAI_API_KEY" ]]; then
        echo "ERROR: empty AZURE_OPENAI_API_KEY after refresh; aborting."
        exit 2
    fi
