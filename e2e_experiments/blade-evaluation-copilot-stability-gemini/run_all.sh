#!/bin/bash
# Run GitHub Copilot CLI (Gemini 2.5 Pro variant) on all 13 Blade datasets.
# Stability sweep: tests whether the gain from naming a specific interp
# library in the prompt depends on which library (imodels vs interpretml).
#
# Notes on the binary:
#   The default copilot binary at ~/.local/bin/copilot uses the precompiled
#   Node SEA build, which hides Gemini models in a hardcoded blocklist
#   (l_n in app.js). We use the JS bundle path via Node 24 + a patched app.js
#   instead, exposed through the wrapper at ~/.local/bin/copilot-gemini.
#
# Prerequisites:
#   1. Copilot CLI authenticated (interactive `/login` or COPILOT_GITHUB_TOKEN)
#   2. ~/.local/bin/copilot-gemini wrapper exists (Node 24 -> patched app.js)
#   3. python prepare_run.py --mode <standard|interpretml>
#
# Usage:
#   bash run_all.sh --mode standard              # imodels prompt
#   bash run_all.sh --mode interpretml           # interpretml prompt
...
MODE="standard"
CUSTOM_OUTPUT_DIR=""
COPILOT_BIN="${COPILOT_BIN:-$HOME/.local/bin/copilot-gemini}"
COPILOT_MODEL="${COPILOT_MODEL:-gemini-2.5-pro}"
