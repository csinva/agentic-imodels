...
# Source dataset directory: try the upstream Blade dataset dump first, then
# fall back to any sibling pipeline run dir that already has per-dataset
# info.json + CSVs prepared.
_CANDIDATES = [
    os.path.join(SCRIPT_DIR, "..", "example-blade-repo", "blade_bench", "datasets"),
    os.path.join(SCRIPT_DIR, "..", "blade-evaluation", "outputs_standard_run1"),
...
DATASETS_DIR = next((p for p in _CANDIDATES if os.path.isdir(p)), os.path.join(SCRIPT_DIR, "outputs"))
