"""gosdt-guesses with its hyperparameters exposed, so the baseline can be judged at more
than one setting.

``guesses_solver.py`` pins the paper's headline recipe (40 stumps of depth 1, no depth
budget) and is not to be modified; this is the same wrapper with those numbers, the depth
budget and ``similar_support`` as arguments, so a small sweep can find the setting that
serves gosdt-guesses best on this suite. Everything else -- the binarization, the child
process, the memory cap, the decoding, the rule that a guessed search space is not a
certificate -- is unchanged, so results stay comparable with the pinned configuration.
"""
...
def _fit_child(payload: dict) -> dict:
    ...
    if payload["guesses"]:
        ...
        guesser = ThresholdGuessBinarizer(n_estimators=payload["n_estimators"],
                                          max_depth=payload["guess_depth"], random_state=2021)
        guesser.set_output(transform="pandas")
        Xg = guesser.fit_transform(frame, y)
        gb = GradientBoostingClassifier(n_estimators=payload["n_estimators"],
                                        max_depth=payload["guess_depth"], random_state=42)
        gb.fit(Xg, y)
        y_ref = gb.predict(Xg)
...
class GuessesGOSDTHP:
    """`depth_budget=0` means the argument is not passed at all, since this API reads a
    budget of 0 literally and returns the root as a leaf; `n_estimators`/`guess_depth` size
    the threshold guesser."""
    ...
    def __init__(self, regularization: float, time_limit: float, memory_limit: int = 6 * (1 << 30),
                 ...
                 n_estimators: int = 40, guess_depth: int = 1, depth_budget: int = 0):
    ...
    def fit(self, X: pd.DataFrame, y):
        payload = {"X": X.to_numpy().tolist(), "columns": [str(c) for c in X.columns],
                   ...
                   "n_estimators": int(self.n_estimators), "guess_depth": int(self.guess_depth),
        ...
        restricted = self.guesses or self.depth_budget
        self.optimal_ = bool(converged and not restricted)
        if restricted:
