...
# numba cannot link a self-recursive function against disk-cached callees; set NUMBA_CACHE=0
...
MODEL_NAME = "v49_cands_pairs_lazy_ws"
...
MULTICORE = True
DESCRIPTION = ("v48 with three changes for data unlike the suite: (1) the pair stage runs over the surviving candidates only when they are few and that is cheap (|candidates| * nv * W <= 131072 word operations; new kernel depth2_cands), with the non-candidates keeping their own bounds and bounding >=4-leaf trees by their split bound, so nodes with many features and few candidates get exact two-level values instead of no pair stage at all (iris 2.5x faster); (2) frame workspaces start with 16 slots and double on demand (a 26k-feature table cost 634 MB per thread); (3) one binarisation pass instead of two.  Exact; the search is otherwise v46's")
...
@njit(cache=NUMBA_CACHE, nogil=True)
def depth2_topk(F, feats, rows, group_of, masks, costs, lam, dist, L, idx, out_ml2_l, out_ml2_r, out_j_l, out_j_r):
    """Best 2-leaf loss of both children for the splits listed in ``idx`` only, against
    every other split (a budgeted version of ``depth2_pairs`` for incumbents: no bounds
    are derived from it).  Outputs for splits not in ``idx`` are left untouched."""
    mf = feats.shape[0]
    W = F.shape[1]
    K = dist.shape[0]
    ij = np.empty(K)
    ...
    for t in range(idx.shape[0]):
        ...
        for j in range(mf):
            ...
            fj = feats[j]
            frj = rows[j]
            nested = gi >= 0 and group_of[fj] == gi
            if nested and j > i:
                for k in range(K):
                    ij[k] = L[j, k]          # higher threshold: left_j inside left_i
            elif nested:
                for k in range(K):
                    ij[k] = L[i, k]          # lower threshold: left_i inside left_j
            else:
                for k in range(K):
                    acc = np.uint64(0)
                    for w in range(W):
...
@njit(cache=NUMBA_CACHE, nogil=True)
def depth2_cands(F, feats, rows, group_of, masks, costs, lam, dist, L, idx, out_ml2_l, out_ml2_r, out_j_l, out_j_r,
                 out_c3_l, out_c3_r):
    """``depth2_pairs`` restricted to the splits in ``idx`` (the candidates) against every
    other split: exact 2-leaf values and peeled-leaf costs for the candidates' children,
    no information (1e300) for the rest.  Costs |idx| * nv pair evaluations instead of
    nv^2 / 2, which is what a node with few surviving candidates needs."""
    mf = feats.shape[0]
    W = F.shape[1]
    K = dist.shape[0]
    for i in range(mf):
...
@njit(cache=NUMBA_CACHE, nogil=True)
def shape_bound(l_leaf, ml2_l, c3_l, r_leaf, ml2_r, c3_r, lam, l_lb, r_lb, l_ub2, r_ub2, split_lb, split_ub2, has_pairs):
    ...
    for i in range(mf):
        if not has_pairs[i]:
            # no pair information for this split (not a candidate): its children keep
            # their bounds, the leaves are its only achievable subtrees here, and a tree
            # with >= 4 leaves rooted at it is bounded by its split bound
            l_ub2[i] = l_leaf[i]; r_ub2[i] = r_leaf[i]
            split_lb[i] = l_lb[i] + r_lb[i]
            split_ub2[i] = 1e300
            if split_lb[i] < lb_ge4:
                lb_ge4 = split_lb[i]
            continue
...
@njit(cache=NUMBA_CACHE, nogil=True)
def expand_kernel(F, features, group_of, kw, mask_matrix, weights, costs, diff, lam, bound, do_exchange, pfb,
                  io, fo, bo, L, dist):
    ...
    if single:
        for t in range(1, nv):
            if gidx[t] != gidx[0]:
                ...
                break
    cheap = nv * nv * W <= 32768
    # the pair stage over the candidates only when they are few and that is cheap (|cands| *
    # nv pair evaluations within a few times the "cheap" threshold); otherwise the full
    # stage under its old rule (many candidates, or cheap), else no stage
    restricted = n_cand * 2 < nv and float(n_cand) * float(nv) * float(W) <= RESTRICTED_PAIR_WORK
    full_ok = (not (n_cand * 2 < nv)) and (cheap or (n_cand >= 8 and nv >= 8))
    pair_work = (float(n_cand) * float(nv) if restricted else 0.5 * float(nv) * float(nv)) * float(K) * float(W)
    pair_cap = pfb[PF_PAIRCAP] if pfb.shape[0] > PF_PAIRCAP else 0.0
    if pair_cap > 0.0 and pfb.shape[0] > PF_N:
        tot0 = 0.0
        for k in range(K):
            tot0 += dist[k]
        if tot0 >= pfb[PF_N] - 0.5:
            pair_cap *= ROOT_PAIR_MULT      # the root is expanded once (cached): it may spend more
    if pair_cap > 0.0 and (restricted or full_ok or n_cand * 2 < nv) and pair_work > pair_cap:
        # over the per-expansion share of the time budget: evaluate the pair stage for the k
        # most promising splits only (by leaf-sum value), for the incumbent it yields - no
        # bounds from it (signalled by lb_ge4 = -1 with ran = False); the work is charged to
        # a global budget for the fit (pfb[PF_PARTIAL]) so a few large nodes get it, not all
        row_cost = max(1.0, float(nv) * float(K) * float(W))
        tot = 0.0
        for k in range(K):
            tot += dist[k]
        is_root = pfb.shape[0] > PF_N and tot >= pfb[PF_N] - 0.5
        kk = int((pfb[PF_PARTIAL] if is_root else min(pair_cap, pfb[PF_PARTIAL])) / row_cost)
        if kk < 2 or n_cand < 2:
            return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0, M, Fc
        kk = min(kk, nv)
        pfb[PF_PARTIAL] -= kk * row_cost
        # rank the splits by impurity (gini) gain: a root split worth splitting further is
        # not the one with the best leaf-sum value
        gain = np.empty(nv)
        rc = np.empty(K)
        base = _impurity(dist[:K], K, 1)
        for t in range(nv):
            for k in range(K):
                rc[k] = dist[k] - L[t, k]
            gain[t] = -(base - _impurity(L[t], K, 1) - _impurity(rc, K, 1))
        order = np.argsort(gain)
        idx = order[:kk].copy()
        for t in range(nv):
            fo[11, t] = fo[0, t]
            fo[12, t] = fo[2, t]
        depth2_topk(Fc, feats, rows, group_of, M[:K], costs, lam, dist[:K], L[:nv], idx, fo[7, :nv], fo[8, :nv], io[2, :nv],
                    io[3, :nv])
        bd2 = 1e300
        i_d2 = 0
        for t in range(kk):
            i = idx[t]
            ul = min(fo[0, i], 2.0 * lam + fo[7, i]); ur = min(fo[2, i], 2.0 * lam + fo[8, i])
            fo[11, i] = ul; fo[12, i] = ur
            if ul + ur < bd2:
                bd2 = ul + ur; i_d2 = i
        return nv, n_cand, best_i, min_rejected, False, i_d2, bd2, -1.0, M, Fc
    if single or n_cand < 2 or nv < 2 or not (restricted or full_ok):
        return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0, M, Fc
    has_pairs = np.ones(nv, dtype=np.bool_)
    if restricted:
        cands = io[1, :n_cand].copy()
        has_pairs[:] = False
        for t in range(n_cand):
            has_pairs[cands[t]] = True
        depth2_cands(Fc, feats, rows, group_of, M[:K], costs, lam, dist[:K], L[:nv], cands, fo[7, :nv], fo[8, :nv],
                     io[2, :nv], io[3, :nv], fo[9, :nv], fo[10, :nv])
    else:
    ...
    i_d2, best_d2, lb_ge4 = shape_bound(fo[0, :nv], fo[7, :nv], fo[9, :nv], fo[2, :nv], fo[8, :nv], fo[10, :nv], lam,
                                        fo[1, :nv], fo[3, :nv], fo[11, :nv], fo[12, :nv], fo[5, :nv], fo[13, :nv], has_pairs)
...
RESTRICTED_PAIR_WORK = float(os.environ.get("RESTRICTED_PAIR_WORK", "131072"))   # |cands| * nv * W allowed for the candidate-only pair stage
PAIR_TIME_FRAC = float(os.environ.get("PAIR_TIME_FRAC", "0.1"))    # share of the time limit one expansion's pair stage may cost
PAIR_OP_SECONDS = float(os.environ.get("PAIR_OP_SECONDS", "4e-10"))  # measured: ~0.4 ns per word operation of the pair kernel
PARTIAL_TIME_FRAC = float(os.environ.get("PARTIAL_TIME_FRAC", "0.2"))  # share of the time limit all budgeted pair stages of a fit may cost
ROOT_PAIR_MULT = float(os.environ.get("ROOT_PAIR_MULT", "2.5"))        # the root's full pair stage may cost this many per-expansion shares
GREEDY_SEED_NODES = int(os.environ.get("GREEDY_SEED_NODES", "256"))  # lookahead-greedy seed: expansions allowed (0 = off)
SEED_TIME_FRAC = float(os.environ.get("SEED_TIME_FRAC", "0.25"))      # share of the time limit the seed may use
SEED_TIME_MAX = float(os.environ.get("SEED_TIME_MAX", "0.1"))         # ... and at most this many seconds
SEED_MODES = [int(v) for v in os.environ.get("SEED_MODES", "0,1").split(",")]   # 0 lookahead, 1 gini, 2 entropy, 3 error
LNS_TIME_FRAC = float(os.environ.get("LNS_TIME_FRAC", "0.4"))          # share of the time limit for the bottom-up repair (0 = off)
LNS_TIME_MAX = float(os.environ.get("LNS_TIME_MAX", "0.2"))            # ... and at most this many seconds (the repair is a seed, not the search)
EXTRAS_MAX_CAP = float(os.environ.get("EXTRAS_MAX_CAP", "1.0"))        # gain seeds and repair only under caps up to this (seconds)
...
def warm_up():
    ...
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), int_to_words(1, 1), masks, np.zeros(0),
                  np.zeros((1, 1)), np.zeros(1), 0.1, 1.0, False, np.zeros(6), np.empty((7, 1), dtype=np.int64), np.empty((14, 1)),
                  np.empty((2, 1), dtype=np.bool_), np.empty((1, 1)), np.empty(2))
    shape_bound(e, e, e, e, e, e, 0.1, np.zeros(1), np.zeros(1), np.empty(1), np.empty(1), np.empty(1), np.empty(1), np.ones(1, dtype=np.bool_))
...
def boost_stumps(Xb: np.ndarray, y_idx: np.ndarray, n_classes: int, rounds: int, max_rows: int = 4000,
                 seed: int = 0):
    """SAMME-style AdaBoost of weighted-majority stumps on the binarised data (numpy only):
    returns (sorted distinct feature indices used, predicted class per row of the full Xb).
    Threshold guessing and lower-bound guessing after McTavish et al. (2022)."""
    rng = np.random.default_rng(seed)
    n, m = Xb.shape
    rows = np.arange(n) if n <= max_rows else rng.choice(n, max_rows, replace=False)
    X = Xb[rows].astype(np.float32)
    y = y_idx[rows]
    K = n_classes
    Y = np.zeros((K, rows.shape[0]), dtype=np.float32)
    Y[y, np.arange(rows.shape[0])] = 1.0
    w = np.full(rows.shape[0], 1.0 / rows.shape[0], dtype=np.float32)
    used = []
    votes = np.zeros((n, K))
    for _ in range(rounds):
        WY = Y * w
        left = WY @ X
        total = WY.sum(axis=1, keepdims=True)
        right = total - left
        err = 1.0 - (left.max(axis=0) + right.max(axis=0))
        j = int(np.argmin(err))
        e = float(err[j])
        pred_left = int(np.argmax(left[:, j])); pred_right = int(np.argmax(right[:, j]))
        if e <= 1e-12 or e >= 1.0 - 1.0 / K:
            if j not in used:
                used.append(j)
            full = np.where(Xb[:, j], pred_left, pred_right)
            votes[np.arange(n), full] += 1.0 if e <= 1e-12 else 0.0
            break
        alpha = float(np.log((1.0 - e) / e) + np.log(K - 1.0))
        pred = np.where(X[:, j] > 0.5, pred_left, pred_right)
        wrong = pred != y
        w = w * np.exp(alpha * wrong.astype(np.float32))
        w /= w.sum()
        if j not in used:
            used.append(j)
        full = np.where(Xb[:, j], pred_left, pred_right)
        votes[np.arange(n), full] += alpha
    return np.array(sorted(used), dtype=np.int64), np.argmax(votes, axis=1)
...
class BinaryEncoder:
    ...
    def fit(self, X) -> "BinaryEncoder":
        ...
        if self.drop_duplicate_columns and rules:
            ...
            if keep.shape[0] != len(rules):
                ...
                Xb = np.ascontiguousarray(Xb[:, keep])
            self._fit_matrix = Xb            # fit_transform returns it without a second pass
        return self
    ...
    def fit_transform(self, X) -> np.ndarray:
        self.fit(X)
        if getattr(self, "_fit_matrix", None) is not None:
            out = self._fit_matrix
            self._fit_matrix = None
            return out
    ...
    def _apply_rules(X: pd.DataFrame, rules: list[dict]) -> np.ndarray:
        """Binary matrix of the rules, one vectorised operation per source column: the
        thresholds of a numeric column are applied as a single broadcast comparison and the
        categories of a categorical column through integer codes (a wide table with tens of
        thousands of rules binarises in well under a second instead of a minute)."""
        n = X.shape[0]
        out = np.zeros((n, len(rules)), dtype=bool)
        by_col: dict[int, list[int]] = {}
        for k, rule in enumerate(rules):
            by_col.setdefault(rule["feature"], []).append(k)
        for j, ks in by_col.items():
            kinds = {rules[k]["type"] for k in ks}
            if kinds == {"categorical"}:
                raw = X.iloc[:, j].to_numpy(dtype=object)
                missing = np.array([_is_missing(v) for v in raw], dtype=bool)
                keys = np.array([str(v) if not m else "" for v, m in zip(raw, missing)], dtype=object)
                codes, uniq = pd.factorize(keys, sort=False)
                index = {u: c for c, u in enumerate(uniq)}
                for k in ks:
                    c = index.get(str(rules[k]["reference"]), -1)
                    out[:, k] = (codes == c) & ~missing
                continue
            col = pd.to_numeric(X.iloc[:, j], errors="coerce").to_numpy(dtype=np.float64)
            ge = [k for k in ks if rules[k]["relation"] == ">="]
            eq = [k for k in ks if rules[k]["relation"] != ">="]
            with np.errstate(invalid="ignore"):
                if ge:
                    refs = np.array([float(rules[k]["reference"]) for k in ge])
                    out[:, ge] = col[:, None] >= refs[None, :]
                for k in eq:
                    out[:, k] = col == rules[k]["reference"]
        return out
...
class BitDataset:
    ...
    def __init__(self, Xb: np.ndarray, y: np.ndarray, n_classes: int,
        ...
        # (grouping by a hash of each packed row: np.unique sorts 3 KB records on wide data)
        packed_rows = np.packbits(np.ascontiguousarray(Xb), axis=1)
        group_of_row: dict[bytes, int] = {}
        inverse = np.empty(n, dtype=np.int64)
        for i in range(n):
            inverse[i] = group_of_row.setdefault(packed_rows[i].tobytes(), len(group_of_row))
        n_groups = len(group_of_row)
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, M, Fc = expand_kernel(
            ...
            self.continuous_feature_exchange and self._has_groups, np.zeros(6), io, fo, bo, L, dist)
        if nv == 0:
            return None
        nv = int(nv)
        if nv < mf:
...
DT_F, DT_GROUP, DT_MASKS, DT_WEIGHTS, DT_COSTS, DT_COSTS_T, DT_DIFF, DT_REF = range(8)
...
PF_LAM, PF_UW, PF_N, PF_PAIRCAP, PF_PARTIAL, PF_DEPTH = range(6)
...
@njit(cache=NUMBA_CACHE, nogil=True)
def _ref_loss(kw, ref, uw):
    """Loss of the reference model on the capture (the lower-bound guess of gosdt-guesses):
    the rows it misclassifies, weighted; 0 when no reference is set."""
    acc = np.uint64(0)
    for w in range(kw.shape[0]):
        acc += _popcount64(kw[w] & ref[w])
    return float(acc) * uw
...
def make_node(st, dat, pf, pi, kw):
    ...
    count, max_loss, min_loss, potential, pred = leaf_stats_words(kw, dat[DT_MASKS], dat[DT_WEIGHTS], dat[DT_COSTS],
                                                                  dat[DT_DIFF], K)
    g = _ref_loss(kw, dat[DT_REF], pf[PF_UW])
    if g > min_loss:
        min_loss = g                     # reference lower-bound guess (approximate mode only)
...
def child_node(st, dat, pf, pi, parent, f, left, leaf, lb, solved, pred, out):
    ...
    if not solved:
        g = _ref_loss(out, dat[DT_REF], pf[PF_UW]) + pf[PF_LAM]
        if g > lb:
            lb = min(g, leaf)            # reference lower-bound guess (approximate mode only)
...
WS_INIT_SLOTS = int(os.environ.get("WS_INIT_SLOTS", "16"))    # frame slots at first (depth 7); grown x2 on demand up to NSLOT


def make_workspace(m, K, W, slots=0):
    """Per-thread workspace; pooled by shape across fits (the frame cache flags are reset).
    Frame slots start small and grow on demand (a wide table costs megabytes per slot)."""
    key = (m, K, W)
    ...
    S = min(NSLOT, max(2, slots if slots > 0 else WS_INIT_SLOTS))
    return (np.zeros((S, 18), dtype=np.int64), np.zeros((S, 16)), np.zeros((S, 7, m), dtype=np.int64),
            np.zeros((S, 14, m)), np.zeros((S, 2, m), dtype=np.bool_), np.zeros((S, m, K)),
            np.zeros((S, K + 1)), np.zeros((S, m), dtype=np.int64), np.zeros((S, m)),
            np.zeros((S, m3, 2, 6), dtype=np.int64), np.zeros((S, 4, m)), np.zeros((S, 2, m), dtype=np.int64),
            np.zeros(W, dtype=np.uint64), np.zeros(W, dtype=np.uint64), np.arange(m, dtype=np.int64),
            np.zeros(MAXD + 1, dtype=np.int64))


def grow_workspace(ws, m, K, W):
    """A workspace with twice the frame slots, the existing frames copied (the search
    re-enters from the root, frames re-armed from the cache where still valid)."""
    old = ws[WS_FI].shape[0]
    new = make_workspace(m, K, W, slots=min(NSLOT, old * 2))
    for a in (WS_FI, WS_FF, WS_IO, WS_FO, WS_BO, WS_L, WS_DIST, WS_GIDX, WS_LBARR, WS_ARG3, WS_F3, WS_K3):
        new[a][:old] = ws[a]
    for a in (WS_KW, WS_KW2, WS_ROOTFEATS, WS_SLOT):
        new[a][:] = ws[a]
    return new
...
@njit(cache=NUMBA_CACHE, nogil=True)
def greedy_seed(st, dat, pf, pi, ws, root, max_depth, max_nodes, mode):
    """Lookahead-greedy incumbent before the search: at each node the frame expansion's
    best tree (a leaf, a split with leaf children, or the best two-level tree of the pair
    stage) chooses the split, both children are seeded the same way, and the greedy
    subtree's value is installed on the node when it beats what the node holds.  Every
    structure it writes is consistent (children exist with their own trees), so the search
    starts from a real incumbent instead of the root's leaf risk.  Returns the nodes
    expanded; stops after ``max_nodes`` (the rest keep their expansion trees)."""
    FI = ws[WS_FI]; FF = ws[WS_FF]
    nub = st[ST_UB]; nsplit = st[ST_SPLIT]; nsolved = st[ST_SOLVED]; npend = st[ST_PEND]; nleaf = st[ST_LEAF]
    meta = st[ST_META]
    kw_buf = ws[WS_KW]; kw_buf2 = ws[WS_KW2]
    meta[3] = meta[1] + 10 ** 9
    n_exp = 0
    d = ws[WS_SLOT][MAXD]           # resumable: the depth saved by a previous call
    if d == -2:
        return 0
    if d < 0:
        d = 0
        ws[WS_SLOT][0] = 0
        FI[0, FI_NODE] = root; FF[0, FF_BUDGET] = 1e300; FI[0, FI_PHASE] = 0
    while d >= 0:
        ps = ws[WS_SLOT][d]
        node = FI[ps, FI_NODE]
        phase = FI[ps, FI_PHASE]
        if phase == 0:
            if nsolved[node] == 1 or d >= max_depth:
                d -= 1
                continue
            if n_exp >= max_nodes:
                ws[WS_SLOT][MAXD] = d      # out of budget: resume here next time
                return n_exp
            n_exp += 1
            resolved = _expand_frame(st, dat, pf, pi, ws, d)
            if meta[2] != 0:
                ws[WS_SLOT][MAXD] = d
                return n_exp
            if resolved:
                d -= 1
                continue
            f = FI[ps, FI_BEST_SPLIT]
            if mode == 0:
                # lookahead: the expansion's best tree chooses; stop where no tree beats the leaf
                if f < 0 or FF[ps, FF_BEST] >= nleaf[node] - EPS:
                    d -= 1
                    continue
            else:
                # impurity gain (1 gini, 2 entropy, 3 error) from the per-split class counts;
                # grows past unprofitable splits, the bottom-up install prunes them under lambda
                f = _gain_split(ws, ps, mode)
                if f < 0:
                    d -= 1
                    continue
            ...
            if ln < 0 or rn < 0:
                ...
                ws[WS_SLOT][MAXD] = d
                return n_exp
            FI[ps, FI_LN] = ln; FI[ps, FI_RN] = rn; FI[ps, FI_II] = f
            cs = 2 * (d + 1) - 1
            ws[WS_SLOT][d + 1] = cs
            FI[cs, FI_NODE] = ln; FF[cs, FF_BUDGET] = 1e300; FI[cs, FI_PHASE] = 0
            FI[ps, FI_PHASE] = 1
            d += 1
            continue
        if phase == 1:
            cs = 2 * (d + 1) - 1
            ws[WS_SLOT][d + 1] = cs
            FI[cs, FI_NODE] = FI[ps, FI_RN]; FF[cs, FF_BUDGET] = 1e300; FI[cs, FI_PHASE] = 0
            FI[ps, FI_PHASE] = 2
            d += 1
            continue
        ln = FI[ps, FI_LN]; rn = FI[ps, FI_RN]; f = FI[ps, FI_II]
        val = nub[ln] + nub[rn]
        if val < nub[node] - EPS:
            nub[node] = val
            if nsplit[node] != f:
...
@njit(cache=NUMBA_CACHE, nogil=True)
def _gain_split(ws, ps, mode):
    """Split of the frame's node with the largest impurity gain (-1 if none is positive)."""
    FI = ws[WS_FI]
    nv = FI[ps, FI_NV]
    K = ws[WS_DIST].shape[1] - 1
    dist = ws[WS_DIST][ps, :K]
    L = ws[WS_L][ps]
    feats = ws[WS_IO][ps, 0, :nv]
    base = _impurity(dist, K, mode)
    rc = np.empty(K)
    best = 1e-12
    bf = -1
    for i in range(nv):
        for k in range(K):
            rc[k] = dist[k] - L[i, k]
        gain = base - _impurity(L[i], K, mode) - _impurity(rc, K, mode)
        if gain > best:
            best = gain
            bf = feats[i]
    return bf
...
def _expand_frame(st, dat, pf, pi, ws, d):
    ...
    nv, n_cand, i0, min_rejected, ran, i_d2, best_d2, lb_ge4, M, Fc = expand_kernel(
        ...
        min(budget, nub[node]), pi[PI_EXCH] == 1 and pi[PI_GROUPS] == 1, pf, io, fo, bo, L, dist)
    leaf_risk = nleaf[node]
    FF[ps, FF_LEAF] = leaf_risk
    if pf[PF_DEPTH] > 0.0 and d >= pf[PF_DEPTH]:
        nv = 0                           # depth budget (approximate mode): no split below it
    if nv == 0:
        nlb[node] = leaf_risk
        nub[node] = leaf_risk
        nsplit[node] = -1
    ...
    if (not ran) and lb_ge4 < 0.0 and best_d2 < best - EPS:
        # budgeted pair stage (incumbent only): install its best two-level tree
        best = best_d2
    ...
    if ran:
        ...
        # budgeted pair stage: cache the frame too (the re-arm derives no bounds from it, and
        # a re-entry must not pay the budgeted stage again); no refilter: its two-level
        # values are not bounds
        for t in range(nv):
            f3[0, t] = l_ub2[t]
            f3[1, t] = r_ub2[t]
        FI[ps, FI_VALID] = 1
    if best < nub[node] - EPS:
        # the expansion's best tree (leaf-sum split or two-level tree, its children installed
        # above) becomes the node's incumbent now, not only when the candidate loop ends: a
        # frame left early (seed, interruption) or re-armed from the cache keeps it
        nub[node] = best
        if best_split != nsplit[node]:
            npend[node, 0] = 0
        nsplit[node] = best_split
        ...
        if phase == 2:
            ...
            if look_ahead and nsolved[first] == 0:
                ...
                if nlb[first] > bf + EPS:
                    ...
                    continue
                FI[ps, FI_PHASE] = 3
                if 2 * (d + 1) >= ws[WS_FI].shape[0]:
                    meta[2] = 3 if 2 * (d + 1) >= NSLOT else 4
                    _flush_frames(st, ws, d)
                    return
                cs = 2 * (d + 1) - 1
                ws[WS_SLOT][d + 1] = cs
...
class CompiledOptimizer:
    ...
    def __init__(self, data: BitDataset, regularization: float, *, groups=None, time_limit=0.0,
                 ...
                 greedy_init=True, upperbound=0.0, engine="numba", memory_limit=0, verbose=False, ref_wrong=None, depth_budget=0,
        ...
        self.dat = (data.F_words, group_of, np.ascontiguousarray(np.vstack(masks)), np.array(weights, dtype=np.float64),
                    ...
                    ref_wrong if ref_wrong is not None else np.zeros(data.W, dtype=np.uint64))
        pair_cap = 0.0
        partial = 0.0
        if time_limit > 0.0:
            pair_cap = PAIR_TIME_FRAC * float(time_limit) / PAIR_OP_SECONDS
            partial = PARTIAL_TIME_FRAC * float(time_limit) / PAIR_OP_SECONDS   # global budget, depleted by the budgeted stages
        self.pf = np.array([self.lam, uniform_w, float(data.n), pair_cap, partial, float(depth_budget)])
        ...
        self.stores = None
        self._chunk_target = 0.05
        self.greedy_init = bool(greedy_init)
        self.seed_nodes = 0
        self.seed_time = 0.0
        self.n_tasks = 0
        self.lns_solved = 0
    ...
    def run(self):
        ...
        st = self.st
        meta = st[ST_META]
        self._chunk_target = 0.05 if self.time_limit <= 0.0 else min(0.05, max(0.0005, self.time_limit / 20.0))
        if self.greedy_init and GREEDY_SEED_NODES > 0:
            # greedy seeds in time-bounded rounds (two expansions first to measure the cost of
            # one on this data): the lookahead greedy, then the gain greedies of SEED_MODES;
            # each installs its tree only where it beats what the store holds
            seed_budget = SEED_TIME_FRAC * (self.time_limit if self.time_limit > 0.0 else 30.0)
            seed_budget = min(seed_budget, SEED_TIME_MAX)
            # the extra seeds and the repair matter under a short cap, where the incumbent is
            # the answer; under a long cap the search decides and they are only overhead
            short_cap = 0.0 < self.time_limit <= EXTRAS_MAX_CAP
            for mode in (SEED_MODES if short_cap else SEED_MODES[:1]):
                if time.perf_counter() - self.start_time >= seed_budget:
                    break
                self.ws[WS_SLOT][MAXD] = -1
                todo = 2
                nodes0 = self.seed_nodes
                while todo > 0 and self.seed_nodes - nodes0 < GREEDY_SEED_NODES:
                    meta[2] = 0
                    self.seed_nodes += int(greedy_seed(st, self.dat, self.pf, self.pi, self.ws, np.int64(root), np.int64(min(24, self.ws[WS_FI].shape[0] // 2 - 1)),
                                                       np.int64(todo), np.int64(mode)))
                    if meta[2] == 2:            # store full during the seed: grow and resume
                        self._alloc(st[ST_KEYS].shape[0] * 2, st)
                        st = self.st; meta = st[ST_META]
                        continue
                    if self.ws[WS_SLOT][MAXD] == -2:
                        break
                    elapsed = time.perf_counter() - self.start_time
                    per = elapsed / max(self.seed_nodes, 1)
                    todo = min(int((seed_budget - elapsed) / max(per, 1e-6)), GREEDY_SEED_NODES - (self.seed_nodes - nodes0))
                if time.perf_counter() - self.start_time >= seed_budget:
                    break
            self.seed_time = time.perf_counter() - self.start_time
            if LNS_TIME_FRAC > 0.0 and short_cap:
                self._lns_repair(root, min(LNS_TIME_FRAC * self.time_limit, LNS_TIME_MAX))
        chunk = 4                       # the first chunk is tiny: wide data costs milliseconds per expansion
        # chunk length target: 50 ms, or a twentieth of the time limit when that is shorter,
        # so a short cap is honoured to within a few per cent (anytime use)
        self._chunk_target = 0.05 if self.time_limit <= 0.0 else min(0.05, max(0.0005, self.time_limit / 20.0))
        budget = None
        try:
            per_max = 1e-9
            while True:
                ...
                if meta[2] == 4:
                    self.ws = grow_workspace(self.ws, self.data.m, self.data.K, self.data.W)
                    continue
                ...
                if dt > 0.0:
                    ...
                    # shorter under a short time limit.  The next chunk is sized by the
                    # slowest expansion seen so far, not the last chunk's average: on wide
                    # data an expansion costs anywhere from microseconds to milliseconds
                    target = self._chunk_target
                    if self.n_jobs > 1:
                        target = min(target, max(0.002, self.parallel_after - (now - self.start_time)))
                    done = max(1, int(meta[1]) - iters_before)
                    per_max = max(per_max, dt / done)
                    chunk = int(min(max(target / per_max, 4), 200000))
    ...
    # ------------------------------------------------------------ LNS repair
    def _incumbent_nodes(self, root):
        """Internal nodes of the incumbent tree in the store (pending nodes are units)."""
        st = self.st; W = self.data.W
        out = []; leaves = []; stack = [(root, -1)]
        a = np.empty(W, dtype=np.uint64); b = np.empty(W, dtype=np.uint64)
        while stack:
            nid, parent = stack.pop()
            f = int(st[ST_SPLIT][nid])
            if f < 0:
                if parent >= 0 and st[ST_SOLVED][nid] != 1:
                    leaves.append((nid, parent))
                continue
            out.append((nid, parent))
            if st[ST_PEND][nid, 0] != 0:
                continue
            child_key(st, self.dat, nid, f, True, a); ln = store_find(st, a)
            child_key(st, self.dat, nid, f, False, b); rn = store_find(st, b)
            if ln >= 0 and rn >= 0:
                stack.append((ln, nid)); stack.append((rn, nid))
        return out, leaves

    def _lns_repair(self, root, budget_s):
        """Bottom-up exact repair of the incumbent: each internal node of the incumbent tree,
        smallest first, is solved as a subproblem with its current value as the budget (so
        the result is its exact optimum or a proof that the current subtree is optimal), and
        every improvement is propagated to the ancestors' values.  Time-bounded; the search
        then starts from the repaired incumbent."""
        st = self.st; meta = st[ST_META]
        deadline = self.start_time + budget_s
        nodes, leaves = self._incumbent_nodes(root)
        self.lns_lists = (len(nodes), len(leaves))
        if not nodes and not leaves:
            return
        parent_of = {nid: p for nid, p in nodes}
        parent_of.update({nid: p for nid, p in leaves})
        # internal nodes smallest first (cheap exact repairs), then the incumbent's leaves
        # smallest first: a leaf solved with its own risk as the budget is replaced by any
        # better subtree, which is how chain-shaped trees below a stump are found
        order = sorted(nodes, key=lambda t: int(st[ST_COUNT][t[0]])) + sorted(leaves, key=lambda t: int(st[ST_COUNT][t[0]]))
        W = self.data.W; a = np.empty(W, dtype=np.uint64); b = np.empty(W, dtype=np.uint64)
        self.lns_solved = 0
        self.lns_visited = 0
        self.lns_time = 0.0
        t_lns = time.perf_counter()
        for nid, _ in order:
            if time.perf_counter() >= deadline:
                break
            self.lns_visited += 1
            if st[ST_SOLVED][nid] == 1 or nid == root:
                continue
            chunk = 4
            per_max = 1e-9
            while True:
                meta[2] = 0; iters_before = int(meta[1]); meta[3] = meta[1] + chunk
                t0 = time.perf_counter()
                solve_iter(st, self.dat, self.pf, self.pi, self.ws, np.int64(nid), float(st[ST_UB][nid]), np.int64(-1),
                           self._no_shared, self._no_table, np.int64(0))
                dt = time.perf_counter() - t0
                if meta[2] == 0:
                    break
                if meta[2] == 2:
                    self._alloc(st[ST_KEYS].shape[0] * 2, st); st = self.st; meta = st[ST_META]
                    continue
                if meta[2] == 4:
                    self.ws = grow_workspace(self.ws, self.data.m, self.data.K, self.data.W)
                    continue
                if meta[2] == 3 or time.perf_counter() >= deadline:
                    break
                if dt > 0.0:
                    per_max = max(per_max, dt / max(1, int(meta[1]) - iters_before))
                    chunk = int(min(max(self._chunk_target / per_max, 4), 200000))
            if st[ST_SOLVED][nid] == 1:
                self.lns_solved += 1
            # propagate the (possibly improved) value to the ancestors along the incumbent
            cur = nid
            while cur in parent_of and parent_of[cur] >= 0:
                p = parent_of[cur]
                f = int(st[ST_SPLIT][p])
                child_key(st, self.dat, p, f, True, a); ln = store_find(st, a)
                child_key(st, self.dat, p, f, False, b); rn = store_find(st, b)
                if ln < 0 or rn < 0:
                    break
                v = float(st[ST_UB][ln] + st[ST_UB][rn])
                if v < st[ST_UB][p] - EPS:
                    st[ST_UB][p] = v
                cur = p
        self.lns_time = time.perf_counter() - t_lns
        self.root_dist_sum = float(self.ws[WS_DIST][0][:self.data.K].sum())
        self.ws[WS_FI][0, FI_VALID] = 0      # the root frame slot was reused by the subproblems
    ...
    def _worth_parallel(self):
        ...
        if remaining < 2:
            return False
        elapsed = time.perf_counter() - self.start_time
        if self.time_limit > 0.0 and self.time_limit - elapsed < 4.0 * max(self.parallel_after, 0.002):
            return False                    # too little time left to pay for the threads
    ...
    def _run_parallel(self, root, budget):
        ...
        self.n_tasks = int(n_keep)
        self.t_handoff = time.perf_counter() - self.start_time
            ...
            try:
                while True:
                    try:
                        ...
                        break
                    if time.perf_counter() > deadline or state["failure"]:
                        raise TimeLimitReached("time")
                    t_task = time.perf_counter()
                    chunk = 2000 if self._chunk_target >= 0.05 else 4   # short caps: tiny first chunk, then by time
                    per_max = 1e-9
                    while True:
                        ws_k[WS_FI][0] = frame0_fi; ws_k[WS_FF][0] = frame0_ff
                        meta_k[2] = 0; iters_before = int(meta_k[1]); meta_k[3] = meta_k[1] + chunk
                        t0 = time.perf_counter()
                        ...
                        if meta_k[2] == 3:
                            raise TimeLimitReached("depth")
                        if meta_k[2] == 4:
                            ws_k = grow_workspace(ws_k, m, K, W)
                            continue
                        now = time.perf_counter()
                        ...
                        if self.memory_limit > 0 and now - last_mem > 0.5:
                            ...
                            if self._mem_bytes() > self.memory_limit:
                                raise TimeLimitReached("memory")
                        if dt > 0.0:
                            per_max = max(per_max, dt / max(1, int(meta_k[1]) - iters_before))
                            chunk = int(min(max(self._chunk_target / per_max, 4), 200000))
                    with lock:
                        state.setdefault("trace", []).append((tid, pos, t_task - self.start_time, time.perf_counter() - self.start_time))
...
def _compile_search():
    ...
    nv, n_cand, i0, mr, ran, i_d2, bd2, lbg, M, Fc = expand_kernel(dat[DT_F], feats, dat[DT_GROUP], st[ST_KEYS][root],
                                                                 ...
                                                                 0.1, 1.0, False, np.zeros(6), io, fo, bo, L, dist)
    ...
    ws[WS_SLOT][MAXD] = -1
    greedy_seed(opt.st, dat, pf, pi, ws, np.int64(opt.root), np.int64(2), np.int64(1), np.int64(0))
    ws[WS_SLOT][MAXD] = -1
    greedy_seed(opt.st, dat, pf, pi, ws, np.int64(opt.root), np.int64(2), np.int64(1), np.int64(1))
...
class GOSDTClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, regularization: float = 0.05, time_limit: float = 0.0,
                 ...
                 verbose: bool = False, n_jobs: int = 1, parallel_after: float = 0.01,
                 force_parallel: bool = False, guess_thresholds: int = 0, reference_lb: bool = False,
                 depth_budget: int = 0):
        self.n_jobs = n_jobs
        self.parallel_after = parallel_after
        self.force_parallel = force_parallel
        self.guess_thresholds = int(guess_thresholds)
        self.reference_lb = bool(reference_lb)
    ...
    def fit(self, X, y, target_name: str = "class"):
        ...
        t0 = time.perf_counter()
        self.encoder_ = BinaryEncoder()
        Xb = self.encoder_.fit_transform(X)
        ...
        # Row clustering: the objective does not depend on the row order, but the kernels
        # skip the empty words of a node's capture mask, so rows are sorted lexicographically
        # by the source columns (most informative at the root first) to make deep capture
        # sets contiguous-ish.
        self.guessed_ = False
        ref_pred = None
        if self.guess_thresholds > 0 or self.reference_lb:
            # gosdt-guesses style: a boosted-stump reference model chooses the thresholds
            # (its used features) and its predictions give the lower-bound guesses
            rounds = self.guess_thresholds if self.guess_thresholds > 0 else 40
            keep, ref_pred = boost_stumps(Xb, y_idx, len(self.classes_), rounds)
            if self.guess_thresholds > 0 and keep.shape[0] < Xb.shape[1]:
                enc = self.encoder_
                remap = {int(old): new for new, old in enumerate(keep.tolist())}
                enc.rules = [enc.rules[i] for i in keep.tolist()]
                enc.groups = [g2 for g2 in ([remap[i] for i in g if i in remap] for g in enc.groups) if len(g2) > 1]
                Xb = Xb[:, keep]
                self.guessed_ = True
            if self.reference_lb:
                self.guessed_ = True
        perm = cluster_rows(Xb, y_idx, len(self.classes_), self.encoder_.groups)
        Xb = Xb[perm]
        y_idx = y_idx[perm]
        if ref_pred is not None:
            ref_pred = ref_pred[perm]
        data = BitDataset(Xb, y_idx, len(self.classes_), costs=self.costs, balance=self.balance)
        ref_wrong = None
        if self.reference_lb and ref_pred is not None:
            wrong = np.asarray(ref_pred != y_idx, dtype=np.uint8)
            packed = np.packbits(wrong, bitorder="little")                 # row i -> bit i, as the capture keys
            buf = np.zeros(data.W * 8, dtype=np.uint8); buf[:packed.shape[0]] = packed
            ref_wrong = np.frombuffer(buf.tobytes(), dtype=np.uint64).copy()
        ...
        if engine == "auto":
            engine = "numba" if HAVE_NUMBA else "python"
        cls = CompiledOptimizer if engine == "numba" else Optimizer
        opt = cls(
            ...
            greedy_init=self.greedy_init, upperbound=self.upperbound, engine=engine,
            memory_limit=self.memory_limit, verbose=self.verbose,
            **({"n_jobs": self.n_jobs, "parallel_after": self.parallel_after, "force_parallel": self.force_parallel,
                "ref_wrong": ref_wrong, "depth_budget": self.depth_budget}
               if cls is CompiledOptimizer else {}),
        )
        root = opt.run()
        self.optimizer_ = opt
        self.optimal_ = opt.optimal and not self.guessed_
        self.stop_reason_ = opt.stop_reason if not self.guessed_ else ("guessed" if opt.optimal else opt.stop_reason)
        self.time_ = opt.elapsed
        self.iterations_ = opt.iterations
        self.size_ = len(opt.memo)
        if isinstance(opt, CompiledOptimizer):
        ...
        if not self.optimal_ and self.stop_reason_ != "guessed":
        ...
        raw = opt.parallel_tree if getattr(opt, "parallel_tree", None) is not None else opt.extract(root)
        self.tree_ = self._decode(raw, data, target_name)
        if hasattr(opt, "release"):
            opt.release()
...
TIME_CAP = float(os.environ.get("OPTTREE_TIME_CAP", "0"))   # anytime rows: cap the time limit (seconds, 0 = none)


GUESS = int(os.environ.get("OPTTREE_GUESS", "0"))            # threshold guessing rounds (0 = off)
REF_LB = os.environ.get("OPTTREE_REF_LB", "0") == "1"        # reference lower-bound guessing
DEPTH = int(os.environ.get("OPTTREE_DEPTH", "0"))            # depth budget (0 = none)


def make_model(regularization: float, time_limit: float):
    if TIME_CAP > 0.0:
        time_limit = min(time_limit, TIME_CAP)
    return OptimalTreeClassifier(regularization=regularization, time_limit=time_limit,
                                 memory_limit=6 * (1 << 30), n_jobs=N_JOBS, parallel_after=0.003,
                                 guess_thresholds=GUESS, reference_lb=REF_LB, depth_budget=DEPTH)
