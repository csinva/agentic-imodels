...
MODEL_NAME = "v16_leaf_count_caps"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v15 + leaf-count caps: an optimal subtree on X with a leaves needs potential >= a lambda "
               "(every leaf has potential >= lambda) and leaf loss - equivalent-points loss >= (a-1) lambda "
               "(replacing it by a leaf must not win), so a <= cap(X); children with cap <= 2 are solved "
               "exactly by the pairwise kernel, the shape relaxation only combines feasible (a, b) and "
               "memoised children also take the per-split bounds")
...
@njit(cache=True, nogil=True)
def node_stats(F, feats, group_of, masks, weights, n_min, costs, diff, lam, dist, total_pot,
               out_feats, out_L, out_l_leaf, out_l_lb, out_l_solved, out_r_leaf, out_r_lb,
               out_r_solved, out_l_pot, out_l_cap, out_r_cap):
    ...
    equivalent-points masks weighted by ``weights``; ``n_min`` is the node's total
    equivalent-points loss.  Returns the number of valid splits written.
    """
    W = F.shape[1]
    K = dist.shape[0]
    R = masks.shape[0]
    ...
    for t in range(feats.shape[0]):
        ...
        out_l_pot[nv] = lpot
        # Leaf-count caps.  In an optimal tree every leaf has potential >= lam (else the
        # leaf can be merged into its sibling for a strict gain) and a subtree with a
        # leaves on X has loss <= leaf loss(X) - (a - 1) lam (else the leaf at X is
        # strictly better) while its loss is >= the equivalent-points loss of X; so an
        # optimal subtree on X has at most cap(X) leaves.
        out_l_cap[nv] = min(np.floor((lpot + EPS) / (lam - EPS)), 1.0 + np.floor((lmax - lmin + EPS) / lam))
        out_r_cap[nv] = min(np.floor((rpot + EPS) / (lam - EPS)), 1.0 + np.floor((rmax - rmin + EPS) / lam))
        nv += 1
    return nv
...
@njit(cache=True, nogil=True)
def shape_bound(l_leaf, ml2_l, c3_l, r_leaf, ml2_r, c3_r, l_cap, r_cap, node_cap, lam,
                l_lb, r_lb, l_ub2, r_ub2, split_lb, split_ub2):
    ...
    for a >= 4; a > cap(X) is impossible for an optimal subtree (leaf-count caps).
    Writes l_ub2/r_ub2 = min(g_1, g_2) (achievable), raises l_lb/r_lb to
    min over feasible a of g_a, split_lb = l_lb + r_lb, split_ub2 = l_ub2 + r_ub2, and
    returns (argmin split_ub2, min split_ub2, lb_ge4) where lb_ge4 = min over splits
    and feasible (a, b) with a + b >= 4, a + b <= node_cap of g_a(left) + g_b(right)
    bounds the optimal tree whenever it has >= 4 leaves (its root split has a- and
    b-leaf optimal subtrees; a, b >= 4 only add lam).
    """
    mf = l_leaf.shape[0]
    four = 4.0 * lam
    best_i = 0
    ...
    for i in range(mf):
        ...
        cr = r_cap[i]
        gl[1] = l_leaf[i]
        gl[2] = 2.0 * lam + ml2_l[i] if cl >= 2.0 else 1e300
        gl[3] = 3.0 * lam + c3_l[i] if cl >= 3.0 else 1e300
        gl[4] = four if cl >= 4.0 else 1e300
        gr[1] = r_leaf[i]
        gr[2] = 2.0 * lam + ml2_r[i] if cr >= 2.0 else 1e300
        gr[3] = 3.0 * lam + c3_r[i] if cr >= 3.0 else 1e300
        gr[4] = four if cr >= 4.0 else 1e300
        ul = min(gl[1], gl[2])
        ur = min(gr[1], gr[2])
        l_ub2[i] = ul
        r_ub2[i] = ur
        ll = min(min(ul, gl[3]), gl[4])
        rr = min(min(ur, gr[3]), gr[4])
        if ll > l_lb[i]:
            l_lb[i] = ll
        if rr > r_lb[i]:
            r_lb[i] = rr
...
@njit(cache=True, nogil=True)
def depth2_pairs(F, feats, group_of, masks, costs, lam, dist, L, out_ml2_l, out_ml2_r, out_j_l, out_j_r,
                 out_c3_l, out_c3_r):
        ...
        for j in range(i + 1, mf):
            ...
            if gi >= 0 and group_of[fj] == gi:
                ...
                for k in range(K):
                    acc = np.uint64(0)
                    for w in range(W):
                        acc += _popcount64(F[fi, w] & F[fj, w] & masks[k, w])
                    ij[k] = acc
            sij = 0.0
            lj = 0.0
            for k in range(K):
...
def warm_up():
    ...
    node_stats(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), 0.0, np.zeros((1, 1)), np.zeros(1), 0.1,
               np.zeros(1), 0.0, np.empty(1, dtype=np.int64), np.empty((1, 1)), np.empty(1), np.empty(1),
               np.empty(1, dtype=np.bool_), np.empty(1), np.empty(1), np.empty(1, dtype=np.bool_), np.empty(1),
               np.empty(1), np.empty(1))
    depth2_pairs(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros((1, 1)), 0.1,
                 np.zeros(1), np.zeros((1, 1)), np.empty(1), np.empty(1), np.empty(1, dtype=np.int64),
                 np.empty(1, dtype=np.int64), np.empty(1), np.empty(1))
    e = np.zeros(1)
    shape_bound(e, e, e, e, e, e, e, e, 4, 0.1, np.zeros(1), np.zeros(1), np.empty(1), np.empty(1), np.empty(1),
                np.empty(1))
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        if self.engine == "numba":
            ...
            if data.zero_diagonal and data.equal_mismatch:
                ...
                n_min = w * (key & data.minority).bit_count()
                self._last_class_masks = M[:K]
                dist = np.array([float((key & t).bit_count()) for t in data.targets])
                mf = features.shape[0]
                buf = self._buffers(mf)
                nv = node_stats(data.F_words, features, self.group_of, M, data.weights1, n_min, data.costs,
                                self._diff, lam, dist, float(dist @ self._diff), *buf)
                if nv == 0:
                    return None
                feats, L, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, l_cap, r_cap = (x[:nv] for x in buf)
                self._last_node_cap = self._node_cap(node, float(dist @ self._diff), n_min)
                return feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, l_cap, r_cap, L, dist
            ...
            if False:
                ...
                for mask_int, mask_words, w in pairs:
                    ...
                    weights.append(w)
                    n_min += w * (key & mask_int).bit_count()
            M = np.stack(masks)
            self._last_class_masks = M[:K]
            dist = np.array([float((key & t).bit_count()) for t in data.targets])
            mf = features.shape[0]
            buf = self._buffers(mf)
            nv = node_stats(data.F_words, features, self.group_of, M, np.array(weights), n_min, data.costs,
                            self._diff, lam, dist, float(dist @ self._diff), *buf)
            if nv == 0:
                return None
            feats, L, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, l_cap, r_cap = (x[:nv] for x in buf)
        ...
        l_cap = np.minimum(np.floor((l_pot + EPS) / (lam - EPS)), 1.0 + np.floor((l_max - lmin + EPS) / lam))
        r_cap = np.minimum(np.floor((r_pot + EPS) / (lam - EPS)), 1.0 + np.floor((r_max - rmin + EPS) / lam))
        self._last_node_cap = self._node_cap(node, total_pot, float(min_total))
        return feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, l_cap, r_cap, L, dist

    def _node_cap(self, node: Node, total_pot: float, n_min: float) -> int:
        """Maximum number of leaves of an optimal tree on this node (see node_stats)."""
        lam = self.lam
        leaf_loss = node.leaf_risk - lam
        return int(min(math.floor((total_pot + EPS) / (lam - EPS)), 1 + math.floor((leaf_loss - n_min + EPS) / lam)))

    def _buffers(self, mf: int):
        """Fresh output arrays for node_stats (the returned views must survive recursion)."""
        K = self.data.K
        return (np.empty(mf, dtype=np.int64), np.empty((mf, K)), np.empty(mf), np.empty(mf),
                np.empty(mf, dtype=np.bool_), np.empty(mf), np.empty(mf), np.empty(mf, dtype=np.bool_),
                np.empty(mf), np.empty(mf), np.empty(mf))
    ...
    def _greedy(self, node: Node, features: np.ndarray, depth: int = 0) -> float:
        ...
        if stats is None:
            ...
            return node.ub
        feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, _, _, _, _, _ = stats
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if stats is None:
            ...
            node.solved = True
            return
        feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, l_cap, r_cap, L, dist = stats
        node_cap = self._last_node_cap
        data = self.data
        F = data.features
        key = node.key
        memo = self.memo
        ...
        split_lb = np.empty(mf_)
        split_ub = np.empty(mf_)
        ...
        lam = self.lam
        l_ub2 = r_ub2 = None
        cheap = mf_ * mf_ * data.W <= 32768
        if (self.engine == "numba" and cand.shape[0] >= 2 and mf_ >= 2
                and (cheap or (cand.shape[0] * 2 >= mf_ and cand.shape[0] >= 8 and mf_ >= 8))):
            mf = feats.shape[0]
            ml2_l = np.empty(mf); ml2_r = np.empty(mf)
            c3_l = np.empty(mf); c3_r = np.empty(mf)
            j_l = np.empty(mf, dtype=np.int64); j_r = np.empty(mf, dtype=np.int64)
            depth2_pairs(data.F_words, feats, self.group_of, self._last_class_masks, data.costs, lam, dist, L,
                         ml2_l, ml2_r, j_l, j_r, c3_l, c3_r)
            l_ub2 = np.empty(mf); r_ub2 = np.empty(mf); split_ub2 = np.empty(mf)
            i, best_d2, lb_ge4 = shape_bound(l_leaf, ml2_l, c3_l, r_leaf, ml2_r, c3_r, l_cap, r_cap, node_cap,
                                             lam, l_lb, r_lb, l_ub2, r_ub2, split_lb, split_ub2)
            best_d2 = float(best_d2); lb_ge4 = float(lb_ge4)
            if best_d2 < best - EPS:
                best = best_d2
                best_split = int(feats[i])
            ...
            if budget < 4 * lam - EPS:
                ...
                return
            keep = np.zeros(mf_, dtype=bool)
            keep[order_buf[:n_cand]] = True
            within = split_lb <= bound + EPS
            cand = np.flatnonzero(keep & within)
            # candidates dropped here by their tightened bound still bound the node from below
            dropped = split_lb[keep & ~within]
            if dropped.shape[0]:
                min_pruned = min(min_pruned, float(dropped.min()))
        ...
        child_lb_max = float(max(l_lb.max(), r_lb.max())) if l_lb.shape[0] else 0.0
        for i in order:
            ...
            if l_ub2 is not None:
                # depth-2 knowledge about these children (exact best 2-leaf tree)
                for cn, ub2, jj, side in ((ln, float(l_ub2[i]), int(j_l[i]), True), (rn, float(r_ub2[i]), int(j_r[i]), False)):
                    ...
                    lbi = float(l_lb[i] if side else r_lb[i])
                    if cn.lb < lbi:
                        cn.lb = lbi
            ...
            if l_ub2 is not None:
                ...
                for cn, other in ((first, second), (second, first)):
                    if not cn.solved and bound - other.lb < 3 * lam - EPS and cn.ub <= bound - other.lb + EPS:
                        cn.lb = cn.ub
                        cn.solved = True
    ...
    def _materialize_depth2(self, node, key, feats, i, j_l, j_r, l_ub2, r_ub2, l_leaf, r_leaf):
        ...
        lkey = key & F[f]
        rkey = key ^ lkey
        for ckey, ub2, jj, leaf in ((lkey, float(l_ub2[i]), int(j_l[i]), float(l_leaf[i])),
                                    (rkey, float(r_ub2[i]), int(j_r[i]), float(r_leaf[i]))):
