...
MULTICORE = True
DESCRIPTION = ("v43c anytime, time limit capped at 100 ms per pair, with two heuristics that decide the incumbent under short caps: (1) a family of greedy seeds - the lookahead greedy (expansion's best tree) and a gini-gain greedy grown deep and pruned bottom-up under lambda (each installs its tree where it beats the store); (2) a bottom-up exact repair of the incumbent (large neighbourhood search with the solver itself): every internal node of the incumbent, smallest first, is solved as a subproblem with its own value as budget, so it ends exactly optimal or proven so, and improvements propagate to the ancestors.  Both run only under caps up to 1 s (under long caps the search decides and they are overhead); at the 30 s cap the file is v43c.  Approximate by the cap only")
...
@njit(cache=NUMBA_CACHE, nogil=True)
def expand_kernel(F, features, group_of, kw, mask_matrix, weights, costs, diff, lam, bound, do_exchange, pair_cap,
                  io, fo, bo, L, dist):
    ...
    if pair_cap > 0.0 and float(nv) * float(nv) * float(K) * float(W) > pair_cap:
        return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0, M, Fc    # over the time budget's share
...
def warm_up():
    ...
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), int_to_words(1, 1), masks, np.zeros(0),
                  np.zeros((1, 1)), np.zeros(1), 0.1, 1.0, False, 0.0, np.empty((7, 1), dtype=np.int64), np.empty((14, 1)),
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, M, Fc = expand_kernel(
            ...
            self.continuous_feature_exchange and self._has_groups, 0.0, io, fo, bo, L, dist)
        if nv == 0:
            return None
        nv = int(nv)
        if nv < mf:
...
PF_LAM, PF_UW, PF_N, PF_PAIRCAP = range(4)
...
def _expand_frame(st, dat, pf, pi, ws, d):
    ...
    nv, n_cand, i0, min_rejected, ran, i_d2, best_d2, lb_ge4, M, Fc = expand_kernel(
        ...
        min(budget, nub[node]), pi[PI_EXCH] == 1 and pi[PI_GROUPS] == 1, pf[PF_PAIRCAP], io, fo, bo, L, dist)
...
class CompiledOptimizer:
    ...
    def __init__(self, data: BitDataset, regularization: float, *, groups=None, time_limit=0.0,
        ...
        self.pf = np.array([self.lam, uniform_w, float(data.n), pair_cap])
    ...
    def _lns_repair(self, root, budget_s):
        ...
        nodes = self._incumbent_nodes(root)
        if not nodes:
            return
        parent_of = {nid: p for nid, p in nodes}
        order = sorted(nodes, key=lambda t: int(st[ST_COUNT][t[0]]))
        ...
        for nid, _ in order:
            ...
            while True:
                ...
                if meta[2] == 3 or time.perf_counter() >= deadline:
                    break
                chunk = int(min(max(chunk * self._chunk_target / dt, 8), 200000)) if dt > 0 else chunk
                    ...
                    while True:
                        ...
                        if self.memory_limit > 0 and now - last_mem > 0.5:
                            ...
                            if self._mem_bytes() > self.memory_limit:
                                raise TimeLimitReached("memory")
                        if dt > 0.0:
                            chunk = int(min(max(chunk * self._chunk_target / dt, 50), 200000))
...
def _compile_search():
    ...
    nv, n_cand, i0, mr, ran, i_d2, bd2, lbg, M, Fc = expand_kernel(dat[DT_F], feats, dat[DT_GROUP], st[ST_KEYS][root],
                                                                 ...
                                                                 0.1, 1.0, False, 0.0, io, fo, bo, L, dist)
