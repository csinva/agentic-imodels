...
MODEL_NAME = "v9_budget_deepening"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v8 + alternating budget deepening: the two children of a split are searched with "
               "budgets that grow geometrically from their lower bounds (2 lambda, 4 lambda, ...) and "
               "alternate, so a losing branch fails at a small budget and each child's rising bound "
               "tightens the other's budget; the solve contract is unchanged, so the search stays exact")
