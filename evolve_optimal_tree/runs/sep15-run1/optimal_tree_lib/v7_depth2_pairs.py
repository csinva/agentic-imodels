...
MODEL_NAME = "v7_depth2_pairs"
DESCRIPTION = ("v5 + duplicate-threshold removal (v6, no measurable effect alone) + MurTree-style pairwise counts: at a node where many splits survive the cheap "
               "filter, one O(m^2) kernel gives the exact best 2-leaf loss of every child, so nodes "
               "with budget < 4 lambda are solved exactly (at most 3 leaves fit; 4-leaf chains exceed "
               "the budget), children get lb = min(leaf, 2 lambda + best2, 3 lambda), and children "
               "with budget < 3 lambda are resolved without expansion")
...
class Optimizer:
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if self._has_groups:
            ...
            # the incumbent DP on multi-column nodes pays off only where the search below
            # is large; small nodes are cheaper to search than to run 23 DPs on
            large = node.count * 4 >= data.n
        ...
        if (self.engine == "numba" and cand.shape[0] * 2 >= feats.shape[0] and cand.shape[0] >= 8
            ...
            if split_ub2[i] < best - EPS:
                ...
                bound = min(budget, best)
            if budget < 4 * lam - EPS:
                # exact: the optimum has at most 3 leaves or exceeds the budget
                node.ub = best
                ...
                if best <= budget + EPS:
                    ...
                    node.lb = max(node.lb, min(best, float(np.min(split_lb)) if split_lb.shape[0] else np.inf))
                return
            within = split_lb <= bound + EPS
            cand = np.flatnonzero(active & within)
