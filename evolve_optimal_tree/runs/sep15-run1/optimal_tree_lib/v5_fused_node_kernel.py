...
MODEL_NAME = "v5_fused_node_kernel"
DESCRIPTION = ("v4 + one fused numba kernel per node: popcounts, class counts, leaf risks, "
               "potentials, equivalent-points losses, leaf-only tests, child lower bounds and the "
               "valid-split compaction are computed in a single @njit call instead of ~15 numpy "
               "calls, cutting the per-expansion interpreter overhead on narrow binary datasets")
...
@njit(cache=True, nogil=True)
def node_stats(F, feats, masks, weights, n_min, costs, diff, lam, dist, total_pot,
...
def warm_up():
    ...
    node_stats(F, np.zeros(1, dtype=np.int64), masks, np.zeros(0), 0.0, np.zeros((1, 1)), np.zeros(1), 0.1,
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        if self.engine == "numba":
            ...
            nv = node_stats(data.F_words, features, M, np.array(weights), n_min, data.costs, self._diff, lam,
                            dist, float(dist @ self._diff), *buf)
            if nv == 0:
                return None
