...
MULTICORE = True
DESCRIPTION = ('v37 with the most promising root candidates split into two half-tasks, one per child: a half solves its child against the budget bound - lb(sibling) (the budget the final solve of a whole task uses), both halves solved give the exact value of the split, and a half proven above its budget proves the split above the incumbent, so the root certificate is unchanged; the longest root task no longer bounds the parallel phase (nor decides the wall time by the core it lands on).  The first PAR_SPLIT_FIRST (2) candidates are split.  Search and bounds unchanged')
...
PAR_SPLIT_FIRST = int(os.environ.get("PAR_SPLIT_FIRST", "2"))   # root candidates solved as two half-tasks
...
@njit(cache=NUMBA_CACHE, nogil=True)
def _tighten_stack(st, ws, d, look_ahead, half):
    ...
    for k in range(1, d + 1):
        ...
        if not look_ahead:
            ...
            ln = FI[p, FI_LN]; rn = FI[p, FI_RN]
            nb = pb - nlb[rn if half == 0 else ln]
        else:
            return
...
@njit(cache=NUMBA_CACHE, nogil=True)
def solve_iter(st, dat, pf, pi, ws, root, budget, only, shared, sh, tid, half):
    ...
    ``FF[0, FF_OUT_KIND]`` (1 solved / 0 pruned) and ``FF[0, FF_OUT_VALUE]``.  With
    ``half`` 0 or 1 only that child of the candidate (true / false side) is solved, against
    the budget bound - lb(other child): solved -> kind 1 with the child's value, else
    kind 0 with lb(child) + lb(other) (> the bound, so the split is pruned)."""
    FI = ws[WS_FI]; FF = ws[WS_FF]
    nlb = st[ST_LB]; nub = st[ST_UB]; nsplit = st[ST_SPLIT]; nsolved = st[ST_SOLVED]; npend = st[ST_PEND]
    meta = st[ST_META]
    lam = pf[PF_LAM]
    ...
    while d >= 0:
        ...
        if only >= 0 and shared[0] < FF[0, FF_BOUND]:
            ...
            _tighten_stack(st, ws, d, look_ahead, half)
        if phase == 0:
            # entry of _solve
        ...
        if phase == 1:
            ...
            if d == 0 and only >= 0 and half >= 0:
                # half-task: solve one child against the other's bound
                cn = ln if half == 0 else rn
                other = rn if half == 0 else ln
                if nsolved[cn] == 1:
                    FF[0, FF_OUT_KIND] = 1.0
                    FF[0, FF_OUT_VALUE] = nub[cn]
                    return
                hb = bound - nlb[other] if look_ahead else bound
                if nlb[cn] > hb + EPS:
                    FF[0, FF_OUT_KIND] = 0.0
                    FF[0, FF_OUT_VALUE] = nlb[cn] + nlb[other]
                    return
                cs = 1
                ws[WS_SLOT][1] = cs
                FI[cs, FI_NODE] = cn; FF[cs, FF_BUDGET] = hb; FI[cs, FI_PHASE] = 0
                FI[ps, FI_PHASE] = 9
                d = 1
                continue
            FI[ps, FI_PHASE] = 2
        ...
        ii = FI[ps, FI_II]
        if phase == 9:
            # the half's child returned
            ln = FI[ps, FI_LN]; rn = FI[ps, FI_RN]
            cn = ln if half == 0 else rn
            other = rn if half == 0 else ln
            if nsolved[cn] == 1:
                FF[0, FF_OUT_KIND] = 1.0
...
class CompiledOptimizer:
            ...
            while True:
                ...
                if self.force_parallel and self.n_jobs > 1:
                    ...
                    solve_iter(st, self.dat, self.pf, self.pi, self.ws, np.int64(root), float(budget), np.int64(-1), self._no_shared,
                               self._no_table, np.int64(0), np.int64(-1))
    ...
    def _run_parallel(self, root, budget):
        ...
        stores = {}
        n_split = min(PAR_SPLIT_FIRST, n_keep) if n_threads > 1 else 0
        for pos in range(n_keep):
            if pos < n_split:
                tasks.put((pos, 0)); tasks.put((pos, 1))
            else:
                tasks.put((pos, -1))
        halves = {}
                        ...
                        if kind == 1:
                            with lock:
                                if value < shared[0]:
                                    shared[0] = value
                        with lock:
                            results.append((pos, kind, value, lbmax, (tid, ln, tid, rn, f)))
                    else:
                        cn = ln if half == 0 else rn
                        with lock:
                            mine = (kind, value, tid, cn, lbmax)
                            other = halves.get((pos, 1 - half))
                            if other is None:
                                halves[(pos, half)] = mine
                            else:
                                # both halves done: exact value if both solved, else pruned
                                lm = max(lbmax, other[4])
                                if kind == 1 and other[0] == 1:
                                    val = value + other[1]
                                    if val < shared[0]:
                                        shared[0] = val
                                    l = mine if half == 0 else other; r = other if half == 0 else mine
                                    results.append((pos, 1, val, lm, (l[2], l[3], r[2], r[3], f)))
                                else:
                                    pv = min(v[1] for v in (mine, other) if v[0] == 0)
                                    results.append((pos, 0, pv, lm, (tid, -1, tid, -1, f)))
            except TimeLimitReached as exc:
                with lock:
        ...
        for pos, kind, value, lbmax, ref in results:
            if kind == 1:
                if ref[4] >= 0 and value < best - EPS:
                    best = value; best_split = ref[4]; best_ref = ref
            else:
                if value < min_pruned:
                    min_pruned = value
        ...
        if best_ref is not None:
            ...
            tid_l, ln, tid_r, rn, f = best_ref
            self.stores = stores
            best_tree = {"feature": f, "true": _StoreView(self, stores[tid_l], stores).extract(ln),
                         "false": _StoreView(self, stores[tid_r], stores).extract(rn)}
    ...
    try:
        while True:
            ...
            while True:
                ...
                t0 = time.perf_counter()
                solve_iter(st, opt.dat, opt.pf, opt.pi, ws, np.int64(root), budget, np.int64(pos), shared, opt._no_table, np.int64(0), np.int64(-1))
                dt = time.perf_counter() - t0
                if meta[2] == 0:
                    break
                if meta[2] == 2:
...
def _compile_search():
    ...
    if not _expand_frame(st, dat, pf, pi, ws, 0):
        solve_iter(st, dat, pf, pi, ws, np.int64(root), float(st[ST_UB][root]), np.int64(0), np.array([1e300]),
                   make_shared_table(1, 8, data.W, 1), np.int64(0), np.int64(-1))
