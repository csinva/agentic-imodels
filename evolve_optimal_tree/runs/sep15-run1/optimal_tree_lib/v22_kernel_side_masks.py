...
MODEL_NAME = "v22_kernel_side_masks"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v21 + the node's class masks are built inside the expansion kernel from the capture "
               "words and the dataset's mask matrix (one numpy op and one allocation less per "
               "expansion), and the children's maximal lower bound (the superset bound) comes from a "
               "numba helper instead of two numpy reductions; same search and bounds")
...
def expand_kernel(F, features, group_of, kw, mask_matrix, weights, costs, diff, lam, bound, do_exchange,
    ...
    if nv == 0:
        return 0, 0, 0, 0.0, False, 0, 0.0, 0.0, M
    feats = io[0, :nv]
    ...
    if single or not (n_cand >= 2 and nv >= 2 and (cheap or (n_cand * 2 >= nv and n_cand >= 8 and nv >= 8))):
        return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0, M
    ...
    return nv, n_cand, best_i, min_rejected, True, i_d2, best_d2, lb_ge4, M
...
def warm_up():
    ...
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), int_to_words(1, 1), masks, np.zeros(0),
                  np.zeros((1, 1)), np.zeros(1), 0.1, 1.0, False, np.empty((6, 1), dtype=np.int64), np.empty((14, 1)),
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        L = np.empty((mf, K))
        dist = np.empty(K + 1)
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, M = expand_kernel(
