...
MULTICORE = True
DESCRIPTION = ('v46 run with the gosdt-guesses recipe (McTavish et al. 2022) as options: a boosted-stump reference model (40 rounds, numpy SAMME on the binarised data) whose used features are the thresholds kept (threshold guessing), whose per-row errors give a lower-bound guess for every subproblem (its loss on the capture + lambda, applied at node creation), plus a depth budget of 5.  Not certified (the guessed bounds are not bounds).  A reference point: the same recipe inside our search against gosdt_guesses_guided, and against the anytime rows')
...
GUESS = int(os.environ.get("OPTTREE_GUESS", "40"))            # threshold guessing rounds (0 = off)
REF_LB = os.environ.get("OPTTREE_REF_LB", "1") == "1"        # reference lower-bound guessing
DEPTH = int(os.environ.get("OPTTREE_DEPTH", "5"))            # depth budget (0 = none)
