...
MODEL_NAME = "v36_cached_root_handoff"
...
MULTICORE = True
DESCRIPTION = ("v35 with the root frame re-armed from the frame cache at the hand-off instead of re-expanded: the parent used to run the root's kernel stages (depth-2 pairs over every feature pair, depth-3 triples) a second time before starting the threads, which on wide data costs more than the sequential phase itself; the cache of v31 still holds that expansion, so only the budget-dependent steps are redone.  Search and bounds unchanged")
