...
MULTICORE = True
DESCRIPTION = ('v43 run as an anytime method with the time limit capped at 1000 ms per pair: returns the incumbent at the cap (certified only when the search finishes earlier); see the shorter-cap rows for the chunking, seed and hand-off rules that make caps meaningful.  Approximate by the cap only, not by any change to the bounds')
