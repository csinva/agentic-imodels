...
MODEL_NAME = "v4_leaf_support_splits"
DESCRIPTION = ("v3 + leaf-support split exclusion: every leaf of an optimal tree has cost-weighted "
               "support >= lambda (removing a smaller leaf saves lambda and costs at most its support), "
               "so a split whose child has potential < lambda is discarded at enumeration time instead "
               "of being kept with the child as a leaf")
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        lsum = L.sum(axis=1)
        # Leaf-support exclusion.  In an optimal tree every leaf has cost-weighted
        # support (potential) >= lam: removing a leaf with smaller support and routing
        # its points into the sibling subtree loses at most that support and saves
        # lam.  A child with potential < lam would contain such a leaf, so no optimal
        # tree uses a split with such a child; the split is dropped here.
