...
MODEL_NAME = "v12_children_from_kernel"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v10 + child nodes are created from the kernel's per-side class counts and "
               "predictions instead of recomputing leaf statistics with popcounts for every new "
               "memo entry (leaf_stats was ~15% of the time on the binary datasets)")
...
def node_stats(F, feats, group_of, masks, weights, n_min, costs, diff, lam, dist, total_pot,
               ...
               out_r_solved, out_l_pot, out_l_pred, out_r_pred):
    ...
    for t in range(feats.shape[0]):
        ...
        l_leaf = lmax + lam
        r_leaf = rmax + lam
        l_solved = (lsum <= 1.0) or (1.0 - lmin < lam) or (lmax - lmin < lam) or (lpot < 2.0 * lam)
        r_solved = (rsum <= 1.0) or (1.0 - rmin < lam) or (rmax - rmin < lam) or (rpot < 2.0 * lam)
        out_feats[nv] = j
        for k in range(K):
            out_L[nv, k] = cnt[k]
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        if self.engine == "numba":
            ...
            if data.zero_diagonal and data.equal_mismatch:
                ...
                if nv == 0:
                    return None
                feats, L, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, l_pred, r_pred = (x[:nv] for x in buf)
                self._last_preds = (l_pred, r_pred)
    ...
    def _buffers(self, mf: int):
        ...
        return (np.empty(mf, dtype=np.int64), np.empty((mf, K)), np.empty(mf), np.empty(mf),
                ...
                np.empty(mf), np.empty(mf, dtype=np.int64), np.empty(mf, dtype=np.int64))
        ...
        for i in order:
            ...
            if ln is None:
                ln = self._child_node(lkey, lkey.bit_count(), lub, llb, bool(l_solved[i]),
                                      int(preds[0][i]) if preds is not None else -1)
            if rn is None:
                rn = self._child_node(rkey, rkey.bit_count(), rub, rlb, bool(r_solved[i]),
                                      int(preds[1][i]) if preds is not None else -1)
