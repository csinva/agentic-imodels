...
MODEL_NAME = "v25_mc8_root_parallel"
...
MULTICORE = True
DESCRIPTION = ("v23 with a parallel phase on 8 cores: the search runs sequentially for 50 ms; if the root "
               "is not solved by then, 8 worker processes are forked (inheriting the memo and the root's "
               "expansion), pull the root's surviving candidate splits from a queue and solve each split's "
               "children exactly as the sequential loop does, with the incumbent shared in memory and only "
               "ever decreasing; a split is either solved or proven above an incumbent that is >= the final "
               "one, so the root is certified exactly as in the sequential loop. Small problems never fork")
