...
MODEL_NAME = "v41_node_keyed_frame_cache"
...
MULTICORE = True
DESCRIPTION = ('v40 with the frame cache of v31 generalised from two slots per depth to a pool of cached frame slots keyed by node: a frame pushed for a node whose expansion is still in the pool re-arms it (budget-dependent steps only) instead of re-running its kernels; slots are handed out FIFO (never one on the current stack) and the node -> slot map is a small open-addressing table rebuilt when half full.  26-36% of the expansions on fico_binary, coupon, car and monk_2 (12-15% on tic-tac-toe and iris) were repeats of an already expanded node (budget deepening re-enters children, chunk re-entries restart from the root).  Pool size from a 16 MB per-workspace budget (64-2048 slots).  Search and bounds unchanged: a cached expansion is a pure function of the node')
...
FI_NODE, FI_PHASE, FI_NV, FI_NKEEP, FI_OI, FI_II, FI_LN, FI_RN, FI_FIRST, FI_SECOND, FI_PRUNED, FI_HAVE_D2, FI_HAVE_D3, FI_SIM, FI_BEST_SPLIT, FI_PARENT_NV, FI_VALID, FI_CACHED_NODE, FI_I_D2 = range(19)
# float frame fields
...
WS_FI, WS_FF, WS_IO, WS_FO, WS_BO, WS_L, WS_DIST, WS_GIDX, WS_LBARR, WS_ARG3, WS_F3, WS_K3, WS_KW, WS_KW2, WS_ROOTFEATS, WS_SLOT, WS_SNODE, WS_MAPK, WS_MAPV, WS_RING = range(20)
NSLOT = 2 * MAXD      # (legacy) frame slots; the workspace now holds a node-keyed pool, slot 0 is the root
WS_BUDGET_BYTES = float(os.environ.get("WS_BUDGET_MB", "16")) * 1e6


def frame_slots(m, K):
    """Cached frame slots for a workspace of this shape: 16 MB worth, 64-2048, plus the root."""
    per = m * (7 * 8 + 14 * 8 + 2 + 8 * K + 8 + 8 + 4 * 8 + 2 * 8) + (K + 1) * 8 + 18 * 8 + 16 * 8
    return 1 + int(min(2048, max(64, WS_BUDGET_BYTES // per)))
...
def make_workspace(m, K, W):
    ...
    S = frame_slots(m, K)
    mapn = 1 << int(2 * S - 1).bit_length()      # map with at least 2 entries per slot
    return (np.zeros((S, 19), dtype=np.int64), np.zeros((S, 16)), np.zeros((S, 7, m), dtype=np.int64),
            ...
            np.full(S, -1, dtype=np.int64), np.full(mapn, -1, dtype=np.int64), np.zeros(mapn, dtype=np.int64),
...
@njit(cache=NUMBA_CACHE, nogil=True)
def _map_slot(mk, mv, node, s):
    """Insert node -> s into the open-addressing map (the key may be present, stale)."""
    mask = mk.shape[0] - 1
    h = (node * np.int64(0x9E3779B1)) & mask
    while True:
        k = mk[h]
        if k == node or k == -1:
            fresh = k == -1
...
@njit(cache=NUMBA_CACHE, nogil=True)
def _acquire_slot(ws, node, d):
    """Slot for a frame of ``node`` pushed below depth ``d``: its cached slot if the pool
    still holds the node, else the next FIFO slot that is not on the stack (its cached
    expansion, if any, is invalidated)."""
    snode = ws[WS_SNODE]; mk = ws[WS_MAPK]; mv = ws[WS_MAPV]; ring = ws[WS_RING]
    S = snode.shape[0]
    mask = mk.shape[0] - 1
    h = (node * np.int64(0x9E3779B1)) & mask
    while True:
    ...
    ring[0] = r
    snode[s] = node
    ws[WS_FI][s, FI_VALID] = 0
    if _map_slot(mk, mv, node, s):
        ring[1] += 1
    if ring[1] * 2 > mk.shape[0]:
        # rebuild the map from the live slots (drops stale entries)
        mk[:] = -1
        n = 0
        for t in range(1, S):
            if snode[t] >= 0:
                _map_slot(mk, mv, snode[t], t)
                n += 1
        ring[1] = n
    return s
...
def _expand_frame(st, dat, pf, pi, ws, d):
    ...
    if nv == 0:
        ...
        nsolved[node] = 1
        return True
    # cache header: everything the re-arm path needs is known here (it does not depend on
    # the budget), so a frame that leaves through one of the early returns below can still
    # be re-armed when it is re-entered with a larger budget
    FI[ps, FI_NV] = nv
    ...
    FI[ps, FI_I_D2] = i_d2
    hdr_feats = io[0, :nv]
    hdr_gidx = ws[WS_GIDX][ps, :nv]
    for t in range(nv):
        hdr_gidx[t] = group_of[hdr_feats[t]]
    hdr_lb = ws[WS_LBARR][ps, :nv]
    hdr_split_lb = fo[5, :nv]
    for t in range(nv):
        hdr_lb[t] = hdr_split_lb[t]
    if ran:
        hdr_f3 = ws[WS_F3][ps, :, :nv]
        hdr_lub2 = fo[11, :nv]; hdr_rub2 = fo[12, :nv]
        for t in range(nv):
            hdr_f3[0, t] = hdr_lub2[t]
            hdr_f3[1, t] = hdr_rub2[t]
    # cached only with the pair stage done: whether it runs depends on the candidates
    # surviving at the current bound, and a re-entry with a larger budget deserves it
    FI[ps, FI_VALID] = 1 if ran else 0
...
def _rearm_frame(st, dat, pf, pi, ws, d):
    ...
    if ran:
        if best_d2 < best - EPS:
            # adopt the best 2-leaf-per-child tree: install its children's structures first
            # (a frame that left through an early return never did), as _expand_frame does
            i_d2 = FI[ps, FI_I_D2]
            best = best_d2
            best_split = feats[i_d2]
            j_l = io[2, :nv]; j_r = io[3, :nv]; l_pred = io[4, :nv]; r_pred = io[5, :nv]
            l_ub2 = fo[11, :nv]; r_ub2 = fo[12, :nv]
            l_solved = ws[WS_BO][ps, 0, :nv]; r_solved = ws[WS_BO][ps, 1, :nv]
            kw_buf = ws[WS_KW]
            for side in range(2):
                ii = i_d2
                if side == 0:
