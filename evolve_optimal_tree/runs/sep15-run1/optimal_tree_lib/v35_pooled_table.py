...
MULTICORE = True
DESCRIPTION = ("v34 with the threads' shared table pooled across fits (allocating it cost 1-5 ms per fit, a large share of a 10 ms pair): one table per (threads, key width) is kept and only its used flags and value column are reset before the threads start (the reset happens-before every thread start, so the lock-free reads keep their guarantees); capacity 16384 slots per thread within the memory budget.  Hand-off after 3 ms instead of 5 ms.  Search and bounds unchanged")
