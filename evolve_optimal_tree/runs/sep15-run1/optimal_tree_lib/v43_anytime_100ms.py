...
MULTICORE = True
DESCRIPTION = ('v43 run as an anytime method with the time limit capped at 100 ms per pair: returns the incumbent at the cap (certified only when the search finishes earlier).  Short caps are meaningful because search chunks scale with the cap (first chunk 4 expansions, then a twentieth of the cap), a time-bounded lookahead-greedy seed gives a full first tree, the hand-off is skipped when fewer than four hand-off costs remain, threads check the deadline before each task and start with a 4-expansion chunk; the cap is honoured to within a few expansions on wide data.  Approximate by the cap only, not by any change to the bounds')
