...
MODEL_NAME = "v30_compiled_mc8_threads"
...
MULTICORE = True
DESCRIPTION = ("v29 with threads instead of forked processes for the parallel phase: the compiled search "
               "releases the GIL, so 8 Python threads run it concurrently on private copies of the memo "
               "(taken after the root's expansion) with the incumbent in a shared array; no fork, no "
               "queues, so the hand-off happens after 10 ms of sequential search instead of 50 ms and "
               "small problems benefit too.  Certification exactly as in v25/v29")
...
class CompiledOptimizer:
    ...
    def _run_parallel(self, root, budget):
        ...
        def worker():
            # private copies of the store and of the root frame
            st_k = self._new_store(st[ST_KEYS].shape[0], st)
            ...
            try:
                while True:
                    ...
                    if kind == 1:
                        ...
                        ext = _StoreView(self, st_k)
                        tree = {"feature": f, "true": ext.extract(ln), "false": ext.extract(rn)}
                    with lock:
                        results.append((pos, kind, value, lbmax, tree))
            except TimeLimitReached as exc:
                with lock:
        ...
        meta[1] += state["iters"]
        best_tree = None
        for pos, kind, value, lbmax, tree in results:
            if kind == 1:
...
class _StoreView:
    """Extraction from a given store (a thread's private copy)."""
