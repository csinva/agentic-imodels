...
MULTICORE = True
DESCRIPTION = ('v39 with the depth-3 triple stage (v20) switched off by default (TRIPLE_MAX_NV = 0; the code stays available).  Under the compiled parallel engine an expansion costs microseconds, so the exact 3-leaf and (2,2) values the stage computes for every child (nv^3/6 triple kernels) cost more than the expansions they save: with it off, in the harness loop, car 0.6-0.7x, monk_2 0.7-0.85x, monk_3 0.58-0.88x, coupon 0.8-0.95x, nothing slower; it never ran on the wide or the large data sets (its gates exclude them).  Without the stage the exact small-tree rule uses the depth-2 values (3 leaves) as before v20.  Bounds unchanged')
...
SH_MIN_DIV = int(os.environ.get("SH_MIN_DIV", "64"))
QOS_MODE = int(os.environ.get("QOS_MODE", "0"))      # 0 none; 1 all threads user-interactive; 2 P-threads front / E-threads back
QOS_P_THREADS = int(os.environ.get("QOS_P_THREADS", "4"))
try:
    import ctypes
...
def _set_qos(cls):
    """macOS thread quality-of-service class (0x21 user-interactive ... 0x09 background)."""
    if _LIBC is not None:
        try:
            _LIBC.pthread_set_qos_class_self_np(cls, 0)
        except Exception:
            pass        # share subproblems with >= n / SH_MIN_DIV rows
...
class CompiledOptimizer:
    ...
    def _run_parallel(self, root, budget):
        ...
        def worker(tid):
            fast = QOS_MODE == 0 or tid < QOS_P_THREADS
            if QOS_MODE == 1:
                _set_qos(0x21)
            elif QOS_MODE == 2:
                _set_qos(0x21 if fast else 0x11)
            ...
            try:
                while True:
                    with lock:
                        if not tasks:
                            break
                        pos = tasks.popleft() if (QOS_MODE != 2 or fast) else tasks.pop()
                    chunk = 2000
                    while True:
...
N_JOBS = int(os.environ.get("OPTTREE_N_JOBS", "8"))
