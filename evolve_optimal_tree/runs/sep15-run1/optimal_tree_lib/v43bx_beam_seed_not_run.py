...
MULTICORE = True
DESCRIPTION = ("v40 + a lookahead-greedy seed before the search (at each node the frame expansion's best tree - leaf, split with leaf children, or the pair stage's best two-level tree - chooses the split; both children are seeded; the greedy subtree value is installed when it beats the node's own; time-bounded to a quarter of the time limit, at most 0.1 s) so the search starts from a real incumbent, and search chunks scaled to the time limit (first chunk 4 expansions, then a twentieth of the limit, at most 50 ms) so short caps are honoured.  Exact: the seed only installs consistent structures and the bounds are unchanged")
...
BEAM_ROOT = int(os.environ.get("BEAM_ROOT", "3"))                      # alternative root splits seeded (0 = off)
...
@njit(cache=NUMBA_CACHE, nogil=True)
def greedy_seed(st, dat, pf, pi, ws, root, max_depth, max_nodes, d0):
    ...
    if d < 0:
        d = d0
        s0 = 0 if d0 == 0 else 2 * (d0 + 1) - 1
        ws[WS_SLOT][d0] = s0
        FI[s0, FI_NODE] = root; FF[s0, FF_BUDGET] = 1e300; FI[s0, FI_PHASE] = 0
    while d >= d0:
        ps = ws[WS_SLOT][d]
        node = FI[ps, FI_NODE]
...
@njit(cache=NUMBA_CACHE, nogil=True)
def root_alternatives(ws, out, exclude):
    """Root candidates ranked by the pair stage's two-level value (split_ub2), excluding
    ``exclude``; fills ``out`` with feature ids, returns how many."""
    FI = ws[WS_FI]
    nv = FI[0, FI_NV]
    if FI[0, FI_HAVE_D2] != 1 or nv <= 0:
        return 0
    feats = ws[WS_IO][0, 0, :nv]
    val = ws[WS_FO][0, 13, :nv]
    order = np.argsort(val)
    n = 0
    for k in range(nv):
        f = feats[order[k]]
        if f != exclude and val[order[k]] < 1e299:
            out[n] = f
            n += 1
            if n >= out.shape[0]:
                break
...
class CompiledOptimizer:
    ...
    def run(self):
        ...
        if self.greedy_init and GREEDY_SEED_NODES > 0:
            # lookahead-greedy seed in time-bounded rounds: two expansions first to measure the
            # cost of one on this data, then as many as fit the seed's share of the time limit
            ...
            while todo > 0 and self.seed_nodes < GREEDY_SEED_NODES:
                ...
                todo = min(int((seed_budget - elapsed) / max(per, 1e-6)), GREEDY_SEED_NODES - self.seed_nodes)
            # beam at the root: the next-best root splits by two-level value, each with both
            # children seeded greedily below the root's frame; the best root tree wins
            alt = np.empty(BEAM_ROOT, dtype=np.int64)
            n_alt = int(root_alternatives(self.ws, alt, np.int64(st[ST_SPLIT][root]))) if BEAM_ROOT > 0 else 0
            kw_a = np.empty(self.data.W, dtype=np.uint64); kw_b = np.empty(self.data.W, dtype=np.uint64)
            for a in range(n_alt):
                elapsed = time.perf_counter() - self.start_time
                if elapsed >= seed_budget:
                    break
                f = int(alt[a])
                child_key(st, self.dat, root, f, True, kw_a); child_key(st, self.dat, root, f, False, kw_b)
                ln = self._node_id(kw_a); rn = self._node_id(kw_b); st = self.st; meta = st[ST_META]
                per = elapsed / max(self.seed_nodes, 1)
                for cn in (ln, rn):
                    todo = int((seed_budget - (time.perf_counter() - self.start_time)) / max(per, 1e-6))
                    if todo <= 0:
                        break
                    self.ws[WS_SLOT][MAXD] = -1
                    meta[2] = 0
                    self.seed_nodes += int(greedy_seed(st, self.dat, self.pf, self.pi, self.ws, np.int64(cn), np.int64(24),
                                                       np.int64(todo), np.int64(1)))
                    if meta[2] == 2:
                        self._alloc(st[ST_KEYS].shape[0] * 2, st); st = self.st; meta = st[ST_META]
                val = float(st[ST_UB][ln] + st[ST_UB][rn])
                if val < st[ST_UB][root] - EPS:
                    st[ST_UB][root] = val
                    if st[ST_SPLIT][root] != f:
                        st[ST_PEND][root, 0] = 0
                    st[ST_SPLIT][root] = f
...
def _compile_search():
    ...
    root_alternatives(ws, np.empty(2, dtype=np.int64), np.int64(-1))
