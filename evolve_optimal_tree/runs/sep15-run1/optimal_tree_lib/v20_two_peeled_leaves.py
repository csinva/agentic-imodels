...
MODEL_NAME = "v20_two_peeled_leaves"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v19 + tighter floors for the subtrees the depth-3 stage does not enumerate: a (1,3) or "
               "(3,1) subtree on a child has a leaf at depth 1 and a leaf at depth 2, both cells known from "
               "the triples, so 4-leaf trees cost >= 4 lambda + min over splits of leaf(A) + cheapest "
               "sub-cell of B; (2,3)/(3,2) trees >= 5 lambda + best 2-leaf(A) + cheapest sub-cell of B; "
               "(3,3) trees >= 6 lambda + cheapest sub-cell of A + cheapest sub-cell of B")
