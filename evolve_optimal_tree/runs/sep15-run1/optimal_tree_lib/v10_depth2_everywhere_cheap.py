...
MODEL_NAME = "v10_depth2_everywhere_cheap"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v9 + the depth-2 pairwise stage also runs on every node where it is cheap "
               "(candidates^2 x words <= 32k word-ops, i.e. the narrow binary datasets), not only "
               "where many candidates survive, so exact resolution below 4 lambda and the "
               "min(leaf, 2 lambda + best2, 3 lambda) child bounds apply throughout those searches")
