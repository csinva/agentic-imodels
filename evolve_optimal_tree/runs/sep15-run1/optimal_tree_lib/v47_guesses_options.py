...
MULTICORE = True
DESCRIPTION = ("v45 (greedy seeds + bottom-up repair) with four changes for short caps on wide data: (1) an expansion's best tree (leaf-sum split or the pair stage's two-level tree, children installed) becomes the node's incumbent immediately, so a frame left early by the seed or re-armed from the cache keeps it (before, the seed's root expansion found fico_1k's 4-leaf optimum and lost it); (2) the root may spend 2.5 per-expansion shares of the cap on its full pair stage (it is expanded once); (3) when the full pair stage is over budget, a budgeted stage evaluates the k splits with the largest gini gain against all others for the incumbent only (no bounds from it; charged to a global 20%% budget per fit); (4) the repair also solves the incumbent's leaves as subproblems, and search chunks are sized by the slowest expansion seen so far, so caps hold to the millisecond on wide data.  Approximate by the cap only")
