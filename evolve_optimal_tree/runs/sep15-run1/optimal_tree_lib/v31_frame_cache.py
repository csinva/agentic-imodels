...
MULTICORE = True
DESCRIPTION = ("v30 + a per-frame expansion cache: the budget deepening re-solves a child several times "
               "with growing budgets, and each re-entry used to redo its kernels; now each depth has two "
               "frame slots (one per sibling), a slot keeps its node's expansion (kernel outputs, "
               "depth-2/3 values, raised bounds) and a re-entry with a larger budget only redoes the "
               "budget-dependent steps (probe checks, candidate filtering and ordering, exact-resolution "
               "tests).  Same search and bounds")
...
# float frame fields
FF_BUDGET, FF_BEST, FF_BOUND, FF_MINPR, FF_LBMAX, FF_STEP, FF_BF, FF_BS, FF_EXACT_BELOW, FF_LEAF, FF_OUT_KIND, FF_OUT_VALUE, FF_BD2, FF_LBGE4, FF_LBREST = range(15)
...
def make_workspace(m, K, W):
    return (np.zeros((NSLOT, 18), dtype=np.int64), np.zeros((NSLOT, 15)), np.zeros((NSLOT, 7, m), dtype=np.int64),
            ...
            np.zeros((NSLOT, m, 2, 6), dtype=np.int64), np.zeros((NSLOT, 4, m)), np.zeros((NSLOT, 2, m), dtype=np.int64),
