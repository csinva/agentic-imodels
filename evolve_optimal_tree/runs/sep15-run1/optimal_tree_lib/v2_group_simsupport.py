...
MODEL_NAME = "v2_group_simsupport"
DESCRIPTION = ("similar-support bound propagated across every threshold of a numeric column: "
               "a proven bound v on split t gives lb >= v - |pot_t - pot_t'| for all thresholds t' "
               "of the same column (one vectorised slice update per proven value), replacing the "
               "immediate-neighbour check")
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        return feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot
    ...
    def _greedy(self, node: Node, features: np.ndarray, depth: int = 0) -> float:
        ...
        if stats is None:
            ...
            return node.ub
        feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, _ = stats
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if stats is None:
            ...
            node.solved = True
            return
        feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot = stats
        data = self.data
        F = data.features
        key = node.key
        memo = self.memo
