...
MODEL_NAME = "v39_own_memory_accounting"
...
MULTICORE = True
DESCRIPTION = ("v37 with the memory guard counting the solver's own live allocations (the main store, every thread's store and the pooled table) instead of the process RSS: on macOS freed large arrays stay in the allocator's cache and RSS never drops, so later fits in a long run were stopped 'for memory' within a second although they held almost nothing.  The cap (6 GB) and its meaning are unchanged; the measure is now deterministic.  Search and bounds unchanged")
