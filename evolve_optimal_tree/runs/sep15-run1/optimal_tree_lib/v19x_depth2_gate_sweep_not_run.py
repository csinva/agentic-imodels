...
MODEL_NAME = "v19_depth2_gate_sweep"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v16b + the budget-independent part of a node's expansion (child statistics, pairwise "
               "kernel, shape bound) is kept on the node while its parent deepens its budget, so the 27% of "
               "expansions that are re-expansions skip the kernel stage; probes return before any slicing; "
               "and the kernel also reports each child's leaf prediction, so creating a child node no "
               "longer recomputes its class counts from the big-int mask")
...
# depth-2 stage gate (see expand_kernel): run when nv^2 W <= GATE_CHEAP_LIMIT, or when at least
# GATE_CAND_FRAC of the candidates survive the cheap filter and there are >= GATE_MIN_CAND of them
GATE_CHEAP_LIMIT = float(os.environ.get("GATE_CHEAP_LIMIT", "32768"))
GATE_CAND_FRAC = float(os.environ.get("GATE_CAND_FRAC", "0.5"))
GATE_MIN_CAND = float(os.environ.get("GATE_MIN_CAND", "8"))


@njit(cache=True, nogil=True)
def expand_kernel(F, features, group_of, M, weights, costs, diff, lam, bound, do_exchange,
                  io, fo, bo, L, dist, cheap_limit, cand_frac, min_cand):
    ...
    if single:
        for t in range(1, nv):
            if gidx[t] != gidx[0]:
                single = False
                break
    cheap = nv * nv * W <= cheap_limit
    if single or not (n_cand >= 2 and nv >= 2 and (cheap or (n_cand >= cand_frac * nv and n_cand >= min_cand and nv >= 8))):
        return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0
    depth2_pairs(F, feats, group_of, M[:K], costs, lam, dist, L[:nv], fo[7, :nv], fo[8, :nv], io[2, :nv], io[3, :nv],
                 fo[9, :nv], fo[10, :nv])
...
def warm_up():
    ...
    node_stats(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
               ...
               np.empty(1), np.empty(1, dtype=np.int64), np.empty(1, dtype=np.int64))
    ...
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
                  ...
                  np.empty((2, 1), dtype=np.bool_), np.empty((1, 1)), np.empty(1), 32768.0, 0.5, 8.0)
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4 = expand_kernel(
            ...
            GATE_CHEAP_LIMIT, GATE_CAND_FRAC, GATE_MIN_CAND)
        if nv == 0:
            return None
        nv = int(nv)
        if nv < mf:
