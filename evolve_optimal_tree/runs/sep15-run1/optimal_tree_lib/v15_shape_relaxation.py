...
MODEL_NAME = "v15_shape_relaxation"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v14 + shape-relaxation bound from the pairwise counts: every tree with >= 4 leaves "
               "has a root split with a- and b-leaf subtrees; with exact 1- and 2-leaf values, a "
               "peeled-leaf bound 3 lambda + min cell cost for 3-leaf subtrees and a lambda otherwise, "
               "the minimum over (split, a, b) lower-bounds all such trees. A node whose best depth-2 "
               "tree is no worse than that bound is solved at any budget; children get "
               "lb = min(leaf, 2 lambda + best2, 3 lambda + best cell, 4 lambda)")
...
class Optimizer:
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if (self.engine == "numba" and cand.shape[0] >= 2 and mf_ >= 2
            ...
            i, best_d2, lb_ge4 = shape_bound(l_leaf, ml2_l, c3_l, r_leaf, ml2_r, c3_r, lam, l_lb, r_lb,
                                             l_ub2, r_ub2, split_lb, split_ub2)
