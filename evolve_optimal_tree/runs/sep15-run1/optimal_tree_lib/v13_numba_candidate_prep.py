...
MODEL_NAME = "v13_numba_candidate_prep"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v10 + the per-node preparation (split bounds, the neighbouring-threshold "
               "exchange test, the cheap filter and the candidate ordering) runs in one numba "
               "kernel instead of ~12 small numpy calls; same bounds, same order, same search")
