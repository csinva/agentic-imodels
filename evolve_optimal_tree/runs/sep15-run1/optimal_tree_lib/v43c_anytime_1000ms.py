...
MODEL_NAME = "v43c_anytime_1000ms"
...
MULTICORE = True
DESCRIPTION = ('v43 anytime (time limit capped at 1000 ms per pair, incumbent returned at the cap, certified only when the search finished earlier) with a pair-stage budget: the pair kernel of an expansion costs about nv^2 * K * W word operations (0.4 ns each), and an expansion skips the stage when that exceeds a tenth of the time limit (never binds at 30 s; under a 10 ms cap the root of a 1357-feature problem no longer costs 25 ms).  With the cap-scaled chunking, the lookahead-greedy seed and the hand-off rules of v43, caps are honoured on every pair.  Approximate by the cap only; the bounds are unchanged, only fewer of them are computed')
...
@njit(cache=NUMBA_CACHE, nogil=True)
def greedy_seed(st, dat, pf, pi, ws, root, max_depth, max_nodes):
...
class CompiledOptimizer:
    ...
    def __init__(self, data: BitDataset, regularization: float, *, groups=None, time_limit=0.0,
        ...
        # pair-stage budget: the pair kernel of one expansion costs about nv^2 * K * W word
        # operations; under a short time limit an expansion may not spend more than a share
        # of it on that stage (never binds at the 30 s cap: the largest root here is 6e7 ops)
        pair_cap = 0.0
        if time_limit > 0.0:
...
def _compile_search():
    ...
    ws[WS_SLOT][MAXD] = -1
    greedy_seed(opt.st, dat, pf, pi, ws, np.int64(opt.root), np.int64(2), np.int64(1))
...
TIME_CAP = 1.000   # anytime rows: cap the time limit (seconds, 0 = none)
