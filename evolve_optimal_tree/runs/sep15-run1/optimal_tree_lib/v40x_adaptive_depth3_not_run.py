...
MODEL_NAME = "v40_adaptive_depth3"
...
MULTICORE = True
DESCRIPTION = ("v39 with the depth-3 triple stage made adaptive: its value depends on the data set (removing it makes car and coupon 25-35% faster and tic-tac-toe or fico_binary a little slower), so each search counts the stage's runs and its pay-offs (node resolved by its bounds, incumbent improved by its trees, a child solved exactly through the 4-leaf rule it enables) and stops running it once 32 runs paid off less than one time in eight.  Counts are carried into the threads' stores at the hand-off.  Bounds unchanged; a skipped stage only removes an optional tightening")
...
D3_ADAPT_MIN_RUNS = int(os.environ.get("D3_ADAPT_MIN_RUNS", "32"))   # runs before the pay-off rate is judged
D3_ADAPT_RATE = float(os.environ.get("D3_ADAPT_RATE", "8"))         # keep the stage while wins * RATE >= runs
...
def _expand_frame(st, dat, pf, pi, ws, d):
    ...
    if ran:
        ...
        if (pi[PI_D3] == 1 and nv <= TRIPLE_MAX_NV and K * W <= D3_MAX_KW
                ...
                and (meta[4] < D3_ADAPT_MIN_RUNS or meta[5] * D3_ADAPT_RATE >= meta[4])):
            meta[4] += 1
            ...
            if have_d2:
                ...
                for side in range(2):
                    ...
                    if nsolved[cn] == 0 and bound - nlb[other] < eb - EPS and nub[cn] <= bound - nlb[other] + EPS:
                        if have_d3 and bound - nlb[other] >= 3.0 * lam - EPS:
                            meta[5] += 1          # solved through the 4-leaf rule the stage enables
                        nlb[cn] = nub[cn]
                        nsolved[cn] = 1
    ...
    def _run_parallel(self, root, budget):
        ...
        def worker(tid):
            ...
            with lock:
                ...
                state["d3"] = state.get("d3", (0, 0)); state["d3"] = (state["d3"][0] + int(meta_k[4]), state["d3"][1] + int(meta_k[5]))
        ...
        self.d3_threads = state.get("d3", (0, 0))
