...
MODEL_NAME = "v18_row_clustering_active_words"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v16b + rows sorted lexicographically by the binary features (most informative first) "
               "so capture sets cluster into few 64-bit words, and the node kernels only visit the words "
               "of the node's mask that are non-zero (fico_binary: 40% of the words instead of 95%); the "
               "budget-independent part of an expansion is also kept on the node while its parent deepens "
               "its budget, so re-expansions skip the kernel stage.  Row order does not change the problem")
...
@njit(cache=True, nogil=True)
def word_span(kw):
    """[w0, w1): the span of words of a capture mask outside of which every word is zero."""
    W = kw.shape[0]
    w0 = 0
    while w0 < W and kw[w0] == 0:
        w0 += 1
    w1 = W
    while w1 > w0 and kw[w1 - 1] == 0:
        w1 -= 1
    return w0, w1


@njit(cache=True, nogil=True)
def node_stats(F, feats, group_of, masks, weights, costs, diff, lam, w0, w1,
...
@njit(cache=True, nogil=True)
def depth2_pairs(F, feats, group_of, masks, costs, lam, dist, L, w0, w1, out_ml2_l, out_ml2_r, out_j_l,
                 out_j_r, out_c3_l, out_c3_r):
...
@njit(cache=True, nogil=True)
def expand_kernel(F, features, group_of, M, weights, costs, diff, lam, bound, do_exchange, kw,
                  io, fo, bo, L, dist):
    ...
    w0 = 0
    w1 = W
    nv = node_stats(F, features, group_of, M, weights, costs, diff, lam, w0, w1, io[0], L, fo[0], fo[1], bo[0],
    ...
    depth2_pairs(F, feats, group_of, M[:K], costs, lam, dist, L[:nv], w0, w1, fo[7, :nv], fo[8, :nv], io[2, :nv],
...
def warm_up():
    ...
    node_stats(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
               np.zeros(1), 0.1, 0, 1, np.empty(1, dtype=np.int64), np.empty((1, 1)), np.empty(1), np.empty(1),
    ...
    depth2_pairs(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros((1, 1)), 0.1,
                 np.zeros(1), np.zeros((1, 1)), 0, 1, np.empty(1), np.empty(1),
                 np.empty(1, dtype=np.int64), np.empty(1, dtype=np.int64), np.empty(1), np.empty(1))
    e = np.zeros(1)
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
                  np.zeros(1), 0.1, 1.0, False, np.zeros(1, dtype=np.uint64), np.empty((4, 1), dtype=np.int64),
                  np.empty((14, 1)),
...
class Node:
    __slots__ = ("key", "count", "leaf_risk", "prediction", "lb", "ub", "split", "solved", "cache")
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4 = expand_kernel(
            ...
            self.continuous_feature_exchange and self._has_groups, kw, io, fo, bo, L, dist)
        if nv == 0:
            return None
        nv = int(nv)
        if nv < mf:
...
class GOSDTClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, target_name: str = "class"):
        ...
        # visit only the non-zero words of a node's capture mask, so rows are sorted
        # lexicographically by the binary features (most informative at the root first)
        # to make the capture sets of deep nodes contiguous-ish.
