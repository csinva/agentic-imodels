...
MODEL_NAME = "v28_compiled_search"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v23 with the whole branch-and-bound compiled: nodes live in arrays (mask words, "
               "counts, bounds, split, deferred depth-3 structure) behind an open-addressing index, the "
               "recursive search with all its stages (kernel probes, single-column DP, depth-2/depth-3 "
               "stages, similar-support propagation, budget deepening) runs as one numba function, and "
               "Python only re-enters it in ~50 ms chunks to enforce the time and memory limits and to "
               "grow the store.  Same bounds and search order as v23; no Python objects per node")
...
# float frame fields
FF_BUDGET, FF_BEST, FF_BOUND, FF_MINPR, FF_LBMAX, FF_STEP, FF_BF, FF_BS, FF_EXACT_BELOW, FF_LEAF = range(10)
...
def make_workspace(m, K, W):
    return (np.zeros((MAXD, 16), dtype=np.int64), np.zeros((MAXD, 10)), np.zeros((MAXD, 7, m), dtype=np.int64),
...
@njit(cache=NUMBA_CACHE, nogil=True)
def solve_iter(st, dat, pf, pi, ws, root, budget):
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                solve_iter(st, self.dat, self.pf, self.pi, self.ws, np.int64(root), float(budget))
