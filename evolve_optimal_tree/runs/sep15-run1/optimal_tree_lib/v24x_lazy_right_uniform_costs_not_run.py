...
MODEL_NAME = "v24_lazy_right_child_uniform_costs"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v23 + in the candidate loop the right child's mask and memo entry are only computed when "
               "the left child's bound plus the kernel's bound for the right child leaves the split "
               "alive (memoised children also take the kernel's bound when it is larger), and with "
               "uniform costs the kernels compute a cell's leaf cost as w * (size - largest class) "
               "instead of the K x K cost loop.  Same bounds")
...
@njit(cache=True, nogil=True)
def node_stats(F, feats, rows, group_of, masks, weights, costs, uniform_w, diff, lam,
    ...
    for t in range(feats.shape[0]):
        ...
        rpred = 0
        if uniform_w > 0.0:
            # uniform costs: the best label is the largest class (first one on ties, as the
            # cost loop below would pick), the leaf loss w * (size - that count)
            lbest = -1.0
            rbest = -1.0
            for p in range(K):
                v = cnt[p]
                ...
                if v > rbest:
                    rbest = v
                    rpred = p
            lmax = uniform_w * (lsum - lbest)
            rmax = uniform_w * (rsum - rbest)
        else:
            for p in range(K):
                al = 0.0
                ar = 0.0
...
@njit(cache=True, nogil=True)
def _leaf_cost_u(cnt, costs, K, uniform_w):
    """Leaf cost of a cell; with uniform costs (all mismatches w, matches 0) it is
    w * (size - largest class), otherwise the K x K loop."""
    if uniform_w > 0.0:
        sz = 0.0
        mx = 0.0
        for k in range(K):
            ...
            if v > mx:
                mx = v
        return uniform_w * (sz - mx)
    return _leaf_cost(cnt, costs, K)
...
@njit(cache=True, nogil=True)
def depth2_pairs(F, feats, rows, group_of, masks, costs, uniform_w, lam, dist, L, out_ml2_l, out_ml2_r, out_j_l,
    ...
    for i in range(mf):
        ...
        for j in range(i + 1, mf):
            ...
            if sij > 0.0:
                for k in range(K):
                    a[k] = ij[k]
                c_ij = _leaf_cost_u(a, costs, K, uniform_w)
            if ri > 0.0:
                for k in range(K):
                    a[k] = L[i, k] - ij[k]
                c_inj = _leaf_cost_u(a, costs, K, uniform_w)
            if rj > 0.0:
                for k in range(K):
                    a[k] = L[j, k] - ij[k]
                c_nij = _leaf_cost_u(a, costs, K, uniform_w)
            if rest > 0.0:
                for k in range(K):
                    a[k] = dist[k] - L[i, k] - L[j, k] + ij[k]
                c_rest = _leaf_cost_u(a, costs, K, uniform_w)
...
@njit(cache=True, nogil=True)
def expand_kernel(F, features, group_of, kw, mask_matrix, weights, costs, uniform_w, diff, lam, bound, do_exchange,
                  io, fo, bo, L, dist):
    ...
    if nA * 10 <= W * 7:
        ...
        rows_in = features
    nv = node_stats(Fc, features, rows_in, group_of, M, weights, costs, uniform_w, diff, lam, io[0], L, fo[0], fo[1], bo[0],
    ...
    depth2_pairs(Fc, feats, rows, group_of, M[:K], costs, uniform_w, lam, dist[:K], L[:nv], fo[7, :nv], fo[8, :nv], io[2, :nv],
...
def warm_up():
    ...
    node_stats(F, np.zeros(1, dtype=np.int64), np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks,
               np.zeros(0), np.zeros((1, 1)), 0.0, np.zeros(1), 0.1, np.empty(1, dtype=np.int64), np.empty((1, 1)),
    ...
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), int_to_words(1, 1), masks, np.zeros(0),
                  np.zeros((1, 1)), 0.0, np.zeros(1), 0.1, 1.0, False, np.empty((7, 1), dtype=np.int64), np.empty((14, 1)),
...
class Optimizer:
        ...
        if self.engine == "numba":
            ...
            if data.zero_diagonal and data.equal_mismatch:
                ...
                nv = node_stats(data.F_words, features, features, self.group_of, M, data.weights1, data.costs,
                                self._uniform_w, self._diff, lam, *buf)
                if nv == 0:
                    return None
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, M, Fc = expand_kernel(
            data.F_words, features, self.group_of, kw, mask_matrix, weights, data.costs, self._uniform_w, self._diff, self.lam, bound,
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        for i in order:
            ...
            rlb0 = float(r_lb[i])
            if ln is None:
                llb, lub = llb0, float(l_leaf[i])
            else:
                llb, lub = ln.lb, ln.ub
                if llb0 > llb:
                    llb = llb0      # this node's kernel bound for the child is also valid
            slb = llb + rlb0
            if slb > bound + EPS:
                # the split is dead by the left child alone: the right child's mask and
                # memo entry are never computed
                if slb < cur:
                    slb = cur
                if slb < min_pruned:
                    min_pruned = slb
