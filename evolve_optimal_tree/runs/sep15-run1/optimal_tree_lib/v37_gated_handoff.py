...
MULTICORE = True
DESCRIPTION = ("v36 with the hand-off to the threads gated on the root's remaining work: at the end of a sequential chunk the root frame's position in its candidate order gives the completed and remaining candidates, and the threads are started only when at least two candidates remain and the remaining work, extrapolated from the elapsed time, exceeds twice the parallel set-up cost (about 4 ms); otherwise the search continues sequentially and the check repeats at the next chunk.  Fits of 3-6 ms no longer pay the parallel phase for nothing.  Search and bounds unchanged")
