...
MODEL_NAME = "v3_column_dp"
DESCRIPTION = ("v2 + exact single-column segmentation DP at every node: trees over one numeric "
               "column's thresholds are exactly the segmentations of the sorted points, so a "
               "quadratic DP over cumulative class counts (from the nested threshold counts) gives "
               "the optimum when only one column splits the node (whole 1-D datasets solve at the "
               "root) and a valid incumbent tree per column otherwise")
