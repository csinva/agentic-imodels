...
MODEL_NAME = "v16_fused_expansion"
...
DESCRIPTION = ("v15 with the same search and bounds but a cheaper node expansion: 76% of expansions on "
               ...
               "stored in the memo when the node survives the shape relaxation")
...
def expand_kernel(F, features, group_of, M, weights, costs, diff, lam, bound, do_exchange,
    ...
    cheap = nv * nv * W <= 32768
    if not (n_cand >= 2 and nv >= 2 and (cheap or (n_cand * 2 >= nv and n_cand >= 8 and nv >= 8))):
