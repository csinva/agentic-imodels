...
MODEL_NAME = "v49_anytime_20ms"
EXACT = False  # this solver certifies optimality; set False for a heuristic variant
# whether the solver uses more than one core (recorded in the multicore column)
MULTICORE = True
DESCRIPTION = ('v49 run as an anytime method with the time limit capped at 20 ms per pair (the v46 anytime machinery: cap-scaled chunks, greedy seeds, bottom-up repair, budgeted pair stage) on top of the candidate-only pair stage and lazy workspaces.  Approximate by the cap only')
...
TIME_CAP = 0.020   # anytime rows: cap the time limit (seconds, 0 = none)
