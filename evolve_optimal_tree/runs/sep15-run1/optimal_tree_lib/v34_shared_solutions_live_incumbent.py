...
MODEL_NAME = "v34_shared_solutions_live_incumbent"
...
MULTICORE = True
DESCRIPTION = ("v33 + two changes to the parallel phase: (1) the shared table also carries exactly solved subproblems (value written once per slot, NaN otherwise), so a thread entering a subproblem another thread solved adopts its optimum instead of re-expanding it; the tree is taken from the owner's store at extraction, after all threads joined; (2) an improved shared incumbent is pushed down the whole frame stack of a task (child budgets re-derived from the parents' bounds exactly as when pushed) instead of reaching only its root frame.  Only exact optima and lower bounds cross threads and budgets only tighten, so certification is unchanged")
...
def shared_table_capacity(T, W):
    """Slots per region: a power of two within the memory budget (at least 4096, at most 2**16)."""
    per_slot = 8 * W + 33
    C = 4096
    while C * 2 * T * per_slot <= SH_MEM_MB * 1e6 and C < (1 << 16):
        C *= 2
    return C
...
class CompiledOptimizer:
    ...
    def _run_parallel(self, root, budget):
        ...
        sh = make_shared_table(n_threads, shared_table_capacity(n_threads, self.data.W), self.data.W,
                               max(2, self.data.n // SH_MIN_DIV))
        stores = {}
