...
MODEL_NAME = "v26_mc8_shared_bound_table"
...
MULTICORE = True
DESCRIPTION = ("v25 + a shared lower-bound table: during the parallel phase the workers publish the "
               "lower bounds they prove for large subproblems (>= 1/32 of the rows) in a lock-protected "
               "open-addressing table in shared memory keyed by the full capture mask, and look the "
               "table up when they create such a child, so a subproblem that another worker already "
               "pruned is pruned without being re-expanded.  Only lower bounds are shared (a value "
               "without its tree would not be extractable), so exactness is unchanged")
...
def warm_up():
    ...
    # the shared-table kernels must be compiled before the workers are forked
    _table_find(np.zeros((1, 1), dtype=np.uint64), np.zeros(1, dtype=np.int64), np.zeros(1, dtype=np.uint8),
                int_to_words(1, 1), 1, 0, 1)
    _key_hash(int_to_words(1, 1), 1)
...
@njit(cache=True, nogil=True)
def _table_find(keys, counts, used, kw, count, h, cap):
    """Slot of key ``kw`` (linear probing from h) or -1 - (first free slot); an entry
    matches only if its row count and every key word agree."""
    W = kw.shape[0]
    slot = h
    for _ in range(cap):
        if used[slot] == 0:
...
@njit(cache=True, nogil=True)
def _key_hash(kw, cap):
    h = np.uint64(1469598103934665603)
    for w in range(kw.shape[0]):
        h = (h ^ kw[w]) * np.uint64(1099511628211)
    return int(h % np.uint64(cap))


# subproblems with fewer rows than this fraction of the data are not shared (too cheap to matter)
TABLE_MIN_FRAC = float(os.environ.get("TABLE_MIN_FRAC", "0.25"))


class SharedBoundTable:
    """Lower bounds of subproblems shared between the parallel workers.

    Fixed-capacity open-addressing table in shared memory (inherited by forking), keyed by
    the full capture mask (W words) plus its row count, storing the best lower bound proven
    for it.  Writes hold the lock; reads do not, which is safe because slots are written
    once (a key is never overwritten by another), bounds only ever increase (a stale read
    is an older, smaller bound of the same key or the initial 0), and a partially written
    key can only coincide with a strict subset of it, which has a different row count.
    """

    def __init__(self, ctx, W: int, capacity: int):
        self.W = W
        self.cap = capacity
        self._keys = ctx.RawArray("Q", capacity * W)
        self._counts = ctx.RawArray("q", capacity)
        self._lbs = ctx.RawArray("d", capacity)
        self._used = ctx.RawArray("B", capacity)
        self.lock = ctx.Lock()
        self.n_full = 0
        self._bind()

    def _bind(self):
        self.keys = np.frombuffer(self._keys, dtype=np.uint64).reshape(self.cap, self.W)
        self.counts = np.frombuffer(self._counts, dtype=np.int64)
        self.lbs = np.frombuffer(self._lbs, dtype=np.float64)
        self.used = np.frombuffer(self._used, dtype=np.uint8)

    def get(self, key: int, count: int) -> float:
        kw = int_to_words(key, self.W)
        h = _key_hash(kw, self.cap)
        slot = _table_find(self.keys, self.counts, self.used, kw, count, h, self.cap)
        return float(self.lbs[slot]) if slot >= 0 else -1.0

    def raise_lb(self, key: int, count: int, lb: float) -> None:
        kw = int_to_words(key, self.W)
        h = _key_hash(kw, self.cap)
        with self.lock:
            slot = _table_find(self.keys, self.counts, self.used, kw, count, h, self.cap)
...
class Optimizer:
    def __init__(self, data: BitDataset, regularization: float, *,
        ...
        self.bound_table = None      # SharedBoundTable during the parallel phase (workers)
        self.table_min_count = 0
        self.start_time = 0.0
        self.elapsed = 0.0
        self.optimal = False
    ...
    # ------------------------------------------------------------ parallel
    def _root_candidates(self, root, features, bound):
        """The root's candidate splits (indices into its feats) in the order the sequential
        loop would visit them, plus the smallest bound among the splits filtered out."""
        res = self._expand(root, features, min(bound, root.ub))
        if res is None:
            return None, [], 1e300
        io, fo, bo, L, dist, mf_, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, pij = res
        split_lb = fo[5]
        min_pruned = float(min_rejected)
        order = io[1, :n_cand].tolist()
        keep = []
        for c in order:
            if split_lb[c] <= bound + EPS:
                keep.append(c)
            elif split_lb[c] < min_pruned:
                min_pruned = float(split_lb[c])
        return res, keep, min_pruned

    def _run_parallel(self, root, budget, features):
        """Parallel phase over the root's candidate splits (see DESCRIPTION)."""
        ctx = multiprocessing.get_context("fork")
        bound = min(budget, root.ub)
        res, cand, min_pruned = self._root_candidates(root, features, bound)
        self.parallel_used = True
        if res is None:
            root.lb = root.ub = root.leaf_risk
            root.split = -1
            root.solved = True
            return
        shared_best = ctx.Value("d", float(root.ub))
        lock = ctx.Lock()
        # shared lower-bound table for subproblems with at least 1/32 of the rows
        table = SharedBoundTable(ctx, self.data.W, 1 << 16)
        self.bound_table = table
        self.table_min_count = max(2, int(self.data.n * TABLE_MIN_FRAC))
        tasks = ctx.Queue()
        results = ctx.Queue()
        # the workers inherit this process (memo, root expansion, dataset) by forking; the
        # queues are only fed after the fork so no feeder thread is alive when forking
        per_worker_memory = self.memory_limit // self.n_jobs if self.memory_limit > 0 else 0
        workers = [ctx.Process(target=_parallel_worker,
                               args=(self, root, features, budget, shared_best, lock, tasks, results,
        ...
        try:
            while alive > 0:
                ...
                if kind == "done":
                    ...
                    continue
                _, c, outcome, value, lb, tree = msg
                done += 1
                if outcome == "solved":
                    # a worker attaches the tree only when its value lowered the shared
                    # incumbent; a solved value without a tree is matched or beaten by
                    # another worker's message that carries one
    ...
    def _solve_root_split(self, root, feats_all, c, res, budget, shared_best, lock):
        """Solve root split ``c`` (index into the root's feats) exactly as the sequential
        loop does, reading the shared incumbent as the bound.  Returns
        ("solved", value, child_lb_max, raw tree) or ("pruned", split lb, child_lb_max, None)."""
        io, fo, bo, L, dist, mf_, n_cand, i0, min_rejected, ran, i_d2, best_d2, lb_ge4, pij = res
        feats = io[0]; l_pred = io[4]; r_pred = io[5]
        l_leaf = fo[0]; l_lb = fo[1]; r_leaf = fo[2]; r_lb = fo[3]
        l_solved = bo[0]; r_solved = bo[1]
        d3 = pij if (pij is not None and len(pij) == 11) else None
        if ran:
            j_l, j_r, l_ub2, r_ub2 = io[2], io[3], fo[11], fo[12]
        else:
            l_ub2 = r_ub2 = None
        if d3 is not None:
            val3, arg3, l_ub3, r_ub3, l_lb3, r_lb3, kind_l, kind_r, _, _, _ = d3
            l_ub2, r_ub2 = l_ub3, r_ub3
        data = self.data
        F = data.features
        key = root.key
        memo = self.memo
        ...
        i = c
        bound = min(budget, shared_best.value)
        j = int(feats[i])
        lkey = key & F[j]
        rkey = key ^ lkey
        ln = memo.get(lkey)
        ...
        while look_ahead and not first.solved:
            bound = min(bound, shared_best.value)
            bf = bound - second.lb
            if first.lb > bf + EPS:
                pruned = True
                break
        ...
        tree = None
        with lock:
            if value < shared_best.value - EPS:
    ...
    def _child_node(self, key: int, count: int, leaf: float, lb: float, solved: bool,
        ...
        if table is not None and count >= self.table_min_count and not solved:
            shared_lb = table.get(key, count)
            if shared_lb > lb:
                lb = shared_lb
...
def _parallel_worker(opt, root, features, budget, shared_best, lock, tasks, results, memory_limit):
    """Worker process body: pull root splits until the sentinel, report each outcome."""
    opt.memory_limit = memory_limit
    opt.iterations = 0
    opt.bound_table._bind()      # fresh numpy views on the inherited shared buffers
    table = opt.bound_table
    min_count = opt.table_min_count
    inner = opt._solve

    lam_half = 0.5 * opt.lam

    def solve_and_publish(node, budget_, feats):
        lb0 = node.lb
        inner(node, budget_, feats)
        if node.count >= min_count and node.lb > lb0 + lam_half:
            table.raise_lb(node.key, node.count, node.lb)
    opt._solve = solve_and_publish
    failure = ""
    res, _, _ = opt._root_candidates(root, features, min(budget, root.ub))
    try:
        while True:
            c = tasks.get()
            if c < 0:
                break
            outcome, value, lbmax, tree = opt._solve_root_split(root, features, c, res, budget, shared_best, lock)
            results.put(("split", c, outcome, float(value), float(lbmax), tree))
    except TimeLimitReached as exc:
        failure = str(exc)
