...
MODEL_NAME = "v16b_fused_expansion"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v15 with the same search and bounds but a cheaper node expansion: 76% of expansions on "
               "the binary datasets are probes pruned by the kernel stage, so node_stats, the candidate "
               "preparation, the pairwise kernel and the shape bound run as one numba call on one "
               "workspace (5 allocations instead of 23 per node, one dispatch instead of four), "
               "node_stats derives the class counts itself, and the best depth-2 tree is only "
               "stored in the memo when the node survives the shape relaxation; the pairwise stage is "
               "skipped on single-column nodes, which the segmentation DP solves exactly")
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        fo = fo[:, :nv]
        bo = bo[:, :nv]
        d2 = (io[2], io[3], fo[11], fo[12], fo[13], int(i_d2), float(best_d2), float(lb_ge4)) if ran else None
