...
MULTICORE = True
DESCRIPTION = ("v32 with the root handed to the 8 threads on time: the search returns to Python only at the end of an iteration chunk, and the first chunk (2000 expansions) took 50-125 ms on wide data, so the first chunk is now 500 expansions and every chunk before the hand-off is sized to end at the hand-off time (5 ms).  The threads' private stores are sized by the parent's contents instead of its capacity and their key arrays are not zeroed (keys are written in full before they are indexed).  Search and bounds unchanged")
...
def make_model(regularization: float, time_limit: float):
    return OptimalTreeClassifier(regularization=regularization, time_limit=time_limit,
                                 memory_limit=6 * (1 << 30), n_jobs=N_JOBS, parallel_after=0.005)
