...
MODEL_NAME = "v14_hardened_superset_lb"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v13 + exactness hardening for every accepted input (the inherited leaf test "
               "1 - min_loss < lambda assumed losses <= 1 and is dropped; a user upperbound is a "
               "budget, never an achievable value) + the superset bound: R(C) >= R(child) for every "
               "child of every split, so the node's lower bound is raised to the largest proven "
               "child bound (restricting an optimal tree to a subset never increases its risk)")
...
def depth2_pairs(F, feats, group_of, masks, costs, lam, dist, L, out_ml2_l, out_ml2_r, out_j_l, out_j_r):
    ...
    for i in range(mf):
        ...
        for j in range(i + 1, mf):
            ...
            if sij > 0.0 and li - sij > 0.0:
                for k in range(K):
                    a[k] = ij[k]
                    b[k] = L[i, k] - ij[k]
                v = _leaf_cost(a, costs, K) + _leaf_cost(b, costs, K)
                if v < out_ml2_l[i]:
                    out_ml2_l[i] = v
                    out_j_l[i] = j
            # left child of j split by i: cells ij and (L[j] - ij)
            if sij > 0.0 and lj - sij > 0.0:
                for k in range(K):
                    a[k] = ij[k]
                    b[k] = L[j, k] - ij[k]
                v = _leaf_cost(a, costs, K) + _leaf_cost(b, costs, K)
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        if self.engine == "numba":
            ...
            if data.zero_diagonal and data.equal_mismatch:
                ...
                if nv == 0:
                    return None
                feats, L, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot = (x[:nv] for x in buf)
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if self._has_groups:
            gidx = self.group_of[feats]
            ug, g_first, g_count = np.unique(gidx, return_index=True, return_counts=True)
            g_span = {int(g): (int(a), int(a + c)) for g, a, c in zip(ug, g_first, g_count) if g >= 0}
            single = len(g_span) == 1 and ug.shape[0] == 1
        ...
        if (self.engine == "numba" and cand.shape[0] >= 2 and mf_ >= 2
            ...
            depth2_pairs(data.F_words, feats, self.group_of, self._last_class_masks, data.costs, lam, dist, L,
                         ml2_l, ml2_r, j_l, j_r)
            l_ub2 = np.minimum(l_leaf, 2 * lam + ml2_l)
            r_ub2 = np.minimum(r_leaf, 2 * lam + ml2_r)
            l_lb = np.maximum(l_lb, np.minimum(l_ub2, 3 * lam))
            r_lb = np.maximum(r_lb, np.minimum(r_ub2, 3 * lam))
            split_lb = l_lb + r_lb
            split_ub2 = l_ub2 + r_ub2
            i = int(np.argmin(split_ub2))
            if split_ub2[i] < best - EPS:
                best = float(split_ub2[i])
                best_split = int(feats[i])
                self._materialize_depth2(node, key, feats, i, j_l, j_r, l_ub2, r_ub2, l_leaf, r_leaf)
                bound = min(budget, best)
            if budget < 4 * lam - EPS:
                # Exact resolution.  Every tree on this node is (a) a leaf, (b) a depth-2
                # tree, or (c) a tree with >= 4 leaves costing >= 4 lam > budget.  `best`
                # is the exact minimum over (a) and (b) (ml2 is the exact best 2-leaf loss
                # of each child, the root split ranges over all splits not provably
                # dominated).  If best <= budget it is achieved by a tree of class (a)/(b)
                # and no class-(c) tree beats it, so R = best.  Otherwise R >= min(best,
                # 4 lam) > budget, which is the bound stored so the caller cannot mistake
                # the node for solved.
                node.ub = best
