...
MODEL_NAME = "v8_sound_lb_lean_nodes"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v7 with a soundness fix and less per-node overhead: when the exact depth-2 stage "
               "proves a node exceeds its budget its lower bound is now min(best, 4 lambda) (the proof's "
               "quantity) so callers never mistake it for solved; the per-column incumbent DP runs only "
               "at the root of multi-column data (it cost 46% of fico's time); leaf statistics of new "
               "child nodes use integer popcounts instead of numpy for uniform costs")
...
class Optimizer:
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        lam = self.lam
        l_ub2 = r_ub2 = None
        if (self.engine == "numba" and cand.shape[0] * 2 >= feats.shape[0] and cand.shape[0] >= 8
                and feats.shape[0] >= 8):
            mf = feats.shape[0]
