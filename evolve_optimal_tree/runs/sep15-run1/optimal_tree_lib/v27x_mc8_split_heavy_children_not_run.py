...
MODEL_NAME = "v27_mc8_split_heavy_children"
...
MULTICORE = True
DESCRIPTION = ("v25 with finer parallel tasks: a root split whose child holds >= 1/4 of the rows is not "
               "one task; that child's own candidate splits become tasks (its kernel stage runs in the "
               "parent before forking, so the workers inherit it) and its sibling is a task of its own, "
               "and the parent combines the outcomes exactly as the sequential loop would (a child is the "
               "minimum over its solved splits, every other split proven above an incumbent that only "
               "decreases).  Fixes the load imbalance where one root split held 61% of the work")
...
class Optimizer:
    def __init__(self, data: BitDataset, regularization: float, *,
        ...
        self.force_parallel = bool(force_parallel)   # tests: skip the sequential phase
        self.parallel_used = False
        self.parallel_tree = None    # raw tree of the parallel phase's best split, if any
        self.start_time = 0.0
        self.elapsed = 0.0
        self.optimal = False
    ...
    def run(self) -> Node:
        ...
        try:
            ...
            if self.n_jobs > 1:
                # sequential phase with a short cap; the parallel phase takes over if needed
                full_limit = self.time_limit
                seq_limit = self.parallel_after if full_limit <= 0.0 else min(full_limit, self.parallel_after)
                self.time_limit = seq_limit
                try:
                    ...
                    if str(exc) != "time" or (full_limit > 0.0 and seq_limit >= full_limit):
                        raise
                    handed_off = True
                finally:
                    self.time_limit = full_limit
                if handed_off:
                    self._run_parallel(root, budget, features)
            else:
                self._solve(root, budget, features)
    ...
    # ------------------------------------------------------------ parallel
    HEAVY_FRAC = float(os.environ.get("HEAVY_FRAC", "0.5"))   # child share of rows that gets split-level tasks

    def _run_parallel(self, root, budget, features):
        """Parallel phase (see DESCRIPTION): tasks are root splits, or for a root split with
        a heavy child that child's own splits plus its sibling as a task of its own."""
        ctx = multiprocessing.get_context("fork")
        self.parallel_used = True
        st = self._head(root, budget, features)
        if st is None:
            return
        (feats, l_leaf, l_lb, l_solved, l_pred, r_leaf, r_lb, r_solved, r_pred, l_ub2, r_ub2,
         j_l, j_r, d3_arg, order, best, best_split, bound, _, min_pruned, child_lb_max) = st
        F = self.data.features
        key = root.key
        heavy = max(2, int(self.HEAVY_FRAC * self.data.n))
        # ---- build the task list and the bookkeeping records (before forking)
        tasks = []
        records = {}
        best_tree = None
        for i in order:
            ...
            lkey = key & F[j]
            rkey = key ^ lkey
            lcount = lkey.bit_count()
            rcount = root.count - lcount
            if lcount < heavy and rcount < heavy:
                tasks.append(("root", i))
                records[i] = None
                continue
            # children as memo nodes with this node's bounds (as the loop would create them)
            ln = self._child_node(lkey, lcount, float(l_leaf[i]), float(l_lb[i]), bool(l_solved[i]), int(l_pred[i]))
            rn = self._child_node(rkey, rcount, float(r_leaf[i]), float(r_lb[i]), bool(r_solved[i]), int(r_pred[i]))
            if l_ub2 is not None:
                for cn, ub2, jj, side in ((ln, float(l_ub2[i]), int(j_l[i]), True), (rn, float(r_ub2[i]), int(j_r[i]), False)):
                    if ub2 < cn.ub - EPS:
                        if d3_arg is not None:
                            arg3, kind_l, kind_r = d3_arg
                    ...
                    if not cn.solved and cn.ub <= cn.lb + EPS:
                        ...
                        cn.solved = True
            rec = {"j": j, "sides": {}}
            for side, cn, other in (("L", ln, rn), ("R", rn, ln)):
                sr = {"key": cn.key, "lb": cn.lb, "ub": cn.ub, "solved": cn.solved, "tree": None,
                      "pending": 0, "best": 1e300, "min_pruned": 1e300, "lbmax": 0.0, "kind": "resolved",
                      "budget": bound - other.lb, "lb_other": other.lb}
                rec["sides"][side] = sr
                if cn.solved:
                    sr["tree"] = self.extract(cn)
                    continue
                if cn.count >= heavy:
                    sub = self._head(cn, sr["budget"], feats)
                    if sub is None:
                        sr.update(lb=cn.lb, ub=cn.ub, solved=cn.solved)
                        if cn.solved:
                            sr["tree"] = self.extract(cn)
                        continue
                    (_, _, _, _, _, _, _, _, _, _, _, _, _, _, sub_order, sub_best, sub_split, _, _,
                     sub_min_pruned, sub_lbmax) = sub
                    sr.update(kind="split", best=float(cn.ub), min_pruned=float(sub_min_pruned),
                              lbmax=float(sub_lbmax), pending=len(sub_order), ub=cn.ub)
                    sr["tree"] = self.extract(cn)     # the head's incumbent for this child (== cn.ub)
                    for jj in sub_order:
                        tasks.append(("split", i, side, cn.key, sr["budget"], other.lb, jj))
                    if not sub_order:
                        self._finish_side(sr)
                else:
                    sr["kind"] = "node"
                    sr["pending"] = 1
                    tasks.append(("node", i, side, cn.key, sr["budget"], other.lb))
            records[i] = rec
        shared_best = ctx.Value("d", float(best))
        lock = ctx.Lock()
        for rec in records.values():
            if rec is not None and all(x["pending"] == 0 for x in rec["sides"].values()):
                best, best_split, best_tree, min_pruned, child_lb_max = self._finish_candidate(
                    rec, best, best_split, best_tree, min_pruned, child_lb_max, shared_best, lock)
        task_q = ctx.Queue()
        results = ctx.Queue()
        per_worker_memory = self.memory_limit // self.n_jobs if self.memory_limit > 0 else 0
        workers = [ctx.Process(target=_parallel_worker,
                               args=(self, root, features, budget, shared_best, lock, task_q, results,
                                     per_worker_memory), daemon=True)
                   for _ in range(self.n_jobs)]
        for w in workers:
            w.start()
        if os.environ.get("MC_DEBUG"):
            print(f"[mc] heavy={heavy} tasks={[(t[0], t[1], t[2] if len(t) > 2 else '') for t in tasks]} best={best:.4f} order={order}", flush=True)
        for t in tasks:
            task_q.put(t)
        for _ in range(self.n_jobs):
            task_q.put(None)
        ...
        try:
            while alive > 0:
                ...
                if timeout <= 0.0:
                    ...
                    break
                try:
                    msg = results.get(timeout=min(timeout, 0.5))
                except Exception:
                    continue
                kind = msg[0]
                if os.environ.get("MC_DEBUG"):
                    print("[mc] msg", msg[:6] if kind != "done" else msg, flush=True)
                if kind == "done":
                    alive -= 1
                    extra_iterations += msg[1]
                    if msg[2]:
                        failure = failure or msg[2]
                    continue
                remaining -= 1
                if kind == "root":
                    _, i, outcome, value, lbmax, tree = msg
                    if outcome == "solved":
                        if tree is not None and value < best - EPS:
                            best = value
                            best_split = int(tree["feature"])
                            best_tree = tree
                            with lock:
                                if best < shared_best.value:
                                    shared_best.value = best
                    else:
                        if value < min_pruned:
                            min_pruned = value
                ...
                sr = rec["sides"][msg[2]]
                if kind == "node":
                    _, i, side, lb, ub, solved, tree = msg
                    sr.update(lb=max(sr["lb"], lb), ub=min(sr["ub"], ub), solved=solved, pending=0)
                    if tree is not None:
                        sr["tree"] = tree
                    sr["kind"] = "resolved"
                else:   # split task result for a heavy child
                    _, i, side, jj, outcome, value, lbmax, tree = msg
                    if os.environ.get("MC_DEBUG"):
                        print(f"[mc]   split msg cand j={rec['j']} side={side} jj={jj} {outcome} value={value:.4f} lbmax={lbmax:.4f} tree={tree is not None}")
                    if sr["kind"] == "split":
                        if outcome == "solved":
                            if value < sr["best"] - EPS:
                                sr["best"] = value
                                if tree is not None:
                                    sr["tree"] = tree
                        else:
                            if value < sr["min_pruned"]:
                                sr["min_pruned"] = value
                        if lbmax > sr["lbmax"]:
                            sr["lbmax"] = lbmax
                        sr["pending"] -= 1
                        if sr["pending"] == 0:
                            self._finish_side(sr)
                if all(x["pending"] == 0 for x in rec["sides"].values()) and not rec.get("done"):
                    best, best_split, best_tree, min_pruned, child_lb_max = self._finish_candidate(
                        rec, best, best_split, best_tree, min_pruned, child_lb_max, shared_best, lock)
        finally:
            for w in workers:
            ...
            for w in workers:
                w.join(timeout=1.0)
        self.iterations += extra_iterations
        root.ub = min(root.ub, best)
        if best_tree is not None:
            self.parallel_tree = best_tree
            root.split = best_split
            root.pending = None
        if failure or remaining > 0:
            raise TimeLimitReached(failure or "time")
        if best <= budget + EPS:
            root.lb = best
            root.solved = True
        else:
            root.lb = max(root.lb, min(best, min_pruned), child_lb_max)

    def _finish_candidate(self, rec, best, best_split, best_tree, min_pruned, child_lb_max, shared_best, lock):
        """Both sides of a heavy root candidate are in: its value if both are solved (an
        achievable tree), else its lower bound; returns the updated root accumulators."""
        rec["done"] = True
        L, R = rec["sides"]["L"], rec["sides"]["R"]
        if L["solved"] and R["solved"] and L["tree"] is not None and R["tree"] is not None:
            value = L["ub"] + R["ub"]
            if value < best - EPS:
                best = value
                best_split = rec["j"]
                best_tree = {"feature": rec["j"], "true": L["tree"], "false": R["tree"]}
                with lock:
                    if best < shared_best.value:
                        shared_best.value = best
        else:
            ...
            if slb < min_pruned:
                min_pruned = slb
        for x in (L, R):
            if x["lb"] > child_lb_max:
                child_lb_max = x["lb"]
        return best, best_split, best_tree, min_pruned, child_lb_max

    @staticmethod
    def _finish_side(sr):
        """All split tasks of a heavy child are in: its value is the best of them (or the
        head's incumbent), its lower bound as in the sequential epilogue."""
        best = sr["best"]
        sr["ub"] = min(sr["ub"], best)
        sr["lb"] = max(sr["lb"], min(best, sr["min_pruned"]), sr["lbmax"])
        # solved relative to its budget: every split is solved or proven above an incumbent
        # that is >= the final one (the parent only uses the value when both sides solved)
        sr["solved"] = sr["ub"] <= sr["budget"] + EPS and sr["tree"] is not None
        sr["kind"] = "resolved"

    def _solve_split(self, node, st, i, shared_best, budget_node, lb_other):
        """Solve split ``i`` of ``node`` (state ``st`` from _head) exactly as the sequential
        loop does; the bound is min(budget_node, shared incumbent - lb_other).  Returns
        ("solved", value, child_lb_max, raw tree) or ("pruned", split lb, child_lb_max, None)."""
        (feats, l_leaf, l_lb, l_solved, l_pred, r_leaf, r_lb, r_solved, r_pred, l_ub2, r_ub2,
         j_l, j_r, d3_arg, order, _, _, _, _, _, _) = st
        data = self.data
        F = data.features
        key = node.key
        memo = self.memo
        ...
        d3 = d3_arg is not None
        if d3:
            arg3, kind_l, kind_r = d3_arg
        bound = min(budget_node, shared_best.value - lb_other)
        j = int(feats[i])
        lkey = key & F[j]
        rkey = key ^ lkey
        ln = memo.get(lkey)
        ...
        if rn is None:
            ...
            rlb, rub = rn.lb, rn.ub
        child_lb_max = max(llb, rlb)
        sub = lub + rub
        if sub < bound - EPS:
            bound = sub
        slb = llb + rlb
        if slb > bound + EPS:
            return "pruned", slb, child_lb_max, None
        if ln is None:
            ln = self._child_node(lkey, lkey.bit_count(), lub, llb, bool(l_solved[i]), int(l_pred[i]))
        if rn is None:
            rn = self._child_node(rkey, rkey.bit_count(), rub, rlb, bool(r_solved[i]), int(r_pred[i]))
        if l_ub2 is not None:
            for cn, ub2, jj, side in ((ln, float(l_ub2[i]), int(j_l[i]), True), (rn, float(r_ub2[i]), int(j_r[i]), False)):
                if ub2 < cn.ub - EPS:
                    if d3:
        ...
        if l_ub2 is not None:
            exact_below = 4 * lam if d3 else 3 * lam
        ...
        while look_ahead and not first.solved:
            bound = min(bound, shared_best.value - lb_other)
            bf = bound - second.lb
            if first.lb > bf + EPS:
                pruned = True
                break
        ...
        child_lb_max = max(child_lb_max, first.lb, second.lb)
        if pruned or first.lb > bound - second.lb + EPS:
            return "pruned", first.lb + second.lb, child_lb_max, None
        bound = min(bound, shared_best.value - lb_other)
        self._solve(second, bound - first.ub if look_ahead else bound, feats)
        child_lb_max = max(child_lb_max, second.lb)
        if second.lb > bound - first.ub + EPS:
            return "pruned", first.ub + second.lb, child_lb_max, None
        value = first.ub + second.ub
        tree = {"feature": j, "true": self.extract(ln), "false": self.extract(rn)}
        return "solved", value, child_lb_max, tree
    ...
    # ---------------------------------------------------------------- solve
    def _head(self, node: Node, budget: float, features: np.ndarray):
        """The expansion of ``_solve`` up to its candidate loop (parallel path): returns None
        when the node is resolved by the kernel stages, else the loop's state."""
        data = self.data
        F = data.features
        key = node.key
        memo = self.memo
        ...
        if best < node.ub - EPS:
            # the incumbent found by the kernel stages is materialised in the memo
            node.ub = best
            if best_split != node.split:
                node.pending = None
            node.split = best_split
        d3_arg = (arg3, kind_l, kind_r) if d3 is not None else None
        return (feats, l_leaf, l_lb, l_solved, l_pred, r_leaf, r_lb, r_solved, r_pred, l_ub2, r_ub2,
                j_l if l_ub2 is not None else None, j_r if l_ub2 is not None else None, d3_arg,
                order, best, best_split, bound, budget, min_pruned, child_lb_max)
...
def _parallel_worker(opt, root, features, budget, shared_best, lock, tasks, results, memory_limit):
    """Worker process body: pull tasks until the sentinel, report each outcome."""
    opt.memory_limit = memory_limit
    opt.iterations = 0
    failure = ""
    heads = {}

    def head_of(node, budget_node):
        st = heads.get(node.key)
        if st is None:
            st = opt._head(node, budget_node, features)
            heads[node.key] = st if st is not None else False
        return st
    ...
    try:
        while True:
            ...
            kind = t[0]
            if kind == "root":
                st = head_of(root, budget)
                if st is False or st is None:
                    results.put(("root", t[1], "pruned", root.lb, 0.0, None))
                    continue
                outcome, value, lbmax, tree = opt._solve_split(root, st, t[1], shared_best, budget, 0.0)
                if outcome == "solved" and tree is not None:
                    with lock:
                        if value < shared_best.value:
                            shared_best.value = value
                results.put(("root", t[1], outcome, float(value), float(lbmax), tree))
            elif kind == "node":
                _, i, side, key, budget_node, lb_other = t
                node = opt.memo[key]
                opt._solve(node, min(budget_node, shared_best.value - lb_other), features)
                tree = opt.extract(node) if node.solved else None
                results.put(("node", i, side, float(node.lb), float(node.ub), bool(node.solved), tree))
            else:
                _, i, side, key, budget_node, lb_other, jj = t
                node = opt.memo[key]
                st = head_of(node, budget_node)
                if st is False:
                    tree = opt.extract(node) if node.solved else None
                    results.put(("node", i, side, float(node.lb), float(node.ub), bool(node.solved), tree))
                    continue
                outcome, value, lbmax, tree = opt._solve_split(node, st, jj, shared_best, budget_node, lb_other)
                # (a child's value plus its sibling's bound is not an achievable root value:
                # only the parent lowers the shared incumbent, from complete trees)
                results.put(("split", i, side, jj, outcome, float(value), float(lbmax), tree))
    except TimeLimitReached as exc:
        failure = str(exc)
    except Exception as exc:  # report and stop rather than hang the parent
        failure = f"worker error: {exc!r}"
    results.put(("done", opt.iterations, failure))
    results.close()
    results.join_thread()
...
class GOSDTClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, regularization: float = 0.05, time_limit: float = 0.0,
                 ...
                 verbose: bool = False, n_jobs: int = 1, parallel_after: float = 0.05,
                 force_parallel: bool = False):
        self.n_jobs = n_jobs
    ...
    def fit(self, X, y, target_name: str = "class"):
        ...
        raw = opt.parallel_tree if opt.parallel_tree is not None else opt.extract(root)
...
def make_model(regularization: float, time_limit: float):
    return OptimalTreeClassifier(regularization=regularization, time_limit=time_limit,
                                 memory_limit=6 * (1 << 30), n_jobs=N_JOBS)
