...
MODEL_NAME = "v6_dedup_thresholds"
DESCRIPTION = ("v5 + duplicate-split removal inside the node kernel: consecutive thresholds of one "
               "numeric column whose captured class counts are identical induce the same split "
               "(nested sets of equal size are equal), so only the first is kept as a candidate")
