...
MULTICORE = True
DESCRIPTION = ("v28 (compiled search) with the root-parallel phase of v25 on 8 cores: after 50 ms of "
               "sequential search the root's surviving candidate splits are handed to 8 forked workers, "
               "each running the compiled search on one split at a time with the shared incumbent as its "
               "bound (only ever decreasing); a split is solved exactly or proven above an incumbent that "
               "is >= the final one, so the root is certified as in the sequential loop")
...
FI_NODE, FI_PHASE, FI_NV, FI_NKEEP, FI_OI, FI_II, FI_LN, FI_RN, FI_FIRST, FI_SECOND, FI_PRUNED, FI_HAVE_D2, FI_HAVE_D3, FI_SIM, FI_BEST_SPLIT, FI_PARENT_NV = range(16)
# float frame fields
FF_BUDGET, FF_BEST, FF_BOUND, FF_MINPR, FF_LBMAX, FF_STEP, FF_BF, FF_BS, FF_EXACT_BELOW, FF_LEAF, FF_OUT_KIND, FF_OUT_VALUE = range(12)
# workspace tuple indices
WS_FI, WS_FF, WS_IO, WS_FO, WS_BO, WS_L, WS_DIST, WS_GIDX, WS_LBARR, WS_ARG3, WS_F3, WS_K3, WS_KW, WS_KW2, WS_ROOTFEATS = range(15)


def make_workspace(m, K, W):
    return (np.zeros((MAXD, 16), dtype=np.int64), np.zeros((MAXD, 12)), np.zeros((MAXD, 7, m), dtype=np.int64),
            np.zeros((MAXD, 14, m)), np.zeros((MAXD, 2, m), dtype=np.bool_), np.zeros((MAXD, m, K)),
            np.zeros((MAXD, K + 1)), np.zeros((MAXD, m), dtype=np.int64), np.zeros((MAXD, m)),
            np.zeros((MAXD, m, 2, 6), dtype=np.int64), np.zeros((MAXD, 4, m)), np.zeros((MAXD, 2, m), dtype=np.int64),
...
def _frame_feats(ws, d):
    ...
    pn = ws[WS_FI][d - 1, FI_NV]
    return ws[WS_IO][d - 1, 0, :pn]
...
def _expand_frame(st, dat, pf, pi, ws, d):
    ...
    lb_arr = ws[WS_LBARR][d, :nv]
    for t in range(nv):
        lb_arr[t] = split_lb[t]
    FI[d, FI_NKEEP] = n_keep
    FI[d, FI_OI] = 0
    FI[d, FI_HAVE_D2] = 1 if ran else 0
    FI[d, FI_HAVE_D3] = 1 if have_d3 else 0
    FI[d, FI_SIM] = 1 if sim else 0
    FI[d, FI_BEST_SPLIT] = best_split
    FI[d, FI_LN] = -1
    FI[d, FI_RN] = -1
    FF[d, FF_BEST] = best
    FF[d, FF_BOUND] = bound
    FF[d, FF_MINPR] = min_pruned
    FF[d, FF_LBMAX] = max_pair(l_lb, r_lb)
    FF[d, FF_EXACT_BELOW] = 4.0 * lam if have_d3 else 3.0 * lam
    return False
...
def _finish_frame(st, ws, d):
    ...
    node = FI[d, FI_NODE]
    best = FF[d, FF_BEST]
    best_split = FI[d, FI_BEST_SPLIT]
    if best_split != st[ST_SPLIT][node]:
        st[ST_PEND][node, 0] = 0
    st[ST_UB][node] = best
    st[ST_SPLIT][node] = best_split
    if best <= FF[d, FF_BUDGET] + EPS:
        st[ST_LB][node] = best
        st[ST_SOLVED][node] = 1
    else:
        v = max(min(best, FF[d, FF_MINPR]), FF[d, FF_LBMAX])
        if v > st[ST_LB][node]:
            st[ST_LB][node] = v
...
def _flush_frames(st, ws, d):
    ...
    for k in range(d + 1):
        ...
        if best < st[ST_UB][node] - EPS:
            st[ST_UB][node] = best
            if FI[k, FI_BEST_SPLIT] != st[ST_SPLIT][node]:
                st[ST_PEND][node, 0] = 0
            st[ST_SPLIT][node] = FI[k, FI_BEST_SPLIT]


@njit(cache=NUMBA_CACHE, nogil=True)
def solve_iter(st, dat, pf, pi, ws, root, budget, only, shared):
    ...
    while d >= 0:
        node = FI[d, FI_NODE]
        phase = FI[d, FI_PHASE]
        if d == 0 and only >= 0 and shared[0] < FF[0, FF_BOUND]:
            FF[0, FF_BOUND] = shared[0]
            if shared[0] < FF[0, FF_BEST]:
                FF[0, FF_BEST] = shared[0]
        if phase == 0:
            # entry of _solve
            if nsolved[node] == 1 or nlb[node] > FF[d, FF_BUDGET] + EPS:
                d -= 1
                continue
            if meta[1] >= meta[3]:
                meta[2] = 1
                ...
                return
            meta[1] += 1
            if _expand_frame(st, dat, pf, pi, ws, d):
                if meta[2] != 0:
                    _flush_frames(st, ws, d)
                    return
                d -= 1
            ...
            continue
        nv = FI[d, FI_NV]
        io = ws[WS_IO][d]; fo = ws[WS_FO][d]; bo = ws[WS_BO][d]
        feats = io[0, :nv]; order_buf = io[1, :nv]; j_l = io[2, :nv]; j_r = io[3, :nv]
        l_pred = io[4, :nv]; r_pred = io[5, :nv]
        l_leaf = fo[0, :nv]; l_lb = fo[1, :nv]; r_leaf = fo[2, :nv]; r_lb = fo[3, :nv]; l_pot = fo[4, :nv]
        split_lb = fo[5, :nv]
        l_solved = bo[0, :nv]; r_solved = bo[1, :nv]
        f3 = ws[WS_F3][d]; k3 = ws[WS_K3][d]; arg3 = ws[WS_ARG3][d]
        gidx = ws[WS_GIDX][d, :nv]; lb_arr = ws[WS_LBARR][d, :nv]
        have_d2 = FI[d, FI_HAVE_D2] == 1
        have_d3 = FI[d, FI_HAVE_D3] == 1
        sim = FI[d, FI_SIM] == 1
        bound = FF[d, FF_BOUND]
        budget_f = FF[d, FF_BUDGET]
        first = FI[d, FI_FIRST]; second = FI[d, FI_SECOND]
        if phase == 1:
            # next candidate
            oi = FI[d, FI_OI]
            if oi >= FI[d, FI_NKEEP]:
            ...
            sub = lub + rub
            if sub < FF[d, FF_BEST] - EPS:
                FF[d, FF_BEST] = sub
                FI[d, FI_BEST_SPLIT] = f
            ...
            if have_d2:
                ...
                for side in range(2):
                    ...
                    if nsolved[cn] == 0 and bound - nlb[other] < eb - EPS and nub[cn] <= bound - nlb[other] + EPS:
                        nlb[cn] = nub[cn]
                        nsolved[cn] = 1
            FI[d, FI_II] = ii; FI[d, FI_LN] = ln; FI[d, FI_RN] = rn
            FI[d, FI_FIRST] = first; FI[d, FI_SECOND] = second
            FI[d, FI_PRUNED] = 0
            FF[d, FF_STEP] = 2.0 * lam
            FI[d, FI_PHASE] = 2
            continue
        ii = FI[d, FI_II]
        if phase == 2:
            ...
            if look_ahead and nsolved[first] == 0:
                ...
                if d + 1 >= MAXD:
                    ...
                    _flush_frames(st, ws, d)
                    return
                FI[d + 1, FI_NODE] = first; FF[d + 1, FF_BUDGET] = min(bf, nlb[first] + FF[d, FF_STEP]); FI[d + 1, FI_PHASE] = 0
                d += 1
                continue
            FI[d, FI_PHASE] = 5
            continue
        if phase == 3:
            ...
            if nsolved[second] == 0:
                ...
                if d + 1 >= MAXD:
                    ...
                    _flush_frames(st, ws, d)
                    return
                FI[d + 1, FI_NODE] = second; FF[d + 1, FF_BUDGET] = min(bs, nlb[second] + FF[d, FF_STEP]); FI[d + 1, FI_PHASE] = 0
                d += 1
                continue
            FF[d, FF_STEP] = FF[d, FF_STEP] * 2.0
            FI[d, FI_PHASE] = 2
            continue
        if phase == 4:
            bs = FF[d, FF_BS]
        ...
        if phase == 5:
            ...
            if d + 1 >= MAXD:
                ...
                _flush_frames(st, ws, d)
                return
            FI[d + 1, FI_NODE] = first
            FF[d + 1, FF_BUDGET] = bound - nlb[second] if look_ahead else bound
            FI[d + 1, FI_PHASE] = 0
            d += 1
            continue
        if phase == 6 or phase == 7:
            if nlb[first] > FF[d, FF_LBMAX]:
                FF[d, FF_LBMAX] = nlb[first]
            if nlb[second] > FF[d, FF_LBMAX]:
                FF[d, FF_LBMAX] = nlb[second]
            ...
            if d + 1 >= MAXD:
                ...
                _flush_frames(st, ws, d)
                return
            FI[d + 1, FI_NODE] = second
            FF[d + 1, FF_BUDGET] = bound - nub[first] if look_ahead else bound
            FI[d + 1, FI_PHASE] = 0
            d += 1
            continue
        if phase == 8:
            ...
            if value < FF[d, FF_BEST] - EPS:
                FF[d, FF_BEST] = value
                FI[d, FI_BEST_SPLIT] = feats[ii]
                FF[d, FF_BOUND] = min(budget_f, value)
            elif sim:
...
class CompiledOptimizer:
    ...
    def __init__(self, data: BitDataset, regularization: float, *, groups=None, time_limit=0.0,
                 ...
                 n_jobs=1, parallel_after=0.05, force_parallel=False):
        self.data = data
        self.lam = float(regularization)
        self.time_limit = float(time_limit)
        self.memory_limit = int(memory_limit)
    ...
    def _alloc(self, cap, old=None):
        W = self.data.W
        st = (np.zeros((cap, W), dtype=np.uint64), np.full(2 * cap, -1, dtype=np.int64), np.zeros(cap, dtype=np.int64),
    ...
    def _run_parallel(self, root, budget):
        """Root-parallel phase on the compiled engine (see DESCRIPTION)."""
        ctx = multiprocessing.get_context("fork")
        st = self.st; ws = self.ws; meta = st[ST_META]
        FI = ws[WS_FI]; FF = ws[WS_FF]
        # the root frame: expanded here so the workers inherit it
        FI[0, FI_NODE] = root; FF[0, FF_BUDGET] = float(budget); FI[0, FI_PHASE] = 0
        meta[2] = 0; meta[3] = meta[1] + 10 ** 9
        if _expand_frame(st, self.dat, self.pf, self.pi, ws, 0):
            if meta[2] == 2:
                self._alloc(st[ST_KEYS].shape[0] * 2, st)
        ...
        # the frame's incumbent (materialised by the kernel stages) belongs to the node now:
        # if no worker improves on it, this is the tree that is returned
        ...
        shared_raw = ctx.RawArray("d", 1)
        shared = np.frombuffer(shared_raw, dtype=np.float64)
        shared[0] = best
        lock = ctx.Lock()
        tasks = ctx.Queue(); results = ctx.Queue()
        # (forked workers share the parent's pages copy-on-write; a worker's resident size
        # overcounts, so each worker gets half the cap rather than a 1/n_jobs share)
        per_worker_memory = self.memory_limit // 2 if self.memory_limit > 0 else 0
        deadline = self.start_time + self.time_limit if self.time_limit > 0.0 else float("inf")
        workers = [ctx.Process(target=_compiled_worker, args=(self, shared, lock, tasks, results, per_worker_memory, deadline),
                               daemon=True) for _ in range(self.n_jobs)]
        for w in workers:
            w.start()
        ...
        for _ in range(self.n_jobs):
            tasks.put(-1)
        alive = self.n_jobs; done = 0; failure = ""; best_tree = None; extra = 0
        try:
            while alive > 0:
                ...
                if msg[0] == "done":
                    ...
                    continue
                _, pos, kind, value, lbmax, tree = msg
                done += 1
                if kind == 1:
                    if tree is not None and value < best - EPS:
                        best = value; best_split = int(tree["feature"]); best_tree = tree
                else:
                    if value < min_pruned:
                        min_pruned = value
    ...
    try:
        while True:
            ...
            while True:
                ...
                t0 = time.perf_counter()
                solve_iter(st, opt.dat, opt.pf, opt.pi, ws, np.int64(root), budget, np.int64(pos), shared)
                dt = time.perf_counter() - t0
                if meta[2] == 0:
                    break
                if meta[2] == 2:
