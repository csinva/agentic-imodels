...
MULTICORE = True
DESCRIPTION = ('v46 with the preprocessing made fast for wide tables (the search is unchanged): the binary encoder applies one broadcast comparison per numeric column and integer codes per categorical column instead of one pandas operation (or a Python loop) per rule, and the equivalent-points classes are found by hashing packed rows instead of sorting them.  Identical matrices, rules, groups and dataset objects on 17 data sets; full COMPAS (7214 rows, 26401 binary features) binarises in 2 s instead of 82 s and fits in 5 s instead of 86 s.  Exact; the gosdt-guesses options of v47 (threshold guessing, reference lower-bound guess, depth budget) are available but off')
...
def expand_kernel(F, features, group_of, kw, mask_matrix, weights, costs, diff, lam, bound, do_exchange, pfb,
    ...
    if pair_cap > 0.0 and float(nv) * float(nv) * float(K) * float(W) > pair_cap:
        ...
        if self.greedy_init and GREEDY_SEED_NODES > 0:
            ...
            for mode in (SEED_MODES if short_cap else SEED_MODES[:1]):
                ...
                while todo > 0 and self.seed_nodes - nodes0 < GREEDY_SEED_NODES:
                    meta[2] = 0
                    self.seed_nodes += int(greedy_seed(st, self.dat, self.pf, self.pi, self.ws, np.int64(root), np.int64(24),
