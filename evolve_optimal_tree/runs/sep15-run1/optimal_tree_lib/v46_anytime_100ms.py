...
def warm_up():
    ...
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), int_to_words(1, 1), masks, np.zeros(0),
                  np.zeros((1, 1)), np.zeros(1), 0.1, 1.0, False, np.zeros(5), np.empty((7, 1), dtype=np.int64), np.empty((14, 1)),
...
class Optimizer:
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, M, Fc = expand_kernel(
            ...
            self.continuous_feature_exchange and self._has_groups, np.zeros(5), io, fo, bo, L, dist)
        if nv == 0:
            return None
        nv = int(nv)
        if nv < mf:
...
PF_LAM, PF_UW, PF_N, PF_PAIRCAP, PF_PARTIAL = range(5)
...
class CompiledOptimizer:
    ...
    def __init__(self, data: BitDataset, regularization: float, *, groups=None, time_limit=0.0,
        ...
        self.pf = np.array([self.lam, uniform_w, float(data.n), pair_cap, partial])
...
def _compile_search():
    ...
    nv, n_cand, i0, mr, ran, i_d2, bd2, lbg, M, Fc = expand_kernel(dat[DT_F], feats, dat[DT_GROUP], st[ST_KEYS][root],
                                                                 ...
                                                                 0.1, 1.0, False, np.zeros(5), io, fo, bo, L, dist)
...
TIME_CAP = 0.100   # anytime rows: cap the time limit (seconds, 0 = none)
