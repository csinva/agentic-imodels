...
MODEL_NAME = "v32_shared_bound_table"
...
MULTICORE = True
DESCRIPTION = ("v31 + a lock-free lower-bound table shared by the 8 threads: each thread publishes the "
               "bounds it proves for subproblems with >= 1/16 of the rows in its own region of a table in "
               "shared memory (keys written once, bounds only raised), and every thread looks a node up in "
               "all regions before expanding it, so a subproblem another thread already pruned is pruned "
               "without being re-expanded.  Only lower bounds are shared, so certification is unchanged")
...
SH_KEYS, SH_COUNTS, SH_LBS, SH_USED, SH_META = range(5)   # meta: [T, C, min_count]


def make_shared_table(T, C, W, min_count):
    return (np.zeros((T, C, W), dtype=np.uint64), np.zeros((T, C), dtype=np.int64), np.zeros((T, C)),
            np.zeros((T, C), dtype=np.uint8), np.array([T, C, min_count], dtype=np.int64))
...
def sh_lookup(sh, kw, count):
    """Largest bound any thread published for the capture ``kw`` (-1 if none).  Lock-free:
    keys are written once (a partially written key can only coincide with a strict subset,
    which has a different row count) and bounds only ever increase."""
    keys = sh[SH_KEYS]; counts = sh[SH_COUNTS]; lbs = sh[SH_LBS]; used = sh[SH_USED]
...
@njit(cache=NUMBA_CACHE, nogil=True)
def sh_publish(sh, tid, kw, count, lb):
    """Record a proven bound in this thread's region (the only writer of that region)."""
    keys = sh[SH_KEYS]; counts = sh[SH_COUNTS]; lbs = sh[SH_LBS]; used = sh[SH_USED]
    C = sh[SH_META][1]
    W = kw.shape[0]
    hmask = np.int64(C - 1)
...
def solve_iter(st, dat, pf, pi, ws, root, budget, only, shared, sh, tid):
    ...
    while d >= 0:
        ...
        if phase == 0:
            ...
            if sh_big:
                # a bound another thread proved for this subproblem
                v = sh_lookup(sh, st[ST_KEYS][node], st[ST_COUNT][node])
                if v > nlb[node]:
                    nlb[node] = v
            ...
            if FI[ps, FI_VALID] == 1 and FI[ps, FI_CACHED_NODE] == node:
                ...
                if _rearm_frame(st, dat, pf, pi, ws, d):
                    if sh_big and nlb[node] > FF[ps, FF_LB0] + 0.25 * lam:
                        sh_publish(sh, tid, st[ST_KEYS][node], st[ST_COUNT][node], nlb[node])
                    d -= 1
                    continue
        ...
        if phase == 1:
            ...
            if oi >= FI[ps, FI_NKEEP]:
                ...
                if sh[SH_META][0] > 0 and st[ST_COUNT][node] >= sh[SH_META][2] and nlb[node] > FF[ps, FF_LB0] + 0.25 * lam:
                    sh_publish(sh, tid, st[ST_KEYS][node], st[ST_COUNT][node], nlb[node])
                d -= 1
                continue
            FI[ps, FI_OI] = oi + 1
...
class CompiledOptimizer:
    ...
    def _run_parallel(self, root, budget):
        ...
        sh = make_shared_table(n_threads, 1 << 13, self.data.W, max(2, self.data.n // 16))
...
def _compile_search():
    ...
    sh_publish(sh, np.int64(0), st[ST_KEYS][root], int(st[ST_COUNT][root]), 0.0)
