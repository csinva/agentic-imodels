...
MODEL_NAME = "v19_depth3_triples"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v18 + an exact depth-3 stage on narrow nodes: the class counts of every triple of "
               "candidate splits give, for every child, the exact best 3-leaf tree and the exact best "
               "(2,2) tree, so each child's achievable risk is min over 1, 2, 3 and (2,2)-leaf trees and "
               "its lower bound min(that, 4 lambda + cheapest peeled cell, 5 lambda + cheapest peeled "
               "cell or 2-leaf cell, 6 lambda); the node is solved without expanding any child whenever "
               "the best such tree is no worse than that bound (every budget below 5 lambda), and the "
               "probes that only existed to learn a child's 3-leaf value disappear")
...
@njit(cache=True, nogil=True)
def node_stats(F, feats, group_of, masks, weights, costs, diff, lam,
               out_feats, out_L, out_l_leaf, out_l_lb, out_l_solved, out_r_leaf, out_r_lb,
               out_r_solved, out_l_pot, out_dist, out_l_pred, out_r_pred):
...
@njit(cache=True, nogil=True)
def expand_kernel(F, features, group_of, M, weights, costs, diff, lam, bound, do_exchange,
                  io, fo, bo, L, dist):
    ...
    K = dist.shape[0] - 1
    W = F.shape[1]
    nv = node_stats(F, features, group_of, M, weights, costs, diff, lam, io[0], L, fo[0], fo[1], bo[0],
                    fo[2], fo[3], bo[1], fo[4], dist, io[4], io[5])
    if nv == 0:
        return 0, 0, 0, 0.0, False, 0, 0.0, 0.0
    feats = io[0, :nv]
    ...
    if single or not (n_cand >= 2 and nv >= 2 and (cheap or (n_cand * 2 >= nv and n_cand >= 8 and nv >= 8))):
        return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0
    depth2_pairs(F, feats, group_of, M[:K], costs, lam, dist[:K], L[:nv], fo[7, :nv], fo[8, :nv], io[2, :nv],
    ...
    return nv, n_cand, best_i, min_rejected, True, i_d2, best_d2, lb_ge4
...
@njit(cache=True, nogil=True)
def depth3_triples(F, feats, masks, costs, uniform_w, lam, dist, L, out_val, out_arg):
    ...
    ``out_val[i, io] = (best 3-leaf loss, best (2,2) loss, cheapest 2-leaf cell loss)`` and
    ...
    idx3 = np.empty(3, dtype=np.int64)
    bits = np.empty(3, dtype=np.int64)
    # the twelve (child, side, split, sub-split) role assignments of a triple's positions,
    # each with its four crossing cells (A and t, A not t, B and t, B not t)
    perms = np.array([[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]], dtype=np.int64)
    c_r = np.empty(12, dtype=np.int64)
    c_s = np.empty(12, dtype=np.int64)
    c_t = np.empty(12, dtype=np.int64)
    c_io = np.empty(12, dtype=np.int64)
    c_a1 = np.empty(12, dtype=np.int64)
    c_a0 = np.empty(12, dtype=np.int64)
    c_b1 = np.empty(12, dtype=np.int64)
    c_b0 = np.empty(12, dtype=np.int64)
    q = 0
    for p in range(6):
        pr = perms[p, 0]
        ps = perms[p, 1]
        ...
        for io in range(2):
            ...
            bits[ps] = 1
            bits[pt] = 1
            c_a1[q] = bits[0] * 4 + bits[1] * 2 + bits[2]
            bits[pt] = 0
            c_a0[q] = bits[0] * 4 + bits[1] * 2 + bits[2]
            bits[ps] = 0
            bits[pt] = 1
            c_b1[q] = bits[0] * 4 + bits[1] * 2 + bits[2]
            bits[pt] = 0
            c_b0[q] = bits[0] * 4 + bits[1] * 2 + bits[2]
            q += 1
    for i in range(nv):
        fi = feats[i]
        for j in range(i + 1, nv):
...
def depth3_bounds(l_leaf, ml2_l, c3_l, r_leaf, ml2_r, c3_r, val, lam, l_ub3, r_ub3, l_lb3, r_lb3, kind_l, kind_r):
    ...
    for i in range(mf):
        for side in range(2):
            ...
            if g4 < ub:
                ub = g4; kind = 3
            lb = min(min(ub, 4.0 * lam + c3), min(5.0 * lam + min(c3, c5), 6.0 * lam))
            if side == 1:
...
def warm_up():
    ...
    node_stats(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
               np.zeros(1), 0.1, np.empty(1, dtype=np.int64), np.empty((1, 1)), np.empty(1), np.empty(1),
               np.empty(1, dtype=np.bool_), np.empty(1), np.empty(1), np.empty(1, dtype=np.bool_), np.empty(1),
               np.empty(2), np.empty(1, dtype=np.int64), np.empty(1, dtype=np.int64))
    ...
    e = np.zeros(1)
    depth3_triples(F, np.zeros(1, dtype=np.int64), masks, np.zeros((1, 1)), 0.0, 0.1, np.zeros(1), np.zeros((1, 1)),
                   np.empty((1, 2, 3)), np.empty((1, 2, 6), dtype=np.int64))
    depth3_bounds(e, e, e, e, e, e, np.zeros((1, 2, 3)), 0.1, np.empty(1), np.empty(1), np.empty(1), np.empty(1),
                  np.empty(1, dtype=np.int64), np.empty(1, dtype=np.int64))
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
                  np.zeros(1), 0.1, 1.0, False, np.empty((6, 1), dtype=np.int64), np.empty((14, 1)),
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        if self.engine == "numba":
            ...
            if data.zero_diagonal and data.equal_mismatch:
                ...
                nv = node_stats(data.F_words, features, self.group_of, M, data.weights1, data.costs,
                                self._diff, lam, *buf)
                if nv == 0:
                    return None
            ...
            self._last_class_masks = M[:K]
            buf = self._buffers(features.shape[0])
            nv = node_stats(data.F_words, features, self.group_of, M, np.array(weights), data.costs,
                            self._diff, lam, *buf)
            if nv == 0:
                return None
            feats, L, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot = (x[:nv] for x in buf[:9])
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        if data.zero_diagonal and data.equal_mismatch:
            ...
            weights = np.array(ws)
        mf = features.shape[0]
        io = np.empty((6, mf), dtype=np.int64)
        fo = np.empty((14, mf))
        bo = np.empty((2, mf), dtype=np.bool_)
        L = np.empty((mf, K))
        dist = np.empty(K + 1)
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4 = expand_kernel(
            data.F_words, features, self.group_of, M, weights, data.costs, self._diff, self.lam, bound,
        ...
        if ran and nv <= TRIPLE_MAX_NV and K * data.W <= D3_MAX_KW and nv * nv * nv / 6.0 * (K * data.W + 150.0) <= TRIPLE_MAX_OPS:
            pij = (M[:K], float(dist[K]))     # depth-3 stage available; computed on demand by _depth3
        dist = dist[:K]
    ...
    def _depth3(self, node: Node, io, fo, L, dist, masks):
        ...
        depth3_triples(data.F_words, io[0], masks, data.costs, self._uniform_w, self.lam, dist[:K], L, val, arg)
        f3 = np.empty((4, nv))
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if self.engine == "numba":
            ...
            d3_avail = pij is not None and len(pij) == 2
        ...
        if d2 is not None:
            ...
            if (d3 is None and d3_avail and budget < pij[1] + D3_MAX_LEAVES * lam - EPS
                ...
                d3 = self._depth3(node, io, fo, L, dist, pij[0])
            if d3 is not None:
