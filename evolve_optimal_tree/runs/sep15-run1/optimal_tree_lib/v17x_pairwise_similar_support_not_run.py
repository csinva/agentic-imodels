...
MODEL_NAME = "v17_pairwise_similar_support"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v16 + similar-support propagation between any two candidate splits: with R' the "
               "objective shifted by each point's minimum cost (a per-node constant), a tree for split j "
               "serves split i after moving the points of the symmetric difference of their left "
               "children, so lb(i) >= lb(j) - pot(L_i xor L_j) = lb(j) - pot(L_i) - pot(L_j) + 2 pot(L_i and L_j); "
               "the pairwise potentials come from the pairwise kernel, so every proven bound on one "
               "split now lowers-bounds every other candidate, not only the thresholds of its column")
...
def node_stats(F, feats, group_of, masks, weights, costs, diff, lam,
               ...
               out_r_solved, out_l_pot, out_dist):
...
@njit(cache=True, nogil=True)
def depth2_pairs(F, feats, group_of, masks, costs, lam, dist, L, diff, out_ml2_l, out_ml2_r, out_j_l, out_j_r,
                 out_c3_l, out_c3_r, out_pij):
...
# widest node (candidate splits) for which the pairwise potential matrix is stored
PIJ_MAX = 400
...
def expand_kernel(F, features, group_of, M, weights, costs, diff, lam, bound, do_exchange,
    ...
    nv = node_stats(F, features, group_of, M, weights, costs, diff, lam, io[0], L, fo[0], fo[1], bo[0],
                    fo[2], fo[3], bo[1], fo[4], dist)
    if nv == 0:
        return 0, 0, 0, 0.0, False, 0, 0.0, 0.0, np.empty((0, 0))
    feats = io[0, :nv]
    ...
    if single or not (n_cand >= 2 and nv >= 2 and (cheap or (n_cand * 2 >= nv and n_cand >= 8 and nv >= 8))):
        return nv, n_cand, best_i, min_rejected, False, 0, 0.0, 0.0, np.empty((0, 0))
    if nv <= PIJ_MAX:
        pij = np.empty((nv, nv))
    else:
        pij = np.empty((0, 0))
    depth2_pairs(F, feats, group_of, M[:K], costs, lam, dist, L[:nv], diff, fo[7, :nv], fo[8, :nv], io[2, :nv],
                 io[3, :nv], fo[9, :nv], fo[10, :nv], pij)
    ...
    return nv, n_cand, best_i, min_rejected, True, i_d2, best_d2, lb_ge4, pij
...
def warm_up():
    ...
    depth2_pairs(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros((1, 1)), 0.1,
                 np.zeros(1), np.zeros((1, 1)), np.zeros(1), np.empty(1), np.empty(1), np.empty(1, dtype=np.int64),
                 np.empty(1, dtype=np.int64), np.empty(1), np.empty(1), np.empty((1, 1)))
    e = np.zeros(1)
    expand_kernel(F, np.zeros(1, dtype=np.int64), np.zeros(2, dtype=np.int64), masks, np.zeros(0), np.zeros((1, 1)),
                  np.zeros(1), 0.1, 1.0, False, np.empty((4, 1), dtype=np.int64), np.empty((14, 1)),
                  np.empty((2, 1), dtype=np.bool_), np.empty((1, 1)), np.empty(1))
...
class Optimizer:
    ...
    def _child_statistics(self, node: Node, features: np.ndarray):
        ...
        if self.engine == "numba":
            ...
            if data.zero_diagonal and data.equal_mismatch:
                ...
                return feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, L, buf[9]
    ...
    def _expand(self, node: Node, features: np.ndarray, bound: float):
        ...
        if data.zero_diagonal and data.equal_mismatch:
            ...
            weights = np.array(ws)
        mf = features.shape[0]
        io = np.empty((4, mf), dtype=np.int64)
        fo = np.empty((14, mf))
        bo = np.empty((2, mf), dtype=np.bool_)
        L = np.empty((mf, K))
        dist = np.empty(K)
        nv, n_cand, i, min_rejected, ran, i_d2, best_d2, lb_ge4, pij = expand_kernel(
        ...
        fo = fo[:, :nv]
        bo = bo[:, :nv]
        d2 = (io[2], io[3], fo[11], fo[12], fo[13], int(i_d2), float(best_d2), float(lb_ge4),
              pij if pij.shape[0] == nv else None) if ran else None
        return (io[0], fo[0], fo[1], bo[0], fo[2], fo[3], bo[1], fo[4], L[:nv], dist,
                fo[5], fo[6], io[1], int(n_cand), int(i), float(min_rejected), d2)
    ...
    def _solve(self, node: Node, budget: float, features: np.ndarray) -> None:
        ...
        if self.engine == "numba":
            ...
            if res is None:
                ...
                node.solved = True
                return
            (feats, l_leaf, l_lb, l_solved, r_leaf, r_lb, r_solved, l_pot, L, dist,
             split_lb, split_ub, order_buf, n_cand, i, min_rejected, d2) = res
            mf_ = feats.shape[0]
        ...
        if d2 is not None:
            ...
            j_l, j_r, l_ub2, r_ub2, split_ub2, i, best_d2, lb_ge4, pij = d2
        ...
        # Pairwise generalisation (v17).  For any two splits i, j of this node, a tree for
        # split j becomes a tree for split i by moving the points of L_i xor L_j between
        # the children; with costs shifted so every class's cheapest prediction costs 0
        # (a constant per node, identical for all splits) each moved point costs at most
        # its potential, so lb(i) >= lb(j) - pot(L_i xor L_j) with
        # pot(L_i xor L_j) = pot(L_i) + pot(L_j) - 2 pot(L_i and L_j).
        sim = self.similar_support and (self._has_groups or pij is not None)
        if sim:
            ...
            if pij is not None:
                def propagate(i, v):
                    if v <= bound + EPS:
                        return
                    np.maximum(lb_arr, v - (pot[i] + pot - 2.0 * pij[i]), out=lb_arr)
            else:
                def propagate(i, v):
                    if v <= bound + EPS:
                        return
