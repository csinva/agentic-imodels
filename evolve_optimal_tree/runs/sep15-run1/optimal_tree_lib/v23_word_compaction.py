...
MODEL_NAME = "v23_word_compaction"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v22 + rows sorted lexicographically by the source columns (most informative first) so "
               "deep capture sets occupy few 64-bit words, and an expansion whose mask leaves at least "
               "30% of the words empty first copies the candidate features' words on the non-empty "
               "words only (inside the kernel); all kernels then run their unchanged contiguous loops "
               "on the compact copy.  Same search and bounds; row order does not change the problem")
