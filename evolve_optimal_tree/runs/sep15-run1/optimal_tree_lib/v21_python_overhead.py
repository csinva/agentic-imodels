...
MODEL_NAME = "v21_python_overhead"
EXACT = True  # this solver certifies optimality; set False for a heuristic variant
DESCRIPTION = ("v20 with Python overhead removed, same search and bounds: the memory guard shelled out "
               "to ps every 1024 iterations (4 ms each, 11% of tic-tac-toe) and is now checked at most "
               "twice a second; the depth-2 incumbent's children are created from the kernel's leaf "
               "statistics instead of recomputing them from the big-int masks; the candidate refilter "
               "after the depth-2 stage runs in numba instead of five numpy calls")
