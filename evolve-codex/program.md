# autoresearch — interpretable regressors (Codex edition)
...
5. shorthand name of the model tried - should always be unique (written in interpretable_regressor.py)
6. brief text description of what this experiment tried - make sure to update this clearly (written in interpretable_regressor.py)

## The experiment loop

The experiment runs on a dedicated branch (e.g. `autoresearch/mar25`).

IMPORTANT: You are running inside Codex in non-interactive mode. You will be invoked repeatedly by an external loop script. Each invocation should run ONE experiment iteration:

1. Look at the git state: the current branch/commit we're on
2. Read `results/overall_results.csv` to see what's been tried already and what worked
3. Edit `interpretable_regressor.py` with an experimental idea. It should only contain one class definition
4. git commit
5. Run the experiment: `uv run interpretable_regressor.py > run.log 2>&1`
6. Read results: `tail -n 5 run.log` and `grep InterpretableRegressor results/overall_results.csv`
7. If the run crashed, check `tail -n 50 run.log` for the stack trace and attempt a fix
8. Record results in `results/overall_results.csv` (do not commit this file)
