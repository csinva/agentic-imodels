...
The idea: give an AI agent a classification training setup and let it experiment autonomously. It modifies `train.py`, trains a classifier, checks if AUC improved across TabArena datasets, keeps or discards, and repeats. You wake up in the morning to a log of experiments and (hopefully) a better interpretable model.

## How it works

The repo has three files that matter:

- **`prepare.py`** — fixed constants, one-time data download (fetches TabArena classification datasets from OpenML), and the evaluation harness (`evaluate_auc`). **Not modified by the agent.**
- **`train.py`** — the single file the agent edits. Defines `InterpretableClassifier` (a scikit-learn compatible model), a `model_factory`, and a training + evaluation loop. Everything is fair game: algorithm, hyperparameters, feature engineering, etc. **This file is edited and iterated on by the agent.**
...
# 3. Download and cache all TabArena datasets (one-time, ~2 min)
uv run prepare.py
...
```
prepare.py      — constants, dataset download + evaluation harness (do not modify)
train.py        — interpretable classifier definition (agent modifies this)
