...
```bash
First, read the src/performance_eval.py, results/cached_runs/mar30/performance_results.csv, and the results/cached_runs/mar30/interpretability_vs_performance.png plot. Include the plot then discuss how different variations effected performance/interpretability and why.

Next, read the tests in src/interp_eval.py and the results in results/cached_runs/mar30/interpretability_results.csv. Write a concise report titled results/cached_runs/mar30/results_report.md. It should describe the interp_eval tests and show a detailed example of how a single test is conducted. Then, it should show a table where each row is a test, column gives a short description, next column gives a detailed description, next column gives pass rate, and final columns show which models passed.
