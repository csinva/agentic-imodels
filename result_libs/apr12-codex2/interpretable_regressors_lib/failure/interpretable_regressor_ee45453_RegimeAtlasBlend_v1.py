...
class RegimeAtlasBlendRegressor(BaseEstimator, RegressorMixin):
    """Regime-aware atlas blend: global OOF stack plus sample-wise tree-vs-linear gating."""
    ...
    def __init__(
        ...
        self.reliability_mix = reliability_mix
        self.gate_max_leaf_nodes = gate_max_leaf_nodes
    ...
    @staticmethod
    def _reliability_weights(P, y):
        errs = np.sqrt(np.mean((P - y.reshape(-1, 1)) ** 2, axis=0))
        inv = 1.0 / np.maximum(errs, 1e-6)
        s = float(np.sum(inv))
        if s <= 1e-12:
            return np.ones(P.shape[1], dtype=float) / float(P.shape[1])
        return inv / s
    ...
    def fit(self, X, y):
        ...
        if n >= 40:
            ...
            for fold_id, (tr, va) in enumerate(kf.split(Xf)):
                hgb, rf, et, ridge = self._build_models(seed_offset=41 * fold_id)
                hgb.fit(Xf[tr], y[tr])
                rf.fit(Xf[tr], y[tr])
                et.fit(Xf[tr], y[tr])
        ...
        data_blend = self._fit_nonnegative_blend(P_oof, y)
        rel_blend = self._reliability_weights(P_oof, y)
        mix = float(np.clip(self.reliability_mix, 0.0, 1.0))
        self.global_blend_weights_ = (1.0 - mix) * data_blend + mix * rel_blend
        self.global_blend_weights_ = self.global_blend_weights_ / np.sum(self.global_blend_weights_)

        self.tree_blend_weights_ = self._fit_nonnegative_blend(P_oof[:, :3], y)
        linear_oof = P_oof[:, 3]
        tree_oof = P_oof[:, :3] @ self.tree_blend_weights_
        global_oof = P_oof @ self.global_blend_weights_

        tree_advantage = np.abs(linear_oof - y) - np.abs(tree_oof - y)
        gate_leaf_nodes = int(np.clip(self.gate_max_leaf_nodes, 3, 10))
        gate_min_samples = int(max(8, n // 30))
        self.gate_tree_ = DecisionTreeRegressor(
            max_leaf_nodes=gate_leaf_nodes,
            min_samples_leaf=gate_min_samples,
            random_state=self.random_state,
        )
        self.gate_tree_.fit(Xf, tree_advantage)
        self.gate_bias_ = float(np.median(tree_advantage))
        tree_adv_std = float(np.std(tree_advantage))
        self.gate_scale_ = float(np.clip(self.gate_gain, 0.5, 6.0) / max(tree_adv_std, 1e-3))

        gate_oof = self._sigmoid(self.gate_scale_ * (self.gate_tree_.predict(Xf) - self.gate_bias_))
        regime_oof = gate_oof * tree_oof + (1.0 - gate_oof) * linear_oof
        channel_oof = np.column_stack([global_oof, regime_oof])
        self.channel_weights_ = self._fit_nonnegative_blend(channel_oof, y)
        ...
        global_train_pred = P_full @ self.global_blend_weights_
        tree_train_pred = P_full[:, :3] @ self.tree_blend_weights_
        linear_train_pred = P_full[:, 3]
        gate_full = self._sigmoid(self.gate_scale_ * (self.gate_tree_.predict(Xf) - self.gate_bias_))
        regime_train_pred = gate_full * tree_train_pred + (1.0 - gate_full) * linear_train_pred
        base_train_pred = (
            self.channel_weights_[0] * global_train_pred
            + self.channel_weights_[1] * regime_train_pred
        )
        ...
        self.residual_scale_ = float(np.clip(self.residual_scale, 0.0, 0.35))
        self.gate_usage_ = float(np.mean(gate_full))
        ...
        w = self.global_blend_weights_
        blend_imp = w[0] * hgb_imp + w[1] * rf_imp + w[2] * et_imp + 0.45 * w[3] * ridge_imp
    ...
    def predict(self, X):
        ...
        global_pred = P @ self.global_blend_weights_
        tree_pred = P[:, :3] @ self.tree_blend_weights_
        linear_pred = P[:, 3]
        gate = self._sigmoid(self.gate_scale_ * (self.gate_tree_.predict(Xf) - self.gate_bias_))
        regime_pred = gate * tree_pred + (1.0 - gate) * linear_pred
        base_pred = self.channel_weights_[0] * global_pred + self.channel_weights_[1] * regime_pred
    ...
    def __str__(self):
        ...
        lines = [
            "Regime Atlas Blend Regressor (Prompt-Aligned)",
            ...
            f"global_blend_weights[hgb, rf, et, ridge]: [{self.global_blend_weights_[0]:.3f}, {self.global_blend_weights_[1]:.3f}, {self.global_blend_weights_[2]:.3f}, {self.global_blend_weights_[3]:.3f}]",
            f"tree_channel_weights[hgb, rf, et]: [{self.tree_blend_weights_[0]:.3f}, {self.tree_blend_weights_[1]:.3f}, {self.tree_blend_weights_[2]:.3f}]",
            f"channel_weights[global_stack, regime_gate]: [{self.channel_weights_[0]:.3f}, {self.channel_weights_[1]:.3f}]",
            f"gate_usage_fraction(tree_channel): {self.gate_usage_:.3f}",
...
RegimeAtlasBlendRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "RegimeAtlasBlend_v1"
model_description = "Two-channel OOF ensemble: global reliability blend plus sample-wise tree-vs-linear gated channel with residual shrinkage and prompt-aligned direct prediction atlas"
model_defs = [(model_shorthand_name, RegimeAtlasBlendRegressor())]
