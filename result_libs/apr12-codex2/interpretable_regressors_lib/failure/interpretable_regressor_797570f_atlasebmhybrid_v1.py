...
class AtlasEBMHybridRegressor(BaseEstimator, RegressorMixin):
    """EBM-anchored multi-expert blend with log-target channel and atlas output."""
    ...
    def _build_models(self, seed_offset=0):
        ...
        ebm = ExplainableBoostingRegressor(
            random_state=seed + 3,
            outer_bags=1,
            max_rounds=int(self.ebm_max_rounds),
            interactions=int(self.ebm_interactions),
            learning_rate=0.03,
        )
        ...
        hgb_log = HistGradientBoostingRegressor(
            max_iter=self.hgb_log_max_iter,
            learning_rate=0.04,
            max_leaf_nodes=max(15, self.hgb_max_leaf_nodes),
            min_samples_leaf=10,
        ...
        return ebm, hgb, hgb_log, rf, et, ridge
    ...
    def _base_predictions(self, X):
        return np.column_stack(
            [
                self.ebm_.predict(X),
                self.hgb_.predict(X),
                self._target_from_logspace(self.hgb_log_.predict(X)),
                self.rf_.predict(X),
                self.et_.predict(X),
    ...
    def fit(self, X, y):
        ...
        if n >= 80:
            tr_idx, va_idx = train_test_split(
                np.arange(n),
                test_size=float(np.clip(self.holdout_size, 0.1, 0.4)),
        ...
        ebm, hgb, hgb_log, rf, et, ridge = self._build_models(seed_offset=29)
        ebm.fit(Xtr, ytr)
        hgb.fit(Xtr, ytr)
        hgb_log.fit(Xtr, ytr_log)
        rf.fit(Xtr, ytr)
        et.fit(Xtr, ytr)
        ridge.fit(Xtr, ytr)

        P_holdout = np.column_stack(
            [
                ebm.predict(Xva),
                hgb.predict(Xva),
                self._target_from_logspace(hgb_log.predict(Xva)),
                rf.predict(Xva),
                et.predict(Xva),
                ridge.predict(Xva),
            ]
        ...
        data_blend = self._fit_nonnegative_blend(P_holdout, yva)
        rel_blend = self._reliability_weights(P_holdout, yva)
        mix = float(np.clip(self.reliability_mix, 0.0, 1.0))
        self.blend_weights_ = (1.0 - mix) * data_blend + mix * rel_blend
        self.blend_weights_ = self.blend_weights_ / np.sum(self.blend_weights_)

        self.ebm_, self.hgb_, self.hgb_log_, self.rf_, self.et_, self.ridge_ = self._build_models(seed_offset=0)
        self.ebm_.fit(Xf, y)
        self.hgb_.fit(Xf, y)
        ...
        base_train_pred = self._base_predictions(Xf) @ self.blend_weights_
        residual = y - base_train_pred
        self.residual_hgb_ = HistGradientBoostingRegressor(
            max_iter=70,
            learning_rate=0.05,
            max_leaf_nodes=15,
            min_samples_leaf=12,
        ...
        self.residual_hgb_.fit(Xf, residual)
        resid_pred = self.residual_hgb_.predict(Xva)
        base_val = P_holdout @ self.blend_weights_
        best_scale, best_mse = 0.0, np.inf
        for s in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25]:
            mse = float(np.mean((yva - (base_val + s * resid_pred)) ** 2))
            if mse < best_mse:
                best_mse = mse
                best_scale = float(s)
        self.residual_scale_ = float(np.clip(best_scale, 0.0, self.residual_strength))

        y_hat = self.predict(Xf)
        self.surrogate_intercept_, self.surrogate_coef_ = self._fit_linear_closed_form(Xf, y_hat, alpha=1e-2)

        ebm_imp = self._approx_effect_importance(self.ebm_, Xf)
        hgb_imp = self._approx_effect_importance(self.hgb_, Xf)
        hgb_log_imp = self._approx_effect_importance(self.hgb_log_, Xf)
        ...
        resid_imp = np.abs(np.asarray(self._fit_linear_closed_form(Xf, self.residual_hgb_.predict(Xf), alpha=1e-2)[1]))
        ...
        imp = 0.78 * blend_imp + 0.22 * self.residual_scale_ * resid_imp
    ...
    def predict(self, X):
        ...
        X = np.asarray(X, dtype=float)
        Xf = self._impute(X)
        base_pred = self._base_predictions(Xf) @ self.blend_weights_
        return base_pred + self.residual_scale_ * self.residual_hgb_.predict(Xf)
    ...
    def __str__(self):
        ...
        lines = [
            "Atlas EBM Hybrid Regressor (Prompt-Aligned)",
            ...
            f"blend_weights[ebm, hgb, hgb_log, rf, et, ridge]: [{self.blend_weights_[0]:.3f}, {self.blend_weights_[1]:.3f}, {self.blend_weights_[2]:.3f}, {self.blend_weights_[3]:.3f}, {self.blend_weights_[4]:.3f}, {self.blend_weights_[5]:.3f}]",
...
AtlasEBMHybridRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "AtlasEBMHybrid_v1"
model_description = "EBM-anchored blend of raw/log HistGBR, RF, ExtraTrees, and Ridge with holdout simplex weighting, reliability mixing, and light residual correction plus query-atlas string"
model_defs = [(model_shorthand_name, AtlasEBMHybridRegressor())]
