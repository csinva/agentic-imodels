...
class RidgeCVCleanRegressor(BaseEstimator, RegressorMixin):
    """RidgeCV over a log-spaced alpha grid; reports full linear formula."""
    ...
    def fit(self, X, y):
        ...
        self.ridge_ = RidgeCV(alphas=np.logspace(-3, 3, 13)).fit(Xs, y)
        # Map back to original scale
        scale = self.scaler_.scale_
        mean = self.scaler_.mean_
        self.coef_ = self.ridge_.coef_ / scale
        self.intercept_ = float(self.ridge_.intercept_ - np.sum(self.coef_ * mean))
    ...
    def __str__(self):
        ...
        lines = [
            f"Ridge Regression (L2, α={self.alpha_:.4g} chosen by CV):",
...
model_description = "RidgeCV on standardized X, report formula in original feature scale"
