...
class HingeLinearSmallRegressor(BaseEstimator, RegressorMixin):
    """Per-feature: {x, max(0,x-med)}; Lasso select top 6; OLS refit; per-feature printing."""

    def __init__(self, max_basis=6):
        self.max_basis = max_basis

    def _basis(self, X):
        return np.hstack([X, np.maximum(0.0, X - self.medians_)])
    ...
    def fit(self, X, y):
        ...
        B = self._basis(X)
        scaler = StandardScaler().fit(B); Bs = scaler.transform(B)
        lasso = LassoCV(cv=3, n_alphas=20, max_iter=5000).fit(Bs, y)
    ...
    def __str__(self):
        ...
        for c, idx in zip(self.coef_, self.support_):
            ...
            else: j = idx - d; per_feat[j].append(("hinge", c, float(self.medians_[j])))
        lines = ["Additive piecewise-linear model: y = intercept + sum_j f_j(x_j)",
                 "  each f_j(x) = a_j*x + b_j*max(0, x - med_j)",
                 f"  intercept: {self.intercept_:.4f}", "", "Per-feature shape:"]
        for j in range(d):
            terms = per_feat.get(j, [])
            if not terms: lines.append(f"  x{j}: (excluded, no effect)"); continue
            parts = []
            for kind, c, m in terms:
                if kind == "lin": parts.append(f"{c:+.4g} * x{j}")
                else: parts.append(f"{c:+.4g} * max(0, x{j} - {m:+.3f})")
            lines.append(f"  x{j}: " + " + ".join(parts))
        return "\n".join(lines)
    """Cluster correlated features (|corr|>0.9), keep one representative per cluster; OLS."""
    ...
    def fit(self, X, y):
        ...
        C = np.abs(Xn.T @ Xn / n)
        # Greedy clustering: pick feature with max |corr(y)|, exclude correlated
        yn = (y - y.mean()) / (y.std() + 1e-12)
        yc = np.abs(Xn.T @ yn / n)
        remaining = set(range(d))
        kept = []
        while remaining:
            j = max(remaining, key=lambda i: yc[i])
            kept.append(j); remaining.discard(j)
    ...
    def __str__(self):
        ...
        lines = [f"OLS Linear Regression (1 representative per correlated-feature cluster, |corr|>0.9 threshold):",
        ...
        if excluded: lines.append(f"Features collapsed into correlated-feature clusters (excluded): {', '.join(excluded)}")
        return "\n".join(lines)
    """Automatic Relevance Determination (ARDRegression) — sparsifies irrelevant coefs."""
    ...
    def fit(self, X, y):
        ...
        ard = ARDRegression(threshold_lambda=1e4).fit(Xs, y)
        self.coef_ = ard.coef_ / scaler.scale_
        self.intercept_ = float(ard.intercept_ - np.sum(self.coef_ * scaler.mean_))
        return self
    ...
    def __str__(self):
        ...
        lines = ["ARD Linear Regression (automatic relevance determination — irrelevant features zeroed):",
        ...
        return "\n".join(lines)
    """OLS, compute t-statistics per coef, keep those with |t|>1.96, refit OLS."""
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            yhat = X1 @ beta
            sigma2 = float(np.sum((y - yhat) ** 2)) / max(1, n - d - 1)
            cov = sigma2 * np.linalg.pinv(X1.T @ X1)
            se = np.sqrt(np.diag(cov))
            t_stats = beta / (se + 1e-12)
            keep = np.where(np.abs(t_stats[1:]) > 1.96)[0]
        except Exception:
            keep = np.arange(d)
        if keep.size == 0: keep = np.arange(d)
    ...
    def __str__(self):
        ...
        terms = " + ".join(f"{c:+.4g}*x{j}" for c, j in zip(self.coef_, self.support_))
        excluded = [f"x{j}" for j in range(self.d_) if j not in self.support_]
        lines = [f"OLS Linear Regression (features with |t-stat|>1.96 retained, {len(self.support_)} kept):",
                 f"  y = {self.intercept_:+.4f} + {terms}", "", "Coefficients:",
                 f"  intercept: {self.intercept_:.4f}"]
        for c, j in zip(self.coef_, self.support_): lines.append(f"  x{j}: {c:.4f}")
        if excluded: lines.append(f"Features not significant (excluded): {', '.join(excluded)}")
        return "\n".join(lines)


class _UnusedB(BaseEstimator, RegressorMixin):
    """LassoLarsIC (BIC) feature selection; OLS refit on active set."""
    ...
    def fit(self, X, y):
        ...
        self.d_ = X.shape[1]
        scaler = StandardScaler().fit(X); Xs = scaler.transform(X)
        lars = LassoLarsIC(criterion="bic", max_iter=200).fit(Xs, y)
        keep = np.where(np.abs(lars.coef_) > 1e-10)[0]
        if keep.size == 0: keep = np.array([int(np.argmax(np.abs(lars.coef_)))]) if np.any(lars.coef_) else np.arange(min(1, self.d_))
        self.support_ = sorted(keep.tolist())
        ols = LinearRegression().fit(X[:, self.support_], y)
        self.coef_ = ols.coef_; self.intercept_ = float(ols.intercept_)
        return self
    ...
    def __str__(self):
        ...
        terms = " + ".join(f"{c:+.4g}*x{j}" for c, j in zip(self.coef_, self.support_))
        excluded = [f"x{j}" for j in range(self.d_) if j not in self.support_]
        lines = [f"BIC-selected Sparse OLS (LassoLarsIC selected {len(self.support_)} features, OLS refit):",
                 f"  y = {self.intercept_:+.4f} + {terms}", "", "Coefficients:",
                 f"  intercept: {self.intercept_:.4f}"]
        for c, j in zip(self.coef_, self.support_): lines.append(f"  x{j}: {c:.4f}")
...
class _UnusedA(BaseEstimator, RegressorMixin):
    """LassoLarsIC (AIC) feature selection; OLS refit on active set."""
    ...
    def fit(self, X, y):
        ...
        self.d_ = X.shape[1]
        scaler = StandardScaler().fit(X); Xs = scaler.transform(X)
        lars = LassoLarsIC(criterion="aic", max_iter=200).fit(Xs, y)
        keep = np.where(np.abs(lars.coef_) > 1e-10)[0]
        if keep.size == 0: keep = np.array([int(np.argmax(np.abs(lars.coef_)))]) if np.any(lars.coef_) else np.arange(min(1, self.d_))
        self.support_ = sorted(keep.tolist())
        ols = LinearRegression().fit(X[:, self.support_], y)
    ...
    def __str__(self):
        ...
        terms = " + ".join(f"{c:+.4g}*x{j}" for c, j in zip(self.coef_, self.support_))
        excluded = [f"x{j}" for j in range(self.d_) if j not in self.support_]
        lines = [f"AIC-selected Sparse OLS (LassoLarsIC selected {len(self.support_)} features, OLS refit):",
                 f"  y = {self.intercept_:+.4f} + {terms}", "", "Coefficients:",
                 f"  intercept: {self.intercept_:.4f}"]
        for c, j in zip(self.coef_, self.support_): lines.append(f"  x{j}: {c:.4f}")
        if excluded: lines.append(f"Features with zero coefficients (excluded): {', '.join(excluded)}")
...
HingeLinearSmallRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "HingeLinearSmall_v1"
model_description = "Per-feature {x, (x-med)_+} basis, Lasso-select 6, OLS refit"
model_defs = [(model_shorthand_name, HingeLinearSmallRegressor())]
