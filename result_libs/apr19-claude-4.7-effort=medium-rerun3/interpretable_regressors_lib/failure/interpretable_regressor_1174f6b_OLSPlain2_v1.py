...
class OLSAvgContribRegressor(BaseEstimator, RegressorMixin):
    """Plain OLS; __str__ reports avg |contribution| of each feature over training data."""
    ...
    def fit(self, X, y):
        ...
        # Average absolute contribution of each feature on training set
        self.mean_abs_contrib_ = np.mean(np.abs(X * self.coef_), axis=0)
        return self
...
OLSAvgContribRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "OLSPlain2_v1"
model_description = "Plain OLS (duplicate of baseline idea, minimal clean formula print)"
model_defs = [(model_shorthand_name, OLSAvgContribRegressor())]
