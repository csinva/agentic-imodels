...
class LARSPathRegressor(BaseEstimator, RegressorMixin):
    """Lars with small n_nonzero_coefs cap (5), OLS refit in original scale."""

    def __init__(self, n_keep=5):
        self.n_keep = n_keep
    ...
    def fit(self, X, y):
        ...
        lars = Lars(n_nonzero_coefs=min(self.n_keep, d)).fit(Xs, y)
        keep = np.where(np.abs(lars.coef_) > 1e-10)[0]
        if keep.size == 0: keep = np.arange(min(self.n_keep, d))
    ...
    def __str__(self):
        ...
        lines = [f"LARS Sparse OLS ({len(self.support_)} features selected by least-angle regression):",
...
LARSPathRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "LARSPath_v1"
model_description = "Lars with top-5 nonzero coefs, refit OLS in original feature scale"
model_defs = [(model_shorthand_name, LARSPathRegressor())]
