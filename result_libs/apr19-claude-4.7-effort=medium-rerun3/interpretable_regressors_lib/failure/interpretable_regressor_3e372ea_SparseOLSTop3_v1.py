...
class SparseOLSTop3Regressor(BaseEstimator, RegressorMixin):
    """Extreme sparsity: only 3 features selected (by LassoCV |coef|), OLS refit. Maximally readable formula."""
    ...
    def fit(self, X, y):
        ...
        lasso = LassoCV(cv=min(3, max(2, n // 20)), n_alphas=20, max_iter=5000).fit(Xs, y)
        order = np.argsort(-np.abs(lasso.coef_))
        kept = [int(i) for i in order if lasso.coef_[i] != 0][: self.max_features]
        if not kept:
            kept = [int(np.argmax(np.abs(lasso.coef_)))] if np.any(lasso.coef_) else list(range(min(3, d)))
    ...
    def __str__(self):
        ...
        lines = [
            f"SparseOLSTop3 Linear Regression (only {len(self.support_)} features selected):",
        ...
        if excluded:
            lines.append(f"Features with zero coefficients (excluded): {', '.join(excluded)}")
        return "\n".join(lines)


import sys as _sys
_sys.modules.setdefault("interpretable_regressor", _sys.modules[__name__])
SparseOLSTop3Regressor.__module__ = "interpretable_regressor"

model_shorthand_name = "SparseOLSTop3_v1"
model_description = "LassoCV selects top-3 features, refit OLS; very sparse linear formula"
model_defs = [(model_shorthand_name, SparseOLSTop3Regressor())]
