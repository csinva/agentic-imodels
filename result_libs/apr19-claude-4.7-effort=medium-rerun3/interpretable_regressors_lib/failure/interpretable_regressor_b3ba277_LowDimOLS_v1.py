...
class LowDimOLSRegressor(BaseEstimator, RegressorMixin):
    """Keep only features with variance >= 1% of max; OLS on remaining."""

    def fit(self, X, y):
        X = np.asarray(X, dtype=float); y = np.asarray(y, dtype=float)
        self.d_ = X.shape[1]
        v = X.var(axis=0)
        keep = np.where(v >= 0.01 * v.max())[0] if v.max() > 0 else np.arange(self.d_)
        if keep.size == 0: keep = np.arange(min(1, self.d_))
    ...
    def __str__(self):
        ...
        lines = ["OLS Linear Regression (low-variance features dropped):",
        ...
        if excluded: lines.append(f"Constant/near-constant features dropped: {', '.join(excluded)}")
        return "\n".join(lines)
...
LowDimOLSRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "LowDimOLS_v1"
model_description = "OLS after dropping features with var < 1% of max variance"
model_defs = [(model_shorthand_name, LowDimOLSRegressor())]
