...
class GreedyForwardOLSRegressor(BaseEstimator, RegressorMixin):
    """Greedy forward selection (maximize train R^2) up to max_feat, then OLS."""

    def __init__(self, max_feat=12):
    ...
    def fit(self, X, y):
        ...
        ybar = y.mean(); sse_null = np.sum((y - ybar) ** 2)
        best_sse = sse_null
        for _ in range(min(self.max_feat, d)):
            ...
            for j in remaining:
                ...
                Xs = X[:, cols]
                try:
                    b, *_ = np.linalg.lstsq(np.column_stack([np.ones(n), Xs]), y, rcond=None)
                    pred = np.column_stack([np.ones(n), Xs]) @ b
                    sse = np.sum((y - pred) ** 2)
                except Exception:
                    sse = best_sse
                if sse < best_new - 1e-10:
                    best_new = sse; best_j = j
            if best_j < 0: break
...
model_shorthand_name = "GreedyForwardOLS12_v1"
model_description = "Greedy forward feature selection (SSE-minimizing) up to 12, then OLS"
