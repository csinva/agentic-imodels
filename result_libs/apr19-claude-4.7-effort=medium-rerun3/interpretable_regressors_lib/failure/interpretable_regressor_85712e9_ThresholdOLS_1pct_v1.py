...
class ThresholdOLS1pctRegressor(BaseEstimator, RegressorMixin):
    """OLS on standardized features, prune coefficients whose std-magnitude is <1% of max; refit OLS."""
    ...
    def fit(self, X, y):
        ...
        absc = np.abs(full.coef_)
        mx = absc.max() if absc.size else 0.0
        if mx > 0:
            keep = np.where(absc >= self.threshold * mx)[0]
        else:
            keep = np.arange(d)
        if keep.size == 0: keep = np.array([int(np.argmax(absc)) if absc.size else 0])
    ...
    def __str__(self):
        ...
        lines = [f"OLS Linear Regression (negligible coefs pruned at 1% of max):",
...
ThresholdOLS1pctRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "ThresholdOLS_1pct_v1"
model_description = "OLS, prune std-coefs below 1% of max, refit OLS"
model_defs = [(model_shorthand_name, ThresholdOLS1pctRegressor())]
