...
class SparseOLSTop5Regressor(BaseEstimator, RegressorMixin):
    """LassoCV selects top-K features (by |coef| on standardized X), refit OLS on original scale."""

    def __init__(self, max_features=5, cv=3):
    ...
    def fit(self, X, y):
        ...
        lasso = LassoCV(cv=min(self.cv, max(2, n // 20)), n_alphas=20, max_iter=5000).fit(Xs, y)
    ...
    def predict(self, X):
        ...
        return X[:, self.support_] @ self.coef_ + self.intercept_
    ...
    def __str__(self):
        ...
        lines = [
            f"SparseOLS (top-{self.max_features} by LassoCV, refit OLS):",
...
SparseOLSTop5Regressor.__module__ = "interpretable_regressor"

model_shorthand_name = "SparseOLSTop5_v1"
model_description = "LassoCV selects top-5 features, refit OLS; clear sparse linear formula"
model_defs = [(model_shorthand_name, SparseOLSTop5Regressor())]
