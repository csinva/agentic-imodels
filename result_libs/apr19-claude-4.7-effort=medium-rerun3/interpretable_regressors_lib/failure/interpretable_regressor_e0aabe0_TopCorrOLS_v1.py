...
class TopCorrOLSRegressor(BaseEstimator, RegressorMixin):
    """Select top-K features by |Pearson correlation with y|, fit OLS."""
    ...
    def fit(self, X, y):
        ...
        self.support_ = sorted(np.argsort(-corr)[:k].tolist())
    ...
    def __str__(self):
        ...
        lines = [
            f"TopCorrOLS (top-{len(self.support_)} features by |corr(x_j, y)|, OLS refit):",
        ...
        for c, j in zip(self.coef_, self.support_):
            lines.append(f"  x{j}: {c:.4f}  (|corr|={self.corr_[j]:.3f})")
        if excluded:
            lines.append(f"Features excluded (low correlation): {', '.join(excluded)}")
        return "\n".join(lines)


import sys as _sys
_sys.modules.setdefault("interpretable_regressor", _sys.modules[__name__])
TopCorrOLSRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "TopCorrOLS_v1"
model_description = "Top-6 features by |correlation with y|, OLS refit"
model_defs = [(model_shorthand_name, TopCorrOLSRegressor())]
