...
class RidgeCVCleanRegressor(BaseEstimator, RegressorMixin):
    """RidgeCV on standardized X, translate coefs back to original scale, print cleanly."""
    ...
    def fit(self, X, y):
        ...
        ridge = RidgeCV(alphas=alphas, cv=3).fit(Xs, y)
        self.coef_ = ridge.coef_ / self.sd_
        self.intercept_ = float(ridge.intercept_ - np.dot(ridge.coef_, self.mu_ / self.sd_))
        return self
    ...
    def __str__(self):
        ...
        lines = ["RidgeCV (L2-regularized linear regression; dense linear formula):",
...
RidgeCVCleanRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "RidgeCVClean_v1"
model_description = "RidgeCV on standardized X, coefs back-transformed to original scale; dense linear formula"
model_defs = [(model_shorthand_name, RidgeCVCleanRegressor())]
