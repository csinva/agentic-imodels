...
class GreedyForwardOLSRegressor(BaseEstimator, RegressorMixin):
    """Greedy forward selection with CV-based early stopping (patience=2)."""

    def __init__(self, max_feat=15, n_splits=3, patience=2):
        self.max_feat = max_feat
        self.n_splits = n_splits
        self.patience = patience

    def _cv_sse(self, X, y, cols, n_splits):
        n = len(y)
        idx = np.arange(n); rng = np.random.RandomState(0); rng.shuffle(idx)
        folds = np.array_split(idx, n_splits)
        total = 0.0
        for k in range(n_splits):
            test = folds[k]; train = np.concatenate([folds[i] for i in range(n_splits) if i != k])
            Xt = np.column_stack([np.ones(len(train)), X[train][:, cols]])
            Xv = np.column_stack([np.ones(len(test)), X[test][:, cols]])
            try:
                b, *_ = np.linalg.lstsq(Xt, y[train], rcond=None)
                pred = Xv @ b
                total += np.sum((y[test] - pred) ** 2)
            except Exception:
                return float("inf")
        return total
    ...
    def fit(self, X, y):
        ...
        best_cv = float("inf"); best_selected = []; no_improve = 0
        for _ in range(min(self.max_feat, d)):
            best_j, best_new = -1, float("inf")
            for j in remaining:
                cols = selected + [j]
                # train SSE for next-feature choice
                Xs = np.column_stack([np.ones(n), X[:, cols]])
                b, *_ = np.linalg.lstsq(Xs, y, rcond=None)
                sse = np.sum((y - Xs @ b) ** 2)
                if sse < best_new: best_new = sse; best_j = j
            if best_j < 0: break
            selected.append(best_j); remaining.remove(best_j)
            cv = self._cv_sse(X, y, selected, self.n_splits)
            if cv < best_cv - 1e-9:
                best_cv = cv; best_selected = list(selected); no_improve = 0
            else:
                no_improve += 1
                if no_improve >= self.patience: break
        if not best_selected: best_selected = [0]
        self.support_ = sorted(best_selected)
...
model_shorthand_name = "GreedyForwardOLS_CV_v1"
model_description = "Greedy forward feature selection with 3-fold CV early-stopping (patience=2), OLS refit"
