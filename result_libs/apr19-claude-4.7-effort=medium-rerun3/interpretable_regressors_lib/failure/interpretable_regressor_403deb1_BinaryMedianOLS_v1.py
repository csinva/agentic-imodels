...
class BinaryMedianOLSRegressor(BaseEstimator, RegressorMixin):
    """Threshold each feature at its median -> binary indicator, then OLS."""

    def fit(self, X, y):
        X = np.asarray(X, dtype=float); y = np.asarray(y, dtype=float)
        n, d = X.shape; self.d_ = d
        self.medians_ = np.median(X, axis=0)
        Z = (X > self.medians_).astype(float)
    ...
    def predict(self, X):
        ...
        Z = (np.asarray(X, dtype=float) > self.medians_).astype(float)
        return Z @ self.coef_ + self.intercept_

    def __str__(self):
        check_is_fitted(self, "coef_")
        lines = ["BinaryMedianOLS: each feature thresholded at its training median to a 0/1 indicator; linear model on indicators:",
                 f"  y = {self.intercept_:+.4f} + sum_j c_j * I[x_j > median_j]", "",
                 f"  intercept: {self.intercept_:.4f}"]
        for j, (c, m) in enumerate(zip(self.coef_, self.medians_)):
            lines.append(f"  I[x{j} > {m:.4g}]: {c:+.4f}")
        return "\n".join(lines)
...
BinaryMedianOLSRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "BinaryMedianOLS_v1"
model_description = "Median-threshold each feature to 0/1 indicators, OLS on indicators"
model_defs = [(model_shorthand_name, BinaryMedianOLSRegressor())]
