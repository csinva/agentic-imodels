...
class OLSPlainRegressor(BaseEstimator, RegressorMixin):
    """Plain OLS on standardized features, reported as y = intercept + sum c_i*x_i (original scale)."""
    ...
    def __str__(self):
        check_is_fitted(self, "ols_")
        coef = self.ols_.coef_
        b = float(self.ols_.intercept_)
        eq = " + ".join(f"{c:+.4g}*x{i}" for i, c in enumerate(coef))
        lines = [f"OLS Linear Regression:  y = {b:+.4f} + {eq}", "", "Coefficients:"]
        for i, c in enumerate(coef):
            lines.append(f"  x{i}: {c:.4f}")
        lines.append(f"  intercept: {b:.4f}")
        return "\n".join(lines)


import sys as _sys
_sys.modules.setdefault("interpretable_regressor", _sys.modules[__name__])
OLSPlainRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "OLSPlain_v1"
model_description = "Plain OLS on raw features; y = intercept + sum c_i * x_i"
model_defs = [(model_shorthand_name, OLSPlainRegressor())]
